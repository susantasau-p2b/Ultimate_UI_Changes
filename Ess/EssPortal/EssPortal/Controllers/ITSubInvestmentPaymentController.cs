...
namespace P2BUltimate.Controllers.Payroll.MainController
{
    public class ITSubInvestmentPaymentController : Controller
    {
        ...
        {
            return View("~/Views/Shared/_ITSubInvestmentPayment.cshtml");
        }

        public ActionResult itsubinvestment_partial()
        {
            return View("~/Views/Shared/_ITSubInvestment.cshtml");
        }
                    ...
                    if (ModelState.IsValid)
                    {
                        ...
                        try
                        {
                            using (TransactionScope ts = new TransactionScope())
                            {
                                ...
                                {
                                    return Json(new { status = true, responseText = "Code Already Exists." }, JsonRequestBehavior.AllowGet);
                                }
        ...
        [HttpPost]
        public ActionResult GetITSubInvLKDetails(List<int> SkipIds)
        {
            using (DataBaseContext db = new DataBaseContext())
            {
