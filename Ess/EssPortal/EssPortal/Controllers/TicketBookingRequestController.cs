...
namespace EssPortal.Controllers
{
    public class TicketBookingRequestController : Controller
    {
        ...
        {
            return View("~/Views/TicketBookingRequest/Index.cshtml");
        }
        ...
        public ActionResult ticketbookPartialSanction()
        {
            return View("~/Views/Shared/_TicketBookReqOnSanction.cshtml");
        }
        ...
        public class ChildGetTicketBookReqClass
        {
        ...
        public class GetTicketBookingReqClass
        {
            ...
            public ChildGetTicketBookReqClass RowData { get; set; }
        }


        public ActionResult GetMyTicketBookingReq()
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                string authority = Convert.ToString(Session["auho"]);

                List<GetTicketBookingReqClass> OHotelBookinglist = new List<GetTicketBookingReqClass>();
                ...
                var db_data = db.EmployeePayroll
                      ...
                      .Include(e => e.TicketBookingRequest.Select(a => a.HotBookReqDetails))
                     .FirstOrDefault();


                if (db_data != null)
                {
                    List<GetTicketBookingReqClass> returndata = new List<GetTicketBookingReqClass>();
                    returndata.Add(new GetTicketBookingReqClass
                    {
                    ...
                    var TicketBookReqDetailslist = db_data.TicketBookingRequest.ToList();

                    foreach (var Ticketitems in TicketBookReqDetailslist)
                    {
                        int WfStatusNew = Ticketitems.HotBookReqDetails.Select(w => w.WFStatus).LastOrDefault();
                        string Comments = Ticketitems.HotBookReqDetails.Select(c => c.Comments).LastOrDefault();
                        ...
                        {
                            GetTicketBookingReqClass ObjTicketBookingRequest = new GetTicketBookingReqClass()
                            {
                                RowData = new ChildGetTicketBookReqClass
                                {
                                    LvNewReq = Ticketitems.Id.ToString(),
                                    EmpLVid = db_data.Id.ToString()

                                },
                                FamilyMember = Ticketitems.TotFamilyMembers.ToString(),
                                BillNo = Ticketitems.BillNo,
                                ReqDate = Ticketitems.ReqDate.Value.ToShortDateString(),
                                BillAmount = Ticketitems.BillAmount != 0 ? Ticketitems.BillAmount.ToString() : "0",
                                Status = StatusNarration
                            };
                            returndata.Add(ObjTicketBookingRequest);
                        }
        ...
        public class ChildGetTBOOKReqClass
        {
            public string LvNewReq { get; set; }
            public string EmpLVid { get; set; }
            public string IsClose { get; set; }
        ...
        public class GetTBookClass1
        {
            ...
            public ChildGetTBOOKReqClass RowData { get; set; }
        }



        #region Get Employee Ticket Book request On Sanction Dropdown

        public ActionResult GetTicketBookReqOnSanction(FormCollection form)
        {
            using (DataBaseContext db = new DataBaseContext())
            {
               ...
               // var returnDataClass = new List<returnDataClass>();
                ...
                List<GetTBookClass1> returndata = new List<GetTBookClass1>();
                returndata.Add(new GetTBookClass1
                {
                    ReqDate = "Requisition Date",
                    BillNo = "Bill No",
                    FamilyMember = "FamilyMember",
                    ...
                    RowData = new ChildGetTBOOKReqClass
                    {
                        LvNewReq = "0",
                        EmpLVid = "0",
                        IsClose = "0",
                ...
                foreach (var item1 in EmpidsWithfunsub)
                {
                    ...
                    var Emps = db.EmployeePayroll
                        ...
                        .Include(e => e.TicketBookingRequest)
                        .Include(e => e.TicketBookingRequest.Select(b => b.HotBookReqDetails))
                        .Include(e => e.TicketBookingRequest.Select(w => w.WFStatus))
                        .ToList();
                    ...
                    {
                        if (item.TicketBookingRequest != null && item.TicketBookingRequest.Count() > 0)
                        {
                            var LvIds = UserManager.FilterTicketBOOK(item.TicketBookingRequest.OrderByDescending(e => e.ReqDate.Value.ToShortDateString()).ToList(),
                               Convert.ToString(Session["auho"]), Convert.ToInt32(SessionManager.CompanyId));

                            if (LvIds.Count() > 0)
                            {
                                var ticketbookingreqdata = item.TicketBookingRequest.Where(e => LvIds.Contains(e.Id)).ToList();
                                foreach (var singleTBookDetails in ticketbookingreqdata)
                                {
                                    if (singleTBookDetails.HotBookReqDetails != null)
                                    {
                                        ...
                                        returndata.Add(new GetTBookClass1
                                        {
                                            RowData = new ChildGetTBOOKReqClass
                                            {
                                                LvNewReq = singleTBookDetails.Id.ToString(),
                                                EmpLVid = item.Id.ToString(),
                                                IsClose = EmpR.ToString(),
                                                LvHead_Id = "",
                                            },

                                            ReqDate = singleTBookDetails.ReqDate.Value.ToShortDateString(),
                                            BillNo = singleTBookDetails.BillNo,
                                            FamilyMember = singleTBookDetails.TotFamilyMembers.ToString(),
                                            BillAmount = singleTBookDetails.BillAmount != 0 ? singleTBookDetails.BillAmount.ToString() : "0",
        ...
        public class EmpTicketREquestdata
        {

            public string Status { get; set; }
            public string Emplvhead { get; set; }
            public int TravelEligibilityPolicy_Id { get; set; }
        ...
        #region Get Employee TicketBookrequest On Sanction Bind DATA

        public ActionResult GetTicketBookingData(string data)
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                string authority = Convert.ToString(Session["auho"]);
                ...
                var W = db.EmployeePayroll
                    .Include(e => e.TicketBookingRequest)
                    .Include(e => e.Employee.EmpName)
                    .Include(e => e.TicketBookingRequest.Select(t => t.GeoStruct))
                    .Include(e => e.TicketBookingRequest.Select(t => t.FuncStruct))
                    .Include(e => e.TicketBookingRequest.Select(t => t.PayStruct))
                    .Include(e => e.TicketBookingRequest.Select(t => t.HotBookReqDetails))
                    .Include(e => e.TicketBookingRequest.Select(t => t.WFStatus))
                    .Include(e => e.TicketBookingRequest.Select(t => t.TravelEligibilityPolicy))
                    .Include(e => e.TicketBookingRequest.Select(t => t.JourneyDetails))
                    .Include(e => e.TicketBookingRequest.Select(t => t.JourneyDetails.Select(j => j.JourneyObject)))
                    .Include(e => e.TicketBookingRequest.Select(t => t.FamilyDetails))
                    .Include(e => e.TicketBookingRequest.Select(t => t.FamilyDetails.Select(a => a.MemberName)))


                    .Where(e => e.Employee.Id == EmpLvIdint && e.TicketBookingRequest.Any(w => w.Id == id)).SingleOrDefault();

                var v = W.TicketBookingRequest.Where(e => e.Id == id).Select(s => new EmpTicketREquestdata
                {
                    EmployeeId = W.Employee.Id,
                    EmployeeName = W.Employee.EmpCode + " " + W.Employee.EmpName.FullNameFML,
                    Lvnewreq = s.Id,
                    Empcode = W.Employee.EmpCode,
                    TravelEligibilityPolicy_Id = s.TravelEligibilityPolicy != null ? s.TravelEligibilityPolicy.Id : 0,
                    ...
                    Eligible_BillAmount = s.Elligible_BillAmount,
        ...
        public ActionResult Update_TicketBOOKReq(TicketBookingRequest TbReq, FormCollection form, String data)
        {
            ...
            var ids = Utility.StringIdsToListString(data);
            var Tbnewreqid = Convert.ToInt32(ids[0]);
            ...
            using (DataBaseContext db = new DataBaseContext())
            {
                ...
                try
                {
                    using (TransactionScope ts = new TransactionScope())
                    {
                        var TicketbookingList = query.TicketBookingRequest.Where(e => e.Id == Tbnewreqid).ToList();
                        //if someone reject lv
                        foreach (var Tbitems in TicketbookingList)
                        {
                            ...
                            if (authority.ToUpper() == "SANCTION")
                            {
                                ...
                                if (Convert.ToBoolean(Sanction) == true)
                                {
                                    ...
                                    Tbitems.TrClosed = Convert.ToBoolean(isClose) == true ? true : false;
                                    Tbitems.WFStatus = db.Lookup.Include(e => e.LookupValues).Where(e => e.Code == "1000").FirstOrDefault().LookupValues.Where(e => e.LookupVal == "1").FirstOrDefault();
                            ...
                            else if (authority.ToUpper() == "APPROVAL")//Hr
                            {
                                ...
                                if (Convert.ToBoolean(Approval) == true)
                                {
                                    ...
                                    Tbitems.TrClosed = Convert.ToBoolean(isClose) == true ? true : false;
                                    Tbitems.WFStatus = db.Lookup.Include(e => e.LookupValues).Where(e => e.Code == "1000").FirstOrDefault().LookupValues.Where(e => e.LookupVal == "3").FirstOrDefault();
                            ...
                            else if (authority.ToUpper() == "RECOMMENDATION")
                            {
                                ...
                                {
                                    //Recomendation yes -7
                                    var CheckAllreadyRecomendation = TicketbookingList.Where(e => e.HotBookReqDetails.Any(r => r.WFStatus == 7)).ToList();
                                ...
                                else if (Convert.ToBoolean(Recomendation) == false)
                                {
                                    ...
                                    var CheckAllreadyRecomendation = TicketbookingList.Where(e => e.HotBookReqDetails.Any(r => r.WFStatus == 8)).ToList();
                                    if (CheckAllreadyRecomendation.Count() > 0)
                                    {
                                        return Json(new Utility.JsonClass { status = false, responseText = "This Leave Allready Rejected....Refresh The Page!" }, JsonRequestBehavior.AllowGet);
                                    }
                            ...
                            if (Tbitems.HotBookReqDetails != null)
                            {
                                oFunctWFDetails_List.AddRange(Tbitems.HotBookReqDetails);
                            }

                            Tbitems.HotBookReqDetails = oFunctWFDetails_List;
                            db.TicketBookingRequest.Attach(Tbitems);
                            db.Entry(Tbitems).State = System.Data.Entity.EntityState.Modified;
                            db.SaveChanges();
