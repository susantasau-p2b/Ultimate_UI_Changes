...
namespace EssPortal.Controllers
{
    public class TravelHotelBookingController : Controller
    {
                    ...
                    if (ModelState.IsValid)
                    {
                        using (TransactionScope ts = new TransactionScope())
                        {
                            ...
                            try
                            {
                                ...
                                return this.Json(new Object[] { journyobjdata.Id, journyobjdata.fulldetails, "Data Saved SucessFully" }, JsonRequestBehavior.AllowGet);
                            }
        ...
        public class returnEditClassh
        {
            ...
            public String Elligible_BillAmount { get; set; }
            public String RatePerDay { get; set; }
            public String StdDiscount { get; set; }
            public String Taxes { get; set; }
            public int HotelEligibilityPolicyId { get; set; }
            public String HotelEligibilityPolicyFulldetails { get; set; }
        ...
        public ActionResult GethotelbookingData(int data)
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                ...
                List<returnEditClassh> oreturnEditClassh = new List<returnEditClassh>();

                oreturnEditClassh.Add(new returnEditClassh
                {
                    Id = ca.Id,
                    HotelName = ca.HotelName,
                    HotelDesc = ca.HotelDesc,
                    CountryId = ca.Country != null ? ca.Country.Id : 0,
                    StateId = ca.State != null ? ca.State.Id : 0,
                    CityId = ca.City != null ? ca.City.Id : 0,
                    BillNo = ca.BillNo,
                    StartDate = ca.StartDate != null ? ca.StartDate.ToString() : "",
                    EndDate = ca.EndDate != null ? ca.EndDate.ToString() : "",
                    NoOfRooms = ca.NoOfRooms.ToString(),
                    Elligible_BillAmount = ca.Elligible_BillAmount.ToString(),
                    RatePerDay = ca.RatePerDay.ToString(),
                    StdDiscount = ca.StdDiscount.ToString(),
                    Taxes = ca.Taxes.ToString(),
                    HotelEligibilityPolicyId = ca.HotelEligibilityPolicy != null ? ca.HotelEligibilityPolicy.Id : 0,
                    HotelEligibilityPolicyFulldetails = ca.HotelEligibilityPolicy != null ? ca.HotelEligibilityPolicy.FullDetails : "",
                    BillAmount = ca.BillAmount.ToString(),
                    Narration = ca.Narration.ToString()
                });

                return Json(oreturnEditClassh, JsonRequestBehavior.AllowGet);
        ...
        public ActionResult EditSave(TravelHotelBooking data1, int data, FormCollection form)
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                ...
                try
                {
                    ...
                    string HotelEligibilitypolicy = form["HotelEligibilityPolicyList"] == "0" ? "" : form["HotelEligibilityPolicyList"];
                    string Countrylist = form["CountryList"] == "0" ? "" : form["CountryList"];
                    string statelist = form["StateList"] == "0" ? "" : form["StateList"];
                    string citylist = form["CityList"] == "0" ? "" : form["CityList"];
                    ...
                    var journyobjdata = db.TravelHotelBooking.Where(e => e.Id == db_data.Id).Select(e => new
                    {
                        Id = e.Id,
                        fulldetails = "HotelName:" + e.HotelName + ",HotelDesc" + e.HotelDesc + ", Narration" + e.Narration
                    }).SingleOrDefault();
