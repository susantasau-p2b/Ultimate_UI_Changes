...
namespace EssPortal.Models
{
   ...
   where T : class
        ...
        //public void DBTrackSave(DbPropertyValues originalBlogValues)
        //{
        //    var changeTrack = db.ChangeTracker.Entries().Where(p => p.State == System.Data.Entity.EntityState.Modified);
        ...
        //                    entityName = ObjectContext.GetObjectType(entry.Entity.GetType()).Name;
        //                    state = entry.State.ToString();
        //                    DT_StandardITRebate stdIT = new DT_StandardITRebate()
        //                    {
        //                        StartAmount = (!entry.CurrentValues["StartAmount"].Equals(originalBlogValues["StartAmount"]) ? (float)originalBlogValues["StartAmount"] : 0),
        //                        EndAmount = (!entry.CurrentValues["EndAmount"].Equals(originalBlogValues["EndAmount"]) ? (float)originalBlogValues["EndAmount"] : 0),
        //                        DeductionAmount = (!entry.CurrentValues["DeductionAmount"].Equals(originalBlogValues["DeductionAmount"]) ? (float)originalBlogValues["DeductionAmount"] : 0),
        //                        DeductionPerc = (!entry.CurrentValues["DeductionPerc"].Equals(originalBlogValues["DeductionPerc"]) ? (float)originalBlogValues["DeductionPerc"] : 0),
        //                        Orig_Id = (int)originalBlogValues["Id"],
        //                        RowVersion = (byte[])originalBlogValues["RowVersion"],
        //                        DBTrack = (P2b.Global.DBTrack)originalBlogValues["DBTrack"]
        //                    };
