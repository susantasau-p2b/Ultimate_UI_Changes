...
namespace EssPortal.Process
{
    public static class LeaveHeadProcess
    {
        ...
        public static ReturnDatacalendarpara LeaveCalendarparaold(int LVheadid, int EmployeeLVId)
        {
            ReturnDatacalendarpara RetDataparam = new ReturnDatacalendarpara();
            using (DataBaseContext db = new DataBaseContext())
            {
        ...
        public static EmployeeLeave _returnCompanyLeave(Int32 CompanyId, Int32 EmpId)
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                EmployeeLeave OCompanyLeave = db.EmployeeLeave.Where(e => e.Id == EmpId)
        ...
        public static double DayDateIsWeaklyOff(DateTime Odate, int EmployeeId)
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                ...
                //}
                //List<WeeklyOffCalendar> OnWeaklyOff = OCompany.WeeklyOffCalendar
        ...
        public static returnDataClass GetHolidayCount(LvNewReq OLvNewReq, int CompanyId)
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                db.Configuration.AutoDetectChangesEnabled = false;
                    ...
                    else if (OLvNewReq.LeaveHead.LvCode == "PL")//IF CL Balance is zero then if  Employee Requested for PL , It exclude Holiday and Weekly off but it is allowed for only 5 working days
                    {
                        ...
                        if (OEmpLvHistory != null)
                        {
                            if (OEmpLvHistory.CloseBal == 0)
                            {
                                ...
                                {
                                    pld = pld + HolidayCount(OLvNewReq, CompanyId); //debit holidays
                                }
                                if (actualpd <= 5)
                                {
