        ...
        return this.each(function () {
            $(this).keydown(function (e) {
                ...
                if (!e.shiftKey && !e.altKey && !e.ctrlKey && !($(this).val().split(".").length > 1 && (key == 110 || key == 190)) &&
                   ...
                   key == 46 || key == 45))
                    return true;
                ShowErrorMessage(this, 'Please Enter Valid Percentange!');
                return false;
            });
            $(this).keyup(function (e) {
                if ($(this).val().indexOf('.') != -1) {
    ...
    */
    function ShowErrorMessage(control, message) {
        ...
        if ($(control)[0].nodeName == "SELECT") {
            ...
            $("<div class='error'>" + message + "</div>").appendTo("body").css({ left: pos.left + w + 20, top: pos.top }).fadeOut(3000);
        } else {
            if ($(control).is(':checkbox')) {
                $("<div class='error'>" + message + "</div>").appendTo("body").css({ left: pos.left + 2, top: pos.top + h + 10 }).fadeOut(3000);
            }
            else {
                $("<div class='error'>" + message + "</div>").appendTo("body").css({ left: pos.left + w + 12, top: pos.top }).fadeOut(3000);
            }
        }
