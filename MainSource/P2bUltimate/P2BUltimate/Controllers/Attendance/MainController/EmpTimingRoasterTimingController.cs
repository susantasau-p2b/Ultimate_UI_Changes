...
{
    [AuthoriseManger]
    public class EmpTimingRoasterTimingController : Controller
    {
        // GET: EmpTimingRoasterData
        public ActionResult Index()
        {
            return View("~/Views/Attendance/MainViews/EmpTimingRoasterTiming/Index.cshtml");
        }
        ...
        public ActionResult EmpTimingRoasterDatesprv(string employeeids, string paymonth)
        {
            ...
            using (DataBaseContext db = new DataBaseContext())
            {
                ...
                if (_fetch_emp_policy != null)
                {
                    ...
                    if (_emp_reporting_struct.IsTimeRoaster)
                    {

                                var employeeDataprv = db.EmployeeAttendance.Include(e => e.EmpTimingRoasterData)
                                    .Where(e => e.Employee.Id == id && e.EmpTimingRoasterData.Any(x => DbFunctions.TruncateTime(x.RoasterDate) >= firstDate &&
                                ...
                                if (employeeDataprv != null)
                                {
                                    ...
                                    for (DateTime i = firstDate; i <= lastDate; i = i.AddDays(1))
                                    {
                                        var removeR = employeeDataprv.EmpTimingRoasterData.Where(e => e.RoasterDate.Value.Date == i).Select(x => x.InTime).FirstOrDefault();
                                        if (removeR != null)
                                        {
                                            var attcode = removeR.Value.ToShortTimeString();
                                            atcode = attcode;
                                        }
        ...
        public ActionResult Create(EmpTimingRoasterData c, FormCollection form) //Create submit
        {
            ...
            try
            {
                using (DataBaseContext db = new DataBaseContext())
                {
                    ...
                    {
                        var val = db.Lookup.Include(e => e.LookupValues).Where(e => e.Code == "1001").FirstOrDefault().LookupValues.Where(e => e.Id == int.Parse(Category)).FirstOrDefault(); //db.LookupValue.Find(int.Parse(EmailActionlist));//db.LookupValue.Find(int.Parse(Category));
                            c.DayType = val;
                    }
                    ...
                    foreach (var Id in EmpId)
                    {
                        ...
                        if (_fetch_emp_policy != null)
                        {
                            ...
                            if (_emp_reporting_struct.IsTimeRoaster)
                            {
                                ...
                                var getEmpAttendance = db.EmployeeAttendance.Include(e => e.EmpTimingRoasterData).Include(e => e.Employee)
                                    .Where(e => e.Employee.Id == Id && e.EmpTimingRoasterData.Any(a => a.RoasterDate >= month)).FirstOrDefault();
                                List<EmpTimingRoasterData> oEmpTimingRoasterDataList = new List<EmpTimingRoasterData>();
                                //delete roaster
                                if (overide)
                                {
                                    var removeR = getEmpAttendance.EmpTimingRoasterData.Where(e => e.RoasterDate.Value.Month == month.Month).ToList();
                                    db.EmpTimingRoasterData.RemoveRange(removeR);
                                    db.SaveChanges();
                                }
                                foreach (RoasterClass item in data)
                                {
                                    ...
                                    //var timingpolicy = db.TimingPolicy.Where(e => e.TimingCode == item.Time + "_Global").SingleOrDefault();
                                    if (timingpolicy == null)
                                    {
                                        MSG.Add(" Please Define Timing policy for this default time..");
                                        return Json(new { status = 0, MSG = MSG }, JsonRequestBehavior.AllowGet);
                                    }

                                    if (getEmpAttendance != null && getEmpAttendance.EmpTimingRoasterData.Any(a => a.RoasterDate == Convert.ToDateTime(item.Date)))
                                    {
                                    ...
                                    EmpTimingRoasterData oEmpTimingRoasterData = new EmpTimingRoasterData
                                    {
                                        DayType = db.LookupValue.Where(e => e.LookupVal.ToLower() == item.Type.ToLower()).SingleOrDefault(),
                                        DBTrack = new DBTrack { Action = "C", CreatedBy = SessionManager.UserName, IsModified = false },
                                        RoasterDate = Convert.ToDateTime(item.Date),
                                        InTime = Convert.ToDateTime(item.Time)
                                    };
                                ...
                                {
                                    foreach (EmpTimingRoasterData oEmpTimingRoasterData in oEmpTimingRoasterDataList)
                                    {
                                        if (getEmpAttendance != null)
                                        {
                                            getEmpAttendance.EmpTimingRoasterData.Add(oEmpTimingRoasterData);
                                        }
                                        else
                                        {
                                            ...
                                            getEmpAttendance.EmpTimingRoasterData = new List<EmpTimingRoasterData> { oEmpTimingRoasterData };
                                        }
                                    }
                            ...
                            {
                                MSG.Add("You can not create Timing roaster for this employee. because this emplyee not assign Time Roaster in reporting timing structure");
                                return Json(new { status = 0, MSG = MSG }, JsonRequestBehavior.AllowGet);
                            }
                        }
                        else
                        {
                            MSG.Add(" Please Create Structure For This Employee");
                            return Json(new { status = 0, MSG = MSG }, JsonRequestBehavior.AllowGet);
                        }
                    }
