...
{
    [AuthoriseManger]
    public class OutDoorDutyReqController : Controller
    {
        // GET: EmpTimingRoasterData
        public ActionResult Index()
        {
            return View("~/Views/Attendance/MainViews/OutDoorDutyReq/Index.cshtml");
        }
        ...
        public ActionResult EmpTimingRoasterDates(string employeeids, string paymonth)
        {
            ...
            using (DataBaseContext db = new DataBaseContext())
            {
               ...
               // var employeeData = db.EmployeeAttendance.Include(e => e.Employee.GeoStruct.Location).Where(e => e.Id == id).SingleOrDefault();
                var employeeProcessData = db.EmployeeAttendance.Include(e => e.Employee.GeoStruct.Location)
                ...
                var employeeoddataprocess = db.EmployeeAttendance.Include(e => e.Employee.GeoStruct.Location)
                ...
                List<int> employeeODData = employeeoddataprocess.OutDoorDutyReq.Where(e => e.isCancel == false && e.TrReject == false && e.WFStatus.LookupVal != "2" && e.WFStatus.LookupVal != "4" && e.ProcessedData != null && e.ProcessedData.SwipeDate >= firstDate && e.ProcessedData.SwipeDate <= lastDate).Select(t => t.ProcessedData.Id).ToList();
                ...
                //   .Include(e => e.OutDoorDutyReq.Select(a => a.WFStatus))
                //  .Where(e => e.Id == id).SingleOrDefault()
                //  .OutDoorDutyReq.Where(e =>  e.EmployeeAttendance.Id == employeeProcessData.Id && e.ProcessedData.SwipeDate >= firstDate && e.ProcessedData.SwipeDate <= lastDate && e.isCancel == false && e.TrReject == false && e.WFStatus.LookupVal != "2" && e.WFStatus.LookupVal != "4").Select(t => t.ProcessedData.Id).ToList();

                //var test = db.OutDoorDutyReq.Include(e => e.ProcessedData ).Where(e => e.EmployeeAttendance.Id == employeeProcessData.Id && e.ProcessedData.SwipeDate >= firstDate && e.ProcessedData.SwipeDate <= lastDate && e.isCancel == false && e.TrReject == false && e.WFStatus.LookupVal != "2" && e.WFStatus.LookupVal != "4").Select(t => t.ProcessedData.Id).ToList();

                var empprocessr = employeeProcessData.ProcessedData.Where(e => e.SwipeDate >= firstDate && e.SwipeDate <= lastDate && !employeeODData.Contains(e.Id)).ToList();

                List<EmpProcessDataRMClass> returndataRm = new List<EmpProcessDataRMClass>();
                List<RemarkConfig> RC = db.RemarkConfig.Include(e => e.MusterRemarks).Where(e => e.IsODAppl == true).ToList();
                foreach (var item in empprocessr)
                {
                   // if (item.MusterRemarks.LookupVal.ToString() != "M" && item.MusterRemarks.LookupVal.ToString() != "PP" && item.MusterRemarks.LookupVal.ToString() != "WO" && item.MusterRemarks.LookupVal.ToString() != "HO" && item.MusterRemarks.LookupVal.ToString() != "LV")
                    foreach (var item1 in RC)
                    {
                        if (item.MusterRemarks.LookupVal.ToUpper() == item1.MusterRemarks.LookupVal.ToUpper())
                        {
                            Boolean IsODTmEntryAppl = false;
                            var RemarkConfig = db.RemarkConfig.Where(e => e.MusterRemarks_Id == item.MusterRemarks_Id).SingleOrDefault();
                            if (RemarkConfig!=null)
                            {
                                IsODTmEntryAppl = RemarkConfig.IsODTimeEntryAppl;
                            }
                            returndataRm.Add(new EmpProcessDataRMClass
                            {
                                Id = item.Id,
                                ...
                                CompInTime = item.TimingCode != null ? item.TimingCode.InTime.Value.ToShortTimeString() : "",
                                CompOutTime = item.TimingCode != null ? item.TimingCode.OutTime.Value.ToShortTimeString() : "",
                                Status = item.MusterRemarks != null ? item.MusterRemarks.LookupVal.ToString() : "",
                                IsODTimeEntryAppl = IsODTmEntryAppl.ToString()
                            });
                            break;
                        }
                ...
                {
                    var remark = AttendanceProcess.Check_HWWO(Convert.ToInt32(SessionManager.CompanyId), employeeProcessData, i);
                    dates.Add(i.ToShortDateString(), remark);
                }
                // return Json(new { dates }, JsonRequestBehavior.AllowGet);
                var result = new { Name = returndataRm };

                return Json(result, JsonRequestBehavior.AllowGet);

                // return Json(new { returndataRm }, JsonRequestBehavior.AllowGet);
            }
        }
        ...
        public class EmpProcessDataRMClass
        {
            public int Id { get; set; }
            public string SwipeDate { get; set; }
            public string InTime { get; set; }
            public string OutTime { get; set; }
            public string CompInTime { get; set; }
            public string CompOutTime { get; set; }
            public string Status { get; set; }
            public string IsODTimeEntryAppl { get; set; }
        ...
        public ActionResult Get_Employee_History(string data, string data1)
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                ...
                int EmpAttId = db.EmployeeAttendance.Include(e => e.Employee).Where(e => e.Employee.Id == EmpId).FirstOrDefault().Id;
                ...
                var db_data = db.RawData.Include(e => e.InputType)
        ...
        public ActionResult Get_EmpTimingRostData(int data)
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                try
                {
                    var db_data = db.EmployeeAttendance.Include(e => e.OutDoorDutyReq)
                        .Include(e => e.OutDoorDutyReq.Select(q => q.ProcessedData))
                       .Where(e => e.Employee.Id == data).SingleOrDefault();
                    if (db_data != null)
                    {
                        List<EmpRepoChildDataClass> returndata = new List<EmpRepoChildDataClass>();
                        foreach (var item in db_data.OutDoorDutyReq.ToList())
                        {
                            ...
                            {
                                Id = item.Id,
                                SwipeDate = item.ProcessedData != null && item.ProcessedData.SwipeDate != null ? item.ProcessedData.SwipeDate.Value.ToString("dd/MM/yyyy") : "",
                                InTime = item.InTime != null ? item.InTime.Value.ToShortTimeString() : "",
                                OutTime = item.OutTime != null ? item.OutTime.Value.ToShortTimeString() : "",
                                Reason = item.Reason != null ? item.Reason.ToString() : "",
                            });
                            // }
                        }
        ...
        public ActionResult CreateRoaster(List<RoasterClass> data, int EmpId)
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                List<string> MSG = new List<string>();
                //List<returnObjClass> returnObjList = new List<returnObjClass>();
                TimeSpan _Timein = new TimeSpan();
                TimeSpan _TimeOut = new TimeSpan();

                DateTime _Timein1 = new DateTime();
                DateTime _TimeOut1 = new DateTime();
                ...
                try
                {
                    ...
                    var swipedate = Convert.ToDateTime(data[0].Date);

                    if (data[0].InTime!=null)
                    {
                        _Timein = Utility._returnTimeSpan(Convert.ToDateTime(data[0].InTime.Value));
                        _Timein1 = Convert.ToDateTime(data[0].InTime.Value);
                    }

                    if (data[0].OutTime!=null)
                    {
                         _TimeOut = Utility._returnTimeSpan(Convert.ToDateTime(data[0].OutTime.Value));
                         _TimeOut1 = Convert.ToDateTime(data[0].OutTime.Value);
                    }

                    if (_TimeOut < _Timein)
                    {
                        MSG.Add("Out Time Should Not Less than In Time. ");
                        return Json(new { status = 0, MSG = MSG }, JsonRequestBehavior.AllowGet);
                    }
                    // if UA and isentertimeappl is false
                    if (_Timein1.TimeOfDay.ToString() == "00:00:00" && _TimeOut1.TimeOfDay.ToString() == "00:00:00")
                    {
                         _Timein1 = DateTime.Now.Date;
                         _TimeOut1 = DateTime.Now.Date;
                    }
                    ...
                    ProcessedData empprocessr = employeeProcessData.ProcessedData.Where(e => e.SwipeDate == swipedate).FirstOrDefault();
                   ...
                   .Where(e => e.Employee.Id == Id).OrderBy(e => e.Id).SingleOrDefault();
                    ...
                    if (empprocessr != null)
                    {
                        ...
                        OutDoorDutyReq OD = new OutDoorDutyReq()
                        {
                            InTime = _Timein1,
                            OutTime = _TimeOut1,
                            ReqDate = DateTime.Now.Date,
                            TimingPolicy = empprocessr.TimingCode,
                            TrClosed = true,
                            Reason = data[0].Reason.ToString(),
                            InputMethod = 0,
                            ...
                            ProcessedData = db.ProcessedData.Find(empprocessr.Id),
                        ...
                        try
                        {
                            db.OutDoorDutyReq.Add(OD);
                            db.SaveChanges();
                            List<OutDoorDutyReq> OFAT = new List<OutDoorDutyReq>();
                            OFAT.Add(OD);
                            var aa = db.EmployeeAttendance.Find(employeeProcessData.Id);
                            if (aa.OutDoorDutyReq != null)
                            {
                                OFAT.AddRange(aa.OutDoorDutyReq); 
                            }
                            aa.OutDoorDutyReq = OFAT;
