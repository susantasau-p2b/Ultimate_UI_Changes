...
{
    [AuthoriseManger]
    public class EmailFieldAssignController : Controller
    {
        ...
        {
            return View("~/Views/Core/MainViews/EmailFieldAssign/Index.cshtml");
        }

        public ActionResult Partial()
        {
            return View("~/Views/Shared/Core/_EmailField.cshtml");
        }

        public ActionResult GetEmailFieldLKDetails(List<int> SkipIds)
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                var fall = db.EmailField.Include(e => e.EmailTemplateField).ToList();
                if (SkipIds != null)
                {
                    foreach (var a in SkipIds)
                    {
                        if (fall == null)
                            fall = db.EmailField.Where(e => e.Id != a).ToList();
                        else
                            fall = fall.Where(e => !e.Id.ToString().Contains(a.ToString())).ToList();
                    }
                }

                var r = (from ca in fall select new { srno = ca.Id, lookupvalue = ca.EmailTemplateField.LookupVal.ToString() }).Distinct();
        ...
        public ActionResult Create(EmailFieldAssign c, FormCollection form) //Create submit
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                string EmailTemplateCodelist = form["EmailTemplateCodelist"] == "0" ? "" : form["EmailTemplateCodelist"];
                string EntityRefCodelist = form["EntityRefCodelist"] == "0" ? "" : form["EntityRefCodelist"];
                string EmailTemplateFieldlist = form["EmailTemplateFieldlist"] == "0" ? "" : form["EmailTemplateFieldlist"];
                ...
                try
                {
                    ...
                    if (EntityRefCodelist != null && EntityRefCodelist != "")
                    {

                        var val = db.Lookup.Include(e => e.LookupValues).Where(e => e.Code == "807").FirstOrDefault().LookupValues.Where(e => e.Id == int.Parse(EntityRefCodelist)).FirstOrDefault(); //db.LookupValue.Find(int.Parse(EntityRefCodelist));
                        c.EntityRefCode = val;
                    }

                    c.EmailTemplateField = null;
                    List<EmailField> OBJ = new List<EmailField>();

                    if (EmailTemplateFieldlist != null && EmailTemplateFieldlist != "")
                    {
                        var ids = Utility.StringIdsToListIds(EmailTemplateFieldlist);
                        foreach (var ca in ids)
                        {
                            var OBJ_val = db.EmailField.Find(ca);
                            OBJ.Add(OBJ_val);
                            c.EmailTemplateField = OBJ;
                        }
                    }
                    ...
                    if (ModelState.IsValid)
                    {
                        using (TransactionScope ts = new TransactionScope())
                        {
                            ...
                            EmailFieldAssign EmailFieldAss = new EmailFieldAssign()
                            {
                                EmailTemplateCode = c.EmailTemplateCode,
                                EntityRefCode = c.EntityRefCode,
                                EmailTemplateField = c.EmailTemplateField,
                                DBTrack = c.DBTrack
                            };

                            db.EmailFieldAssign.Add(EmailFieldAss);
        ...
        public class EditEmailFieldAssign
        {
            public Array EmailTemplateField_Id { get; set; }
            public Array EmailTemplateField_val { get; set; }
        ...
        public ActionResult Edit(int data)
        {
            ...
            using (DataBaseContext db = new DataBaseContext())
            {
                var Q = db.EmailFieldAssign
                    .Include(e => e.EmailTemplateCode)
                    .Include(e => e.EntityRefCode)
                    .Where(e => e.Id == data).Select
                    (e => new
                    {
                        EmailTemplateCode_Id = e.EmailTemplateCode.Id == null ? 0 : e.EmailTemplateCode.Id,
                        EntityRefCode_Id = e.EntityRefCode.Id == null ? 0 : e.EntityRefCode.Id,
                        Action = e.DBTrack.Action
                    }).ToList();

                List<EditEmailFieldAssign> cat = new List<EditEmailFieldAssign>();

                var add_data = db.EmailFieldAssign
                               .Include(e => e.EmailTemplateField).Include(e => e.EmailTemplateField.Select(r => r.EmailTemplateField)).Where(e => e.Id == data).ToList();

                foreach (var ca in add_data)
                {
                    cat.Add(new EditEmailFieldAssign
                    {
                        EmailTemplateField_Id = ca.EmailTemplateField.Select(e => e.Id.ToString()).ToArray(),
                        EmailTemplateField_val = ca.EmailTemplateField.Select(e => e.EmailTemplateField.LookupVal.ToString()).ToArray(),
                    });
                }
                ...
                var Corp = db.EmailFieldAssign.Find(data);
        ...
        [HttpPost]
        public async Task<ActionResult> EditSave(EmailFieldAssign c, int data, FormCollection form) // Edit submit
        {
            List<String> Msg = new List<String>();
            using (DataBaseContext db = new DataBaseContext())
            {
                try
                {
                    ...
                    if (EmailTemplateCodelist != null && EmailTemplateCodelist != "")
                    {
                        ...
                        var val = db.Lookup.Include(e => e.LookupValues).Where(e => e.Code == "806").FirstOrDefault().LookupValues.Where(e => e.Id == int.Parse(EmailTemplateCodelist)).FirstOrDefault(); //db.LookupValue.Find(int.Parse(EmailTemplateCodelist));
                        c.EmailTemplateCode = val;
                        c.EmailTemplateCode_Id = Convert.ToInt32(EmailTemplateCodelist);
                    }
                    else
                    {
                        c.EmailTemplateCode_Id = null;
                    }
                    ...
                    if (EntityRefCodelist != null && EntityRefCodelist != "")
                    {
                        ...
                        var val = db.Lookup.Include(e => e.LookupValues).Where(e => e.Code == "807").FirstOrDefault().LookupValues.Where(e => e.Id == int.Parse(EntityRefCodelist)).FirstOrDefault(); //db.LookupValue.Find(int.Parse(EntityRefCodelist));
                        c.EntityRefCode = val;
                        c.EntityRefCode_Id = Convert.ToInt32(EntityRefCodelist);
                    }
                    else
                    {
                        c.EntityRefCode_Id = null;
                    }
                  
                    var db_data = db.EmailFieldAssign.Include(e => e.EmailTemplateField).Where(e => e.Id == data).SingleOrDefault();
                    db_data.EmailTemplateField = null;
                    List<EmailField> OBJ = new List<EmailField>();
                    ...
                    if (EmailTemplateFieldlist != null && EmailTemplateFieldlist != "")
                    {
                        ...
                        foreach (var ca in ids)
                        {
                            var OBJ_val = db.EmailField.Find(ca);
                            OBJ.Add(OBJ_val);
                            db_data.EmailTemplateField = OBJ;
                        }
                    }
                    db.EmailFieldAssign.Attach(db_data);
                    ...
                    if (Auth == false)
                    {
                        ...
                        if (ModelState.IsValid)
                        {
                            ...
                            {
                                EmailFieldAssign blog = null; // to retrieve old data
                                DbPropertyValues originalBlogValues = null;

                                using (var context = new DataBaseContext())
                                {
                                    blog = context.EmailFieldAssign.Where(e => e.Id == data).Include(e => e.EmailTemplateCode).Include(e => e.EmailTemplateField)
                                        .Include(e => e.EntityRefCode).SingleOrDefault();
                                ...
                                EmailFieldAssign corp = new EmailFieldAssign()
                                    {
                                       EmailTemplateCode = c.EmailTemplateCode,
                                       EmailTemplateCode_Id=c.EmailTemplateCode_Id ,
                                       EntityRefCode=c.EntityRefCode,
                                       EntityRefCode_Id=c.EntityRefCode_Id,
                                        DBTrack = c.DBTrack,
                                        Id = data
                                    };


                                db.EmailFieldAssign.Attach(corp);
                    ...
                    else
                    {
                        using (TransactionScope ts = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                        {
                            ...
                            using (var context = new DataBaseContext())
                            {
                                blog = context.EmailFieldAssign.Where(e => e.Id == data).Include(e => e.EmailTemplateCode)
                                    .Include(e => e.EntityRefCode).SingleOrDefault();
                                originalBlogValues = context.Entry(blog).OriginalValues;
                            }
                            c.DBTrack = new DBTrack
                            ...
                            db.EmailFieldAssign.Attach(blog);
        ...
        public ActionResult P2BGrid(P2BGrid_Parameters gp)
        {
            ...
            try
            {
                ...
                IEnumerable<EmailFieldAssign> Corporate = null;
                if (gp.IsAutho == true)
                {
                    Corporate = db.EmailFieldAssign.Include(e => e.EmailTemplateCode).Include(e => e.EntityRefCode).Where(e => e.DBTrack.IsModified == true).AsNoTracking().ToList();
                }
                else
                {
                    Corporate = db.EmailFieldAssign.Include(e => e.EmailTemplateCode).Include(e => e.EntityRefCode).AsNoTracking().ToList();
                }

                IEnumerable<EmailFieldAssign> IE;
                if (!string.IsNullOrEmpty(gp.searchField) && !string.IsNullOrEmpty(gp.searchString))
                {
                    IE = Corporate;
                    if (gp.searchOper.Equals("eq"))
                    {
                        jsonData = IE.Where(e => (e.EmailTemplateCode != null ? e.EmailTemplateCode.LookupVal.ToUpper().ToString().Contains(gp.searchString.ToUpper()) : false)
                                || (e.EntityRefCode != null ? e.EntityRefCode.LookupVal.ToUpper().ToString().Contains(gp.searchString.ToUpper()) : false)
                                || (e.Id.ToString().Contains(gp.searchString))
                                ).Select(a => new Object[] { a.EmailTemplateCode != null ? a.EmailTemplateCode.LookupVal : "", a.EntityRefCode != null ? a.EntityRefCode.LookupVal : "", a.Id }).ToList();
                    ...
                    if (pageIndex > 1)
                    {
                        ...
                        jsonData = IE.Skip(pageIndex).Take(pageSize).Select(a => new Object[] { a.EmailTemplateCode != null ? Convert.ToString(a.EmailTemplateCode.LookupVal) : "", a.EmailTemplateCode != null ? Convert.ToString(a.EmailTemplateCode.LookupVal) : "", a.Id }).ToList();
                    }
                ...
                {
                    IE = Corporate;
                    Func<EmailFieldAssign, dynamic> orderfuc;
                    if (gp.sidx == "Id")
                    {
                    ...
                    if (gp.sord == "asc")
                    {
                        ...
                        jsonData = IE.Select(a => new Object[] { a.EmailTemplateCode != null ? Convert.ToString(a.EmailTemplateCode.LookupVal) : "", a.EntityRefCode != null ? Convert.ToString(a.EntityRefCode.LookupVal) : "", a.Id }).ToList();
                    }
                    ...
                    {
                        int h = pageIndex * pageSize;
                        jsonData = IE.Skip(pageIndex).Take(pageSize).Select(a => new Object[] { a.EmailTemplateCode != null ? Convert.ToString(a.EmailTemplateCode.LookupVal) : "", a.EntityRefCode != null ? Convert.ToString(a.EntityRefCode.LookupVal) : "", a.Id }).ToList();
                    }
        ...
        public async Task<ActionResult> Delete(int data)
        {
            ...
            using (DataBaseContext db = new DataBaseContext())
            {
                try
                {
                    EmailFieldAssign corporates = db.EmailFieldAssign.Include(e => e.EmailTemplateCode)

                                                       .Where(e => e.Id == data).SingleOrDefault();


                    LookupValue val = corporates.EmailTemplateCode;
