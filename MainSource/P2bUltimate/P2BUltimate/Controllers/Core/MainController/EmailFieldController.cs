...
{
     [AuthoriseManger]
    public class EmailFieldController : Controller
    {
     //   private DataBaseContext db = new DataBaseContext();
        ...
        public ActionResult Create(EmailField lkval, FormCollection form)
        {
             List<string> Msg = new List<string>();
             string EmailTemplateFieldPlist = form["EmailField_drop"] == "0" ? "" : form["EmailField_drop"];
             using (DataBaseContext db = new DataBaseContext())
             {
                 try
                 {
                     if (EmailTemplateFieldPlist != null && EmailTemplateFieldPlist != "")
                     {

                         var val = db.Lookup.Include(e => e.LookupValues).Where(e => e.Code == "808").FirstOrDefault().LookupValues.Where(e => e.Id == int.Parse(EmailTemplateFieldPlist)).FirstOrDefault(); //db.LookupValue.Find(int.Parse(EmailTemplateFieldPlist));
                        lkval.EmailTemplateField = val;
                     }

                     lkval.DBTrack = new DBTrack { Action = "C", CreatedBy = SessionManager.UserName, CreatedOn = DateTime.Now };
                     EmailField EmailField = new EmailField
                     {
                        EmailTemplateField = lkval.EmailTemplateField,
                     ...
                     try
                     {
                         if (ModelState.IsValid)
                         {
                             using (TransactionScope ts = new TransactionScope())
                             {
                                 ...
                                 ts.Complete();
                                 Msg.Add("  Record Saved Successfully");
                                 return Json(new Utility.JsonReturnClass { Id = EmailField.Id, Val = EmailField.EmailTemplateField.LookupVal.ToString(), success = true, responseText = Msg }, JsonRequestBehavior.AllowGet);
                    ...
                    if (ModelState.IsValid)
                    {
                        using (TransactionScope ts = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                        {
                            ...
                            if (DBTrackFile.ByteArrayCompare((byte[])TempData["CurrRowVersion"], (byte[])TempData["RowVersion"]) == true)
                            {
                                ...
                                return Json(new Utility.JsonReturnClass { Id = lkval.Id, Val = lkval.LookupVal, success = true, responseText = Msg }, JsonRequestBehavior.AllowGet);
