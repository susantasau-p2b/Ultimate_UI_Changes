...
namespace P2BUltimate.Controllers.Core.MainController
{
    ...
    public class EmployeeFamilyDetailsController : Controller
    {
        ...
        {
            return View("~/Views/Core/MainViews/EmployeeFamilyDetails/Index.cshtml");
        }
        ...
        public ActionResult P2BGrid(P2BGrid_Parameters gp)
        {
            try
            {
                ...
                Employee = db.Employee.Include(q => q.FamilyDetails).Include(q => q.EmpName).ToList();
                if (gp.IsAutho == true)
                {
                    Employee = db.Employee.Include(q => q.FamilyDetails).Include(q => q.EmpName).Where(e => e.DBTrack.IsModified == true).ToList();
                }
                else
                {
                    // Employee = db.Employee.Where(a=>a.Id==db.EmpAcademicInfo.Contains()).ToList();
                    Employee = db.Employee.Include(q => q.FamilyDetails).Where(q => q.FamilyDetails.Count() > 0).ToList();
                }
                ...
                if (!string.IsNullOrEmpty(gp.searchField))
                {
                    ...
                    if (gp.searchOper.Equals("eq"))
                    {
                        jsonData = IE.Where(e => (e.Id.ToString().Contains(gp.searchString.ToUpper()))
                                                      || (e.EmpCode.ToString().Contains(gp.searchString.ToUpper())) || (e.EmpName.FullNameFML.ToString().Contains(gp.searchString.ToUpper())))
                                                  .Select(a => new Object[] { a.Id, a.EmpCode, a.EmpName.FullNameFML }).ToList();
                    }
                    if (pageIndex > 1)
                    {
                        int h = pageIndex * pageSize;
                        jsonData = IE.Skip(pageIndex).Take(pageSize).Select(a => new Object[] { a.Id, a.EmpCode, a.EmpName.FullNameFML }).ToList();
                    }
                    totalRecords = IE.Count();
                }
                else
                {
                    ...
                    {
                        IE = IE.OrderBy(orderfuc);
                        jsonData = IE.Select(a => new Object[] { a.Id, a.EmpCode, a.EmpName.FullNameFML }).ToList();
                    }
                    else if (gp.sord == "desc")
                    {
                        IE = IE.OrderByDescending(orderfuc);
        ...
        public ActionResult Edit(int data)
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                try
                {
                    ...
                    var EmpSocialInfo = db.Employee
                      .Include(e => e.FamilyDetails)
                       .Include(e => e.FamilyDetails.Select(x => x.ContactDetails))
                       .Include(e => e.FamilyDetails.Select(x => x.Address))
                       .Include(e => e.FamilyDetails.Select(x => x.Relation))
                       .Include(e => e.FamilyDetails.Select(x => x.MemberName))
                        //.Include(e => e.FamilyDetails.Select(lm => lm.BenefitList))
                        //.Include(e => e.FamilyDetails.Select(lm => lm.BenefitList.Select(f => f.BenefitType)))
                      .Where(e => e.Id == data).FirstOrDefault();


                    //var EmpSocialInfo1 = db.FamilyDetails
                    //  .Include(e => e.ContactDetails)
                    //  .Include(e => e.Address)
                    //  .Include(e => e.Relation)
                    ...
                    //  //.Include(e => e.FamilyDetails.Select(lm => lm.BenefitList.Select(f => f.BenefitType)))
                    //  .Where(e => e.Id == data).SingleOrDefault();
                    //  var AID = EmpSocialInfo1.Id;
                    // var datae=EmpSocialInfo.Select(q=>q.)
                    string DateofBirth = "";
                    int Relations_Id = 0;
                    string Action = "";
                    foreach (var item in EmpSocialInfo.FamilyDetails)
                    {
                        DateofBirth = item.DateofBirth.Value.ToShortDateString();
                        Relations_Id = item.Relation == null ? 0 : item.Relation.Id;
                        Action = item.DBTrack.Action;
                    }
                    var r = (from ca in EmpSocialInfo.FamilyDetails
                             select new
                             {
                    ...
                    var add_data = db.Employee
                        .Include(e => e.FamilyDetails)
                         .Include(e => e.FamilyDetails.Select(x => x.ContactDetails))
                         .Include(e => e.FamilyDetails.Select(x => x.Address))
                         .Include(e => e.FamilyDetails.Select(x => x.Address.Area))
                         .Include(e => e.FamilyDetails.Select(x => x.Address.City))
                         .Include(e => e.FamilyDetails.Select(x => x.Address.Country))
                         .Include(e => e.FamilyDetails.Select(x => x.Address.District))
                         .Include(e => e.FamilyDetails.Select(x => x.Address.State))
                         .Include(e => e.FamilyDetails.Select(x => x.Address.StateRegion))
                         .Include(e => e.FamilyDetails.Select(x => x.Address.Taluka))
                         .Include(e => e.FamilyDetails.Select(x => x.Relation))
                         .Include(e => e.FamilyDetails.Select(x => x.MemberName))
                    ...
                    foreach (var item in add_data.FamilyDetails)
                    {
                        benifitnominee1.Add(
                     new benefitlists1
                     {
                         Add_Id = item.Address == null ? 0 : item.Address.Id,
                         Address_FullAddress = item.Address == null ? "" : item.Address.FullAddress,

                         Cont_Id = item.ContactDetails == null ? 0 : item.ContactDetails.Id,
                         FullContactDetails = item.ContactDetails == null ? "" : item.ContactDetails.FullContactDetails,

                         name_Id = item.MemberName == null ? 0 : item.MemberName.Id,
                         name_FullDetails = item.MemberName == null ? "" : item.MemberName.FullNameFML,
                    ...
                    //{
                    //    var LKup = db.FamilyDetails.Find(item);
                    //    TempData["RowVersion"] = LKup.RowVersion;
                    //    Auth = LKup.DBTrack.IsModified;

                    //}
                    return this.Json(new Object[] { r, benifitnominee1, benifitnominee, "", "", JsonRequestBehavior.AllowGet });
                }
                catch (Exception ex)
                {
                    LogFile Logfile = new LogFile();
        ...
        [HttpPost]
        public async Task<ActionResult> EditSave(FamilyDetails c, int data, FormCollection form) // Edit submit
        {
            List<string> Msg = new List<string>();
            using (DataBaseContext db = new DataBaseContext())
            {
                try
                {
                    ...
                    var db_Data = db.FamilyDetails
                                  .Include(e => e.ContactDetails)
                  .Include(e => e.Address)
                  .Include(e => e.Relation)
                  .Include(e => e.MemberName)
                        //.Include(e => e.CanFamilyDetails.Select(lm => lm.BenefitList))
                        //.Include(e => e.CanFamilyDetails.Select(lm => lm.BenefitList.Select(f => f.BenefitType)))
                                 .Where(e => e.Id == data).SingleOrDefault();
                    int AID = db_Data.Id;

                    FamilyDetails corporates = db.FamilyDetails
                        //.Include(e => e.ContactDetails)
                   .Include(e => e.Address)
                   .Include(e => e.Relation)
                   .Include(e => e.MemberName)
                   ...
                   .Include(e => e.ContactDetails).Where(e => e.Id == AID).SingleOrDefault();
                    ...
                    var nomname = corporates.MemberName;
                        ...
                        if (ModelState.IsValid)
                        {
                            try
                            {
                                ...
                                using (TransactionScope ts = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                                {
                                    ...
                                    using (var context = new DataBaseContext())
                                    {
                                        db.FamilyDetails.Attach(corporates);
                                        ...
                                        var Curr_OBJ = db.FamilyDetails.Find(AID);
                                        ...
                                        if (DBTrackFile.ByteArrayCompare((byte[])TempData["CurrRowVersion"], (byte[])TempData["RowVersion"]) == true)
                                        {
                                            ...
                                            blog = context.FamilyDetails.Where(e => e.Id == AID).Include(e => e.Relation)
                                                                    .Include(e => e.Address).Include(e => e.MemberName)
                    ...
                    else
                    {
                        using (TransactionScope ts = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                        {
                            ...
                            using (var context = new DataBaseContext())
                            {
                                blog = context.FamilyDetails.Where(e => e.Id == AID).SingleOrDefault();
                                originalBlogValues = context.Entry(blog).OriginalValues;
                            }
                            c.DBTrack = new DBTrack
                            {
                            ...
                            using (var context = new DataBaseContext())
                            {
                                ...
                                // Old_Corp = context.FamilyDetails.Where(e => e.Id == AID).Include(e => e.Relation)
                  ...
                  .Include(e => e.Nominees.Select(lm => lm.BenefitList.Select(f => f.BenefitType)))
                    ...
                    else
                    {
                        using (TransactionScope ts = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                        {
                            ...
                            var selectedValues = EmpSocialInfo1.Nominees.Select(e => e.BenefitList);
                            ...
                            db.Entry(EmpSocialInfo1.Nominees).State = System.Data.Entity.EntityState.Deleted;
            ...
            using (DataBaseContext db = new DataBaseContext())
            {
                try
                {
                    var db_data = db.Employee
                        .Include(e => e.FamilyDetails)
                        .Include(e => e.FamilyDetails.Select(t => t.MemberName))
                        .Include(e => e.FamilyDetails.Select(t => t.Relation))
                        .Where(e => e.Id == data).AsNoTracking().AsParallel().SingleOrDefault();
                    if (db_data != null)
                    {
                        List<ChildDataClass> returndata = new List<ChildDataClass>();
                        foreach (var c in db_data.FamilyDetails)
                        {
                            returndata.Add(new ChildDataClass
                            {
                                Id = c.Id,
                                MemberName = c.MemberName.FullNameFML.ToString(),
                                Relation = c.Relation.LookupVal.ToString(),
                                DateOfBirth =c.DateofBirth==null?"": c.DateofBirth.Value.ToShortDateString(),
                                IsHandicap = c.IsHandicap,
                                HandicapRemark = c.HandicapRemark == null ? "" : c.HandicapRemark
                            });
                        }
        ...
        public ActionResult GridEditData(int data)
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                var Q = db.FamilyDetails.Include(e => e.MemberName).Include(e => e.Relation)
                 .Where(e => e.Id == data).AsEnumerable().Select
                 (c => new
                 {
                     DateBirth =c.DateofBirth==null?"": c.DateofBirth.Value.ToShortDateString(),
                     Relation = c.Relation == null ? 0 : c.Relation.Id,
                     MemberName = c.MemberName == null ? 0 : c.MemberName.Id,
                     IsHandicap = c.IsHandicap,
                     HandicapRemark = c.HandicapRemark == null ? "" : c.HandicapRemark,
                     name_FullDetails = c.MemberName == null ? "" : c.MemberName.FullNameFML,
                 }).SingleOrDefault();
        ...
        public ActionResult GridDelete(int data)
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                var LvEP = db.FamilyDetails.Find(data);
                db.FamilyDetails.Remove(LvEP);
                db.SaveChanges();
