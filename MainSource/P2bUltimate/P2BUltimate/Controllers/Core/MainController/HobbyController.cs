...
namespace P2BUltimate.Controllers.Core.MainController
{
    public class HobbyController : Controller
    {
        ...
        {
            return View("~/Views/Core/MainViews/Hobby/Index.cshtml");
        }

        public ActionResult partial()
        {
            return View("~/Views/Shared/Core/_Hobby.cshtml");
        }
        ...
        public ActionResult P2BGrid(P2BGrid_Parameters gp)
        {
            try
            {
                ...
                IEnumerable<Hobby> hobby = null;
                if (gp.IsAutho == true)
                {
                    hobby = db.Hobby.Where(e => e.DBTrack.IsModified == true).ToList();
                }
                else
                {
                    hobby = db.Hobby.ToList();
                }

                IEnumerable<Hobby> IE;
                if (!string.IsNullOrEmpty(gp.searchField))
                {
                    IE = hobby;
                    if (gp.searchOper.Equals("eq"))
                    {
                        jsonData = IE.Select(a => new { a.Id, a.HobbyName }).Where((e => (e.Id.ToString() == gp.searchString) || (e.HobbyName.ToLower() == gp.searchString.ToLower()))).ToList();
                        //jsonData = IE.Select(a => new Object[] { a.Id, Convert.ToString(a.Code), Convert.ToString(a.Name), Convert.ToString(a.BusinessType) != null ? Convert.ToString(a.BusinessType.Id) : "" }).Where(a => a.Contains(Convert.ToInt32(gp.searchString))).ToList();
                    }
                    if (pageIndex > 1)
                    {
                        int h = pageIndex * pageSize;
                        jsonData = IE.Skip(pageIndex).Take(pageSize).Select(a => new Object[] { a.Id, a.HobbyName }).ToList();
                    }
                ...
                {
                    IE = hobby;
                    Func<Hobby, int> orderfuc = (c =>
                                                               gp.sidx == "Id" ? c.Id : 0);
                    if (gp.sord == "asc")
                    {
                        IE = IE.OrderBy(orderfuc);
                        jsonData = IE.Select(a => new Object[] { a.Id, Convert.ToString(a.HobbyName) }).ToList();
                    }
        ...
        public ActionResult Create(Hobby lkval, FormCollection form) //Create submit
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                lkval.DBTrack = new DBTrack { Action = "C", CreatedBy = SessionManager.UserName, IsModified = false };
                List<string> Msg = new List<string>();
                Hobby LookupValue = new Hobby
                {
                ...
                try
                {
                    if (ModelState.IsValid)
                    {
                        using (TransactionScope ts = new TransactionScope())
                        {
                            ...
                            return Json(new Utility.JsonReturnClass { Id = LookupValue.Id, Val = LookupValue.HobbyName, success = true, responseText = Msg }, JsonRequestBehavior.AllowGet);
        ...
        public ActionResult Edit(int data)
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                var Q = db.Hobby.Where(e => e.Id == data).Select
                 (e => new
                 {
                ...
                var add_data = db.Hobby.Where(e => e.Id == data)
                   .ToList();
                ...
                var W = db.DT_Hobby
                     ...
                     {
                         DT_Id = e.Id,
                         HobbyName = e.HobbyName == null ? "" : e.HobbyName,
                     }).OrderByDescending(e => e.DT_Id).FirstOrDefault();

                var Corp = db.Hobby.Find(data);
        ...
        public int EditS(int data, Hobby c, DBTrack dbT)
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                var CurCorp = db.Hobby.Find(data);
                TempData["CurrRowVersion"] = CurCorp.RowVersion;
                db.Entry(CurCorp).State = System.Data.Entity.EntityState.Detached;
                if (DBTrackFile.ByteArrayCompare((byte[])TempData["CurrRowVersion"], (byte[])TempData["RowVersion"]) == true)
                {
                    c.DBTrack = dbT;
                    Hobby corp = new Hobby()
                    {
        ...
        [HttpPost]
        public async Task<ActionResult> EditSave1(Hobby c, int data, FormCollection form) // Edit submit
        {
            ...
            using (DataBaseContext db = new DataBaseContext())
            {
                        ...
                        try
                        {
                            ...
                            using (TransactionScope ts = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                            {
                                ...
                                using (var context = new DataBaseContext())
                                {
                                    ...
                                    var Obj = DBTrackFile.ModifiedDataHistory("Core/P2b.Global", "M", blog, c, "Hobby", c.DBTrack);
                                    DT_Hobby DT_Corp = (DT_Hobby)Obj;
                                    db.DT_Hobby.Add(DT_Corp);
                                    db.SaveChanges();
                                }
                                ...
                                //  return Json(new Object[] { c.Id, c.HobbyName, "Record Updated", JsonRequestBehavior.AllowGet });
                                Msg.Add("  Record Updated");
                                return Json(new Utility.JsonReturnClass { Id = c.Id, Val = c.HobbyName, success = true, responseText = Msg }, JsonRequestBehavior.AllowGet);
                ...
                else
                {
                    using (TransactionScope ts = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                    {
                        Hobby Old_Corp = db.Hobby.Where(e => e.Id == data).SingleOrDefault();
                        ...
                        using (var context = new DataBaseContext())
                        {
                            ////DBTrackFile.DBTrackSave("Core/P2b.Global", "M", Old_Corp, Curr_Corp, "Hobby", c.DBTrack);
                        }

                        ts.Complete();
                        // return Json(new Object[] { Old_Corp.Id, c.HobbyName, "Record Updated", JsonRequestBehavior.AllowGet });
                        Msg.Add("  Record Updated");
                        return Json(new Utility.JsonReturnClass { Id = Old_Corp.Id, Val = c.HobbyName, success = true, responseText = Msg }, JsonRequestBehavior.AllowGet);
        ...
        [HttpPost]
        public async Task<ActionResult> EditSave(Hobby c, int data, FormCollection form) // Edit submit
        {
            List<string> Msg = new List<string>();
            using (DataBaseContext db = new DataBaseContext())
            {
                try
                {
                        ...
                        if (ModelState.IsValid)
                        {
                            try
                            {
                                ...
                                using (TransactionScope ts = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                                {
                                    ...
                                    var Curr_Lookup = db.Hobby.Find(data);
                                    ...
                                    if (DBTrackFile.ByteArrayCompare((byte[])TempData["CurrRowVersion"], (byte[])TempData["RowVersion"]) == true)
                                    {
                                        ...
                                        Msg.Add("  Record Updated");
                                        return Json(new Utility.JsonReturnClass { Id = lk.Id, Val = lk.HobbyName, success = true, responseText = Msg }, JsonRequestBehavior.AllowGet);
                    ...
                    else
                    {
                        using (TransactionScope ts = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                        {
                            ...
                            Hobby qualificationDetails = new Hobby()
                            {
                            ...
                            using (var context = new DataBaseContext())
                            {
                                ...
                                Old_Corp = context.Hobby.Where(e => e.Id == data)
                                .SingleOrDefault();
                                DT_Hobby DT_Corp = (DT_Hobby)obj;
                            ...
                            Msg.Add("  Record Updated");
                            return Json(new Utility.JsonReturnClass { Id = blog.Id, Val = c.HobbyName, success = true, responseText = Msg }, JsonRequestBehavior.AllowGet);
            ...
            using (DataBaseContext db = new DataBaseContext())
            {
                if (auth_action == "C")
                {
                    using (TransactionScope ts = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                    {
                        ...
                        Hobby corp = db.Hobby.FirstOrDefault(e => e.Id == auth_id);
                        ...
                        using (var context = new DataBaseContext())
                        {
                            ...
                            ////DBTrackFile.DBTrackSave("Core/P2b.Global", "M", corp, null, "Hobby", corp.DBTrack);
                        }

                        ts.Complete();
                        return Json(new Object[] { corp.Id, corp.HobbyName, "Record Authorised", JsonRequestBehavior.AllowGet });
                    }
                }
                else if (auth_action == "M")
                {

                    Hobby Old_Corp = db.Hobby.Where(e => e.Id == auth_id).SingleOrDefault();


                    DT_Hobby Curr_Corp = db.DT_Hobby
                                                .Where(e => e.Orig_Id == auth_id && e.DBTrack.Action == "M" && e.DBTrack.IsModified == true)
                                                .OrderByDescending(e => e.Id)
                                                .FirstOrDefault();

                    Hobby corp_hobby = new Hobby();


                    corp_hobby.HobbyName = Curr_Corp.HobbyName == null ? Old_Corp.HobbyName : Curr_Corp.HobbyName;
                    ...
                    if (ModelState.IsValid)
                    {
                        try
                        {
                            ...
                            using (TransactionScope ts = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                            {
                                ...
                                ts.Complete();
                                return Json(new Object[] { corp_hobby.Id, corp_hobby.HobbyName, "Record Updated", JsonRequestBehavior.AllowGet });
                            }
                        }
                        catch (DbUpdateConcurrencyException ex)
                        {
                ...
                else if (auth_action == "D")
                {
                    using (TransactionScope ts = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                    {
                        ...
                        Hobby corp = db.Hobby.AsNoTracking().FirstOrDefault(e => e.Id == auth_id);
                        ...
                        using (var context = new DataBaseContext())
                        {
                            ...
                            //DBTrackFile.DBTrackSave("Core/P2b.Global", "D", corp, null, "Hobby", corp.DBTrack);
                        }
        ...
        //            using (var context = new DataBaseContext())
        //            {
        //                //DBTrackFile.DBTrackSave("Core/P2b.Global", "D", hobby, null, "Hobby", hobby.DBTrack);
        //            }
        //            ts.Complete();
        ...
        //                    ////DBTrackFile.DBTrackSave("Core/P2b.Global", "D", hobby, null, "Hobby", dbT);
        //                }
        //                ts.Complete();
        //                return Json(new Object[] { "", "Data removed.", JsonRequestBehavior.AllowGet });
        ...
        public async Task<ActionResult> Delete(int data)
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                ...
                Hobby Aw = db.Hobby.Where(e => e.Id == data).SingleOrDefault();
