...
{
     [AuthoriseManger]
	public class LocationObjController : Controller
	{
		//
		// GET: /LocationObj/
		...
		{
			return View("~/Views/Shared/Core/_LocationObj.cshtml");
		}
		...
		public ActionResult PopulateDropDownList(string data, string data2)
		{
            ...
            {
                var qurey = db.LocationObj.ToList();
                int selected = 0;
                ...
                foreach (var item in qurey)
                {
                    drop.Add(new SelectListItem
                    {
                        ...
                        Text = "LocCode :" + item.LocCode + " LocDesc :" + item.LocDesc,
        ...
        public ActionResult Create(LocationObj c, FormCollection form)
        {
            ...
            using (DataBaseContext db = new DataBaseContext())
            {
                try
                {
                    if (ModelState.IsValid)
                    {
                        ...
                        {
                            if (db.LocationObj.Any(o => o.LocCode == c.LocCode))
                            {
                            ...
                            {
                                LocCode = c.LocCode == null ? "" : c.LocCode.Trim(),
                                LocDesc = c.LocDesc == null ? "" : c.LocDesc.Trim(),
                                DBTrack = c.DBTrack
                            };
                            try
                            {
                                ...
                                DT_LocationObj DT_DeptObj = (DT_LocationObj)rtn_Obj;
                                db.Create(DT_DeptObj);
                                db.SaveChanges();
                                ts.Complete();
                                Msg.Add("  Data Saved successfully  ");
                                return Json(new Utility.JsonReturnClass { Id = LocationObj.Id, success = true, responseText = Msg }, JsonRequestBehavior.AllowGet);
