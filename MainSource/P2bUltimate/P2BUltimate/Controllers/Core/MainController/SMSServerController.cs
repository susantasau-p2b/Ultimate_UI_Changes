...
namespace P2BUltimate.Controllers.Core.MainController
{
    ...
    public class SMSServerController : Controller
    {
        ...
        {
            return View("~/Views/Core/MainViews/SMSServer/Index.cshtml");
        }

        [HttpPost]
        // [ValidateAntiForgeryToken]
        public ActionResult Create(SMSServer c, FormCollection form) //Create submit
        {
                ...
                try
                {
                    ...
                    if (ModelState.IsValid)
                    {
                        using (TransactionScope ts = new TransactionScope())
                        {
                            ...
                            SMSServer corporate = new SMSServer()
                            {
                                UserId = c.UserId,
        ...
        public ActionResult Edit(int data)
        {
            ...
            using (DataBaseContext db = new DataBaseContext())
            {
                ...
                var Corp = db.SMSServer.Find(data);
        ...
        [HttpPost]
        public async Task<ActionResult> EditSave(SMSServer c, int data, FormCollection form) // Edit submit
        {
            List<String> Msg = new List<String>();
            using (DataBaseContext db = new DataBaseContext())
            {
                try
                {
                    ...
                    if (Auth == false)
                    {
                        ...
                        if (ModelState.IsValid)
                        {
                            ...
                            {
                                SMSServer blog = null; // to retrieve old data
                                DbPropertyValues originalBlogValues = null;

                                using (var context = new DataBaseContext())
                                {
                                    blog = context.SMSServer.Where(e => e.Id == data)
                                                            .SingleOrDefault();
                    ...
                    else
                    {
                        using (TransactionScope ts = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                        {
                            ...
                            using (var context = new DataBaseContext())
                            {
                                blog = context.SMSServer.Where(e => e.Id == data).SingleOrDefault();
                                originalBlogValues = context.Entry(blog).OriginalValues;
                            }
                            c.DBTrack = new DBTrack
                            {
        ...
        public ActionResult P2BGrid(P2BGrid_Parameters gp)
        {
            ...
            try
            {
                ...
                IEnumerable<SMSServer> Corporate = null;
                if (gp.IsAutho == true)
                {
                    Corporate = db.SMSServer.Where(e => e.DBTrack.IsModified == true).AsNoTracking().ToList();
                }
                else
                {
                    Corporate = db.SMSServer.AsNoTracking().ToList();
                }

                IEnumerable<SMSServer> IE;
                if (!string.IsNullOrEmpty(gp.searchField) && !string.IsNullOrEmpty(gp.searchString))
                {
                    ...
                    if (gp.searchOper.Equals("eq"))
                    {
                        ...
                        jsonData = IE.Where(e => (e.UserId.ToString().Contains(gp.searchString))
                ...
                {
                    IE = Corporate;
                    Func<SMSServer, dynamic> orderfuc;
                    if (gp.sidx == "Id")
                    {
        ...
        public async Task<ActionResult> Delete(int data)
        {
            ...
            using (DataBaseContext db = new DataBaseContext())
            {
                try
                {
                    SMSServer corporates = db.SMSServer
