...
{
    [AuthoriseManger]
    public class LvCalendarController : Controller
    {
        //private DataBaseContext db = new DataBaseContext();

        public ActionResult Index()
        {
            return View("~/Views/Leave/MainViews/LvCalendar/Index.cshtml");
        }
                    ...
                    if (ModelState.IsValid)
                    {
                        ...
                        try
                        {
                            using (TransactionScope ts = new TransactionScope())
                            {
                                ...
                                var alrdy = db.Calendar.Where(e => (e.Name.LookupVal.ToUpper() == "LEAVECALENDAR")
                                    && (e.Default == true && calendar.Default == true)).Count();
                                // && calendar.FromDate.Value.Date<=e.FromDate.Value.Date && calendar.ToDate.Value.Date<=e.ToDate.Value.Date).Count();

                                if (alrdy > 0)
                                {
                                    Msg.Add("   Default Leave Year already exist.  ");
                                    return Json(new Utility.JsonReturnClass { success = false, responseText = Msg }, JsonRequestBehavior.AllowGet);
                                    // return this.Json(new Object[] { "", "", "From Date already exists.", JsonRequestBehavior.AllowGet });
                                }

                                //var alrD = db.Calendar.Where(e=>e.FromDate <= calendar.FromDate && e.ToDate >= calendar.ToDate).Count();
                                //if (alrD > 0)
                                //{
                                //    Msg.Add("  Leave Year With thids Period already exist.  ");
                                //    return Json(new Utility.JsonReturnClass { success = false, responseText = Msg }, JsonRequestBehavior.AllowGet);
                                //    // return this.Json(new Object[] { "", "", "From Date already exists.", JsonRequestBehavior.AllowGet });
                                //}
                                ...
                                if (alrDq == true)
                                {
                                    Msg.Add("  Leave Year With this Period already exist.  ");
                                    return Json(new Utility.JsonReturnClass { success = false, responseText = Msg }, JsonRequestBehavior.AllowGet);
                                    // return this.Json(new Object[] { "", "", "From Date already exists.", JsonRequestBehavior.AllowGet });
                                }

                                //var alrD = db.Calendar.Where(e => e.Name.LookupVal.ToUpper() == calendar.Name.LookupVal.ToUpper())
                                //    .Any(q => q.FromDate <= c.FromDate && q.ToDate <= c.ToDate);
                                //if (alrD == true)
                                //{
                                //    Msg.Add("  Leave Year With this Period already exist.  ");
                                //    return Json(new Utility.JsonReturnClass { success = false, responseText = Msg }, JsonRequestBehavior.AllowGet);
        ...
        public ActionResult P2BGrid(P2BGrid_Parameters gp)
        {
            try
            {
                ...
                var Calendar = db.Calendar.Include(e => e.Name).Where(a => a.Name.LookupVal.ToString() == "LeaveCalendar").ToList();
                    ...
                    if (gp.searchOper.Equals("eq"))
                    {
                        ...
                        jsonData = IE
                            .Where((e => (e.Id.ToString() == gp.searchString)
                                || (e.Name.LookupVal.ToUpper().ToString().Contains(gp.searchString.ToUpper().ToString()))
                                || (e.FromDate.Value.ToShortDateString().Contains(gp.searchString.ToString()))
                                || (e.ToDate.Value.ToShortDateString().Contains(gp.searchString.ToString()))
                                || (e.Default.ToString().ToUpper() == gp.searchString.ToString()))).ToList()
                        .Select(a => new Object[] { a.Name.LookupVal.ToString(), a.FromDate.Value.ToShortDateString(), a.ToDate.Value.ToShortDateString(), a.Default.ToString(), a.Id });
