...
{
    [AuthoriseManger]
    public class ArrJVNonStandardEmpController : Controller
    {
        ...
        {
            return View("~/Views/Shared/Payroll/_ArrJVNonStandardEmp.cshtml");
        }

        public ActionResult JVNonStandardEmpCreate(ArrJVNonStandardEmp jv, FormCollection form)
        {
            ...
            ArrJVNonStandardEmp ArrJVNonStandardEmp = new ArrJVNonStandardEmp();
            ArrJVNonStandardEmp.ArrProductCode = jv.ArrProductCode;
            ArrJVNonStandardEmp.ArrAccountNo = jv.ArrAccountNo;
            ArrJVNonStandardEmp.ArrSubAccountNo = jv.ArrSubAccountNo;
            ArrJVNonStandardEmp.DBTrack = new DBTrack { Action = "C", CreatedBy = SessionManager.UserName, IsModified = false };
                ...
                using (TransactionScope ts = new TransactionScope())
                {
                    using (DataBaseContext db = new DataBaseContext())
                    {
                        if (EmpLocIn_table != null)
                        {
                            ...
                            ArrJVNonStandardEmp.Branch = loc;
                        }
                        foreach (var i in Emplist)
                        {
                            ...
                            if (AccountType_drop != 0)
                            {
                                ...
                                ArrJVNonStandardEmp.AccountType = value;
                            }
                            //if (db.ArrJVNonStandardEmp.Include(q => q.EmployeePayroll).Any(a => a.EmployeePayroll.Id == i && a.ProductCode == jv.ProductCode))
                            //{
                            ...
                            ArrJVNonStandardEmp.EmployeePayroll = tempemppayroll;
                            db.ArrJVNonStandardEmp.Add(ArrJVNonStandardEmp);
                            db.SaveChanges();
                            //DBTrackFile.DBTrackSave("Core/P2b.Global", "C", corporate, null, "Corporate", null);

                            if (emppass == "")
                            {
                                emppass = ArrJVNonStandardEmp.Id.ToString();
                            }
                            else
                            {
                                emppass = emppass + "," + ArrJVNonStandardEmp.Id.ToString();
                            }
                        }
