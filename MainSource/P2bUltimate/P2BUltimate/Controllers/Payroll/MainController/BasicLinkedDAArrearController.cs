...
{
    public class BasicLinkedDAArrearController : Controller
    {
        //   private DataBaseContext db = new DataBaseContext();

        public ActionResult Index()
        {
            return View("~/Views/Payroll/MainViews/BasicLinkedDAArrear/Index.cshtml");
        }
        ...
        public async Task<ActionResult> EditSave(BasicLinkedDA ESOBJ, int data, FormCollection form) // Edit submit
        {
            ...
            try
            {
                ...
                using (DataBaseContext db = new DataBaseContext())
                {
                    var chkDataExist = db.BasicLinkedDA.Where(e => e.EffectiveFrom == ESOBJ.EffectiveFrom && e.EffectiveTo == ESOBJ.EffectiveTo).SingleOrDefault();
                    if (chkDataExist == null)
                    {
                        Msg.Add("For Entered Effective Period, No Data Exists !!!");
                        return Json(new Utility.JsonReturnClass { success = false, responseText = Msg }, JsonRequestBehavior.AllowGet);
                    }

                    if (ESOBJ.EffectiveTo.Value < ESOBJ.EffectiveFrom.Value)
                    {
                        Msg.Add("Effective To Date must be greater than Effective From Date");
                        return Json(new Utility.JsonReturnClass { success = false, responseText = Msg }, JsonRequestBehavior.AllowGet);
                    }

                    if (ESOBJ.ToDate.Value < ESOBJ.FromDate.Value)
                    {
                        Msg.Add(" To Date must be greater than From Date");
                        return Json(new Utility.JsonReturnClass { success = false, responseText = Msg }, JsonRequestBehavior.AllowGet);
                    }
                    if (ESOBJ.EffectiveTo.Value.ToString("MM/YYYY") != ESOBJ.EffectiveFrom.Value.ToString("MM/YYYY"))
                    {
                        Msg.Add("Only One Effective Month you can Define");
                        return Json(new Utility.JsonReturnClass { success = false, responseText = Msg }, JsonRequestBehavior.AllowGet);
                    }
                    if (ESOBJ.ToDate.Value.ToString("MM/YYYY") != ESOBJ.FromDate.Value.ToString("MM/YYYY"))
                    {
                        Msg.Add(" From Month and to Month Will same Define");
                        return Json(new Utility.JsonReturnClass { success = false, responseText = Msg }, JsonRequestBehavior.AllowGet);
                    }
                    ...
                    double NewDApoint = ESOBJ.DAPoint;
                    //var db_data1 = db.BasicLinkedDA.Where(e => e.Id == data).SingleOrDefault();
                    var db_data1 = db.BasicLinkedDA.Where(e => e.EffectiveFrom == ESOBJ.EffectiveFrom && e.EffectiveTo == ESOBJ.EffectiveTo).SingleOrDefault();
                            ...
                            try
                            {
                                using (TransactionScope ts = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                                {
                                    ...
                                    using (var context = new DataBaseContext())
                                    {
                                        ...
                                        //                       .SingleOrDefault();
                                        blog = context.BasicLinkedDA.Where(e => e.EffectiveFrom == ESOBJ.EffectiveFrom && e.EffectiveTo == ESOBJ.EffectiveTo).SingleOrDefault();
                                   ...
                                   // int a = EditS(data, ESOBJ, ESOBJ.DBTrack);
                                    ...
                                    var m1 = db.BasicLinkedDA.Where(e => e.EffectiveFrom == ESOBJ.EffectiveFrom && e.EffectiveTo == ESOBJ.EffectiveTo).ToList();
                                    foreach (var s in m1)
                                    {
                                    ...
                                    var CurCorp = db.BasicLinkedDA.Where(e => e.EffectiveFrom == ESOBJ.EffectiveFrom && e.EffectiveTo == ESOBJ.EffectiveTo).SingleOrDefault();
        ...
        public ActionResult P2BGrid(P2BGrid_Parameters gp)
        {
            try
            {
                ...
                {
                    BasicLinkedDA = db.BasicLinkedDA.Where(e => e.OldDAPoint != 0).AsNoTracking().ToList();
                }
                ...
                if (!string.IsNullOrEmpty(gp.searchField) && !string.IsNullOrEmpty(gp.searchString))
                {
                    ...
                    if (gp.searchOper.Equals("eq"))
                    {
                        jsonData = IE.Where(e => (e.EffectiveFrom.Value.ToShortDateString().ToString().Contains(gp.searchString))
                            ...
                            ).Select(a => new Object[] { a.EffectiveFrom.Value.ToShortDateString(), a.EffectiveTo.Value.ToShortDateString(), a.Id }).ToList();
                        //jsonData = IE.Select(a => new Object[] { a.Id, Convert.ToString(a.Code), Convert.ToString(a.Name), Convert.ToString(a.BusinessType) != null ? Convert.ToString(a.BusinessType.Id) : "" }).Where(a => a.Contains(Convert.ToInt32(gp.searchString))).ToList();
                    }
                    ...
                    {
                        int h = pageIndex * pageSize;
                        jsonData = IE.Skip(pageIndex).Take(pageSize).Select(a => new Object[] { a.EffectiveFrom, a.EffectiveTo, a.Id }).ToList();
                    }
                    totalRecords = IE.Count();
                }
                else
                {
                    ...
                    {
                        IE = IE.OrderBy(orderfuc);
                        jsonData = IE.Select(a => new Object[] { Convert.ToString(a.EffectiveFrom.Value.ToShortDateString()), Convert.ToString(a.EffectiveTo.Value.ToShortDateString()), a.Id }).ToList();
                    }
                    else if (gp.sord == "desc")
                    {
                        IE = IE.OrderByDescending(orderfuc);
