...
{
    [AuthoriseManger]
    public class ITInvestmentPayment80CCCto80CCFController : Controller
    {
        //
        // GET: /ITInvestmentPayment80CCCto80CCF/
        //private DataBaseContext db = new DataBaseContext();
        public ActionResult Index()
        {
            return View("~/Views/Payroll/MainViews/ITInvestmentPayment80CCCto80CCF/Index.cshtml");
        }
        public ActionResult partial()
        {
            return View("~/Views/Shared/Payroll/_ITInvestmentPayment.cshtml");
        }
        public ActionResult partialSubInvest()
        {
            return View("~/Views/Shared/Payroll/_SubInvestmentPayment.cshtml");
        }
        ...
        public ActionResult GetITInvestmentLKDetails(List<int> SkipIds)
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                ...
                //var r = (
                //    from ca in fall.Where(e => e.IsSalaryHead == false)
                ...
                foreach (var a in fall.Where(e => e.IsSalaryHead == false))
                {
                    var returndata = db.ITSection.Include(e => e.ITSectionListType).Include(e => e.ITSectionList).Where(e => e.Id == a.ITSection_Id).FirstOrDefault();
                    returnlist.Add(new ITSectionClass()
                    {
                        lookupvalue = a.FullDetails + " - " + returndata.FullDetails,
                        srno = a.Id,
        ...
        //            .Include(e => e.ITSectionListType)
        //            //  .Where(e => e.ITSectionListType.LookupVal.ToUpper() == "EXTRAREBATE").ToList();
        //        .Where(e => e.ITSectionListType.LookupVal.ToUpper() == "EXTRAREBATE").FirstOrDefault();
        //        var returnpara = new
        //        {
        //            //Id = fall.Select(a => a.Id.ToString()).ToArray(),
        //            //FullDetails = fall.Select(a => a.FullDetails).ToArray()
        //            Id = fall.Id,
        //            FullDetails = fall.FullDetails
        //        };
        ...
        public ActionResult GetSubInvPayLKDetails(List<int> SkipIds)
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                ...
                var r = (from ca in fall select new { srno = ca.Id, lookupvalue = ca.ActualInvestment }).Distinct();
        ...
        public ActionResult Create(ITInvestmentPayment ITP, FormCollection form, String forwarddata) //Create submit
        {
            ...
            using (DataBaseContext db = new DataBaseContext())
            {
                try
                {
                    ...
                    string LoanAdvHeadlist = form["LoanAdvanceHeadlist"] == "0" ? "" : form["LoanAdvanceHeadlist"];
                    string ITSubInvestmentPaymentlist = form["ITSubInvestmentPaymentlist2"] == "0" ? "" : form["ITSubInvestmentPaymentlist2"];
                    ...
                    if (ITInvestmentslist != null && ITInvestmentslist != "")
                    {
                        ...
                        ITP.ITSection = db.ITSection.Find(value.ITSection_Id);
                    }
                    ...
                    if (ITSubInvestmentPaymentlist != null && ITSubInvestmentPaymentlist != "")
                    {
                        List<int> ITPayIds = one_ids(ITSubInvestmentPaymentlist);
                        foreach (var ca in ITPayIds)
                        {
                            var value = db.ITSubInvestmentPayment.Find(ca);
                            ObjITSubInvestmentPayment.Add(value);
                    ...
                    if (ModelState.IsValid)
                    {
                        ...
                        {
                            OEmployee = db.Employee.AsNoTracking().Where(r => r.Id == i).Select(q => q.Id).SingleOrDefault();

                            //  OEmployeePayroll= db.EmployeePayroll.Where(e => e.Employee.Id == OEmployee).SingleOrDefault();
                     ...
                     .Include(e => e.ITInvestmentPayment.Select(r => r.FinancialYear)).AsNoTracking().Where(e => e.Employee.Id == OEmployee).SingleOrDefault();
                            ...
                            var calf = OEmployeePayroll.ITInvestmentPayment.Any(e => e.FinancialYear.Id == ITP.FinancialYear.Id && e.InvestmentDate == ITP.InvestmentDate
                                && e.ITSection.Id == ITP.ITSection.Id && e.ITInvestment.Id == ITP.ITInvestment.Id);
                            if (calf == true)
                            {
                            ...
                            using (TransactionScope ts = new TransactionScope())
                            {
                                try
                                {
                                    ...
                                    else
                                    {
                                      ...
                                      var aa = db.EmployeePayroll.Include(e => e.ITInvestmentPayment).Where(e => e.Employee.Id == OEmployee).SingleOrDefault();
                        ...
                        if (ModelState.IsValid)
                        {
                            ...
                            catch (DbUpdateConcurrencyException ex)
                            {
                                ...
                                if (databaseEntry == null)
                                {
                                    ...
                                    List<string> Msgd = new List<string>();
                                    Msgd.Add("Unable to save changes. The record was deleted by another user");
                                    return Json(new Utility.JsonReturnClass { success = false, responseText = Msgd }, JsonRequestBehavior.AllowGet);
                                }
                                else
                                {
                            ...
                            List<string> Msgs = new List<string>();
                            Msgs.Add("Record modified by another user.So refresh it and try to save again.");
            ...
            using (DataBaseContext db = new DataBaseContext())
            {
                try
                {
                    ...
                    else if (auth_action == "M")
                    {
                        ...
                        {
                            List<string> Msgn = new List<string>();
                            Msgn.Add("Data removed from history");
                            return Json(new Utility.JsonReturnClass { success = true, responseText = Msgn }, JsonRequestBehavior.AllowGet);
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (gp.sord == "asc")
                    {
                        ...
                        jsonData = IE.Select(a => new Object[] { a.Id, a.InvestmentDate.Value.ToString("dd/MM/yyyy"), Convert.ToString(a.ActualInvestment), a.DeclaredInvestment }).ToList();
                    }
                    ...
                    {
                        int h = pageIndex * pageSize;
                        jsonData = IE.Skip(pageIndex).Take(pageSize).Select(a => new Object[] { a.Id, a.InvestmentDate.Value.ToString("dd/MM/yyyy"), a.ActualInvestment, a.DeclaredInvestment }).ToList();
                    }
        ...
        public class ITinvestmentPaymentChildDataClass
        {
            ...
            public string DeclaredSubInvestment { get; set; }
            public string ActualSubInvestment { get; set; }
            public string ITSection { get; set; }
        }
            ...
            using (DataBaseContext db = new DataBaseContext())
            {
                try
                {
                    var db_data = db.EmployeePayroll.Include(e => e.ITInvestmentPayment)
                        ...
                        .Include(e => e.ITInvestmentPayment.Select(t => t.ITSubInvestmentPayment))
                        .Include(e => e.ITInvestmentPayment.Select(t => t.ITSubInvestmentPayment.Select(q => q.ITSubInvestment)))
                    ...
                    if (db_data.Count > 0)
                    {
                        ...
                        //if (db_data.SelectMany(e=>e.ITInvestmentPayment.Select(t=>t.ITSection.ITSectionListType.LookupVal.ToUpper() == " "))) { }
                        List<ITinvestmentPaymentChildDataClass> returndata = new List<ITinvestmentPaymentChildDataClass>();
                        var ca = db_data.SelectMany(e => e.ITInvestmentPayment).ToList();

                        foreach (var item in ca)
                        {
                            var itsection = item.ITSection;
                            //  if (itsection != null && itsection.ITSectionListType.LookupVal.ToUpper() == "EXTRAREBATE")   //var A = item.ITSection.ITSectionListType.LookupVal.ToUpper() == "EXTRAREBATE";
                            if (itsection != null && itsection.ITSectionListType.LookupVal.ToUpper() == "EXTRAREBATE")   //var A = item.ITSection.ITSectionListType.LookupVal.ToUpper() == "EXTRAREBATE";
                            {
                                var itinvest = item.ITInvestment;
                                if (item.ITSubInvestmentPayment.Count > 0)
                                {
                                    foreach (var itemsub in item.ITSubInvestmentPayment)
                                    {
                                        var itsubinvest = itemsub.ITSubInvestment;
                                        if (itsubinvest != null)
                                        {
                                            returndata.Add(new ITinvestmentPaymentChildDataClass
                                            {
                                                Id = item.Id.ToString(),
                                                ITInvestmentName = itinvest != null ? itinvest.ITInvestmentName : "",
                                                ...
                                                ITSection = item.ITSection.ITSectionList.LookupVal != null ? item.ITSection.ITSectionList.LookupVal.ToString() : "",
                                                SubId = itemsub.Id.ToString(),
                                                SubInvestmentName = itsubinvest.SubInvestmentName,
                                                ActualSubInvestment = itemsub.ActualInvestment.ToString(),
                                                DeclaredSubInvestment = itemsub.DeclaredInvestment.ToString(),
                                                // SubInvestmentName=itemsub.
                                            });
                                        }
                                ...
                                else
                                {
                                    returndata.Add(new ITinvestmentPaymentChildDataClass
                                    {
                                        ...
                                        ITInvestmentName = itinvest != null ? itinvest.ITInvestmentName : "",
                                        InvestmentDate = item.InvestmentDate != null ? item.InvestmentDate.Value.ToString("dd/MM/yyyy") : "",
                                        DeclaredInvestment = item.DeclaredInvestment.ToString(),
                                        ActualInvestment = item.ActualInvestment.ToString(),
                                        ITSection = item.ITSection.ITSectionList.LookupVal != null ? item.ITSection.ITSectionList.LookupVal.ToString() : "",
                                        SubId = "",
                                        SubInvestmentName = "",
                                        ActualSubInvestment = "",
                                        DeclaredSubInvestment = "",
                                        // SubInvestmentName=itemsub.
                                    });
                                }
                            }
        ...
        public JsonResult EditGridDetailsSubInvest(int data)
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                var Q = db.ITSubInvestmentPayment.Include(e => e.ITSubInvestment)
                     .Where(e => e.Id == data).Select
                     (e => new
                     {
                         ITInvestmentName = e.ITSubInvestment.SubInvestmentName,
                         InvestmentDate = e.InvestmentDate,
                         DeclaredInvestment = e.DeclaredInvestment,
                         ActualInvestment = e.ActualInvestment,
                         Action = e.DBTrack.Action
                     }).ToList();
                var ITInvestmentPayment = db.ITSubInvestmentPayment.Find(data);
                Session["RowVersion"] = ITInvestmentPayment.RowVersion;
                var Auth = ITInvestmentPayment.DBTrack.IsModified;
                //return Json(new Object[] { Q, add_data, "", Auth, JsonRequestBehavior.AllowGet });
                return Json(new { data = Q }, JsonRequestBehavior.AllowGet);
        ...
        public ActionResult GridEditSaveSubInvest(ITSubInvestmentPayment ITP, FormCollection form, string data)
        {
            List<string> Msg = new List<string>();
            using (DataBaseContext db = new DataBaseContext())
            {
                try
                {
                    ...
                    {
                        var id = Convert.ToInt32(data);
                        var db_data = db.ITSubInvestmentPayment.Where(e => e.Id == id).SingleOrDefault();
                        db_data.ActualInvestment = ITP.ActualInvestment;
                        db_data.DeclaredInvestment = ITP.DeclaredInvestment;
                        try
                        {
                            db.ITSubInvestmentPayment.Attach(db_data);
                            db.Entry(db_data).State = System.Data.Entity.EntityState.Modified;
                            db.SaveChanges();
                            db.Entry(db_data).State = System.Data.Entity.EntityState.Detached;
                            return Json(new { status = true, responseText = "Record Updated..!" }, JsonRequestBehavior.AllowGet);
        ...
        public ActionResult GridDelete(int data)
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                var IT80c = db.ITInvestmentPayment.Include(e => e.ITSubInvestmentPayment).Where(e => e.Id == data).SingleOrDefault();
                string deletefilepath = IT80c.Path;
                ...
                if (IT80c.ITSubInvestmentPayment.Count > 0)
                {
                    db.ITSubInvestmentPayment.RemoveRange(IT80c.ITSubInvestmentPayment);
                }
        ...
        public ActionResult GridDeleteSubInvest(int data)
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                var IT80c = db.ITSubInvestmentPayment.Where(e => e.Id == data).SingleOrDefault();
                string deletefilepath = IT80c.Path;
                if (deletefilepath != null)
                {
                    FileInfo File = new FileInfo(deletefilepath);
                ...
                //    db.ITSubInvestmentPayment.RemoveRange(IT80c.ITSubInvestmentPayment);
                //}
                db.ITSubInvestmentPayment.Remove(IT80c);
                db.SaveChanges();
                //return Json(new Object[] { "", "", " Record Deleted Successfully " }, JsonRequestBehavior.AllowGet);
                List<string> Msgs = new List<string>();
                Msgs.Add("Record Deleted Successfully ");
