...
namespace P2BUltimate.Controllers.Payroll.MainController
{
    ...
    public class ITInvestmentPayment80CController : Controller
    {
        ...
        //        .Where(e => e.ITSectionListType.LookupVal.ToUpper() == "REBATE").FirstOrDefault();
        //        var returnpara = new
        //        {
        ...
        public ActionResult GetSubInvPayLKDetails(List<int> SkipIds)
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                List<ITSubInvestmentPayment> fall = new List<ITSubInvestmentPayment>();
                fall = db.ITSubInvestmentPayment.Include(e => e.ITSubInvestment).ToList();
                var fall1 = db.ITInvestmentPayment.SelectMany(e => e.ITSubInvestmentPayment).ToList();
                ...
                foreach (var item in fall1)
                {
                    if (SkipIds != null)
                    {
                        ...
                        {
                            if (fall == null)
                                fall = db.ITSubInvestmentPayment.Include(e => e.ITSubInvestment).Where(e => !e.Id.ToString().Contains(a.ToString())).ToList();
                            else
                                fall = fall.Where(e => e.Id != a && e.Id != item.Id).ToList();

                        }
                    }
                    else
                    {
                        fall = fall.Where(e =>  e.Id != item.Id).ToList();
                    }
        ...
        public ActionResult Create(ITInvestmentPayment ITP, FormCollection form, String forwarddata) //Create submit
        {
            ...
            using (DataBaseContext db = new DataBaseContext())
            {
                try
                {
                    ...
                    {
                        ITInvestmentc = int.Parse(ITInvestmentslist);
                        ITInvestment ITInvest = db.ITInvestment.Find(ITInvestmentc);
                        ITP.ITInvestment = ITInvest;
                        ITP.ITSection = db.ITSection.Find(ITInvest.ITSection_Id);
                    }
                    ...
                    if (ModelState.IsValid)
                    {
                     ...
                     .Include(e => e.ITInvestmentPayment.Select(r => r.FinancialYear)).AsNoTracking().Where(e => e.Employee.Id == OEmployee).SingleOrDefault();
                            ...
                            if (calf == true)
                            {
                                Msg.Add("  Data Already Exist For THis Employee For Date " + ITP.InvestmentDate.Value.ToShortDateString() + ".");
                                return Json(new Utility.JsonReturnClass { success = false, responseText = Msg }, JsonRequestBehavior.AllowGet);
                            }
                            using (TransactionScope ts = new TransactionScope())
                            {
                ...
                try
                {
                    ...
                    if (db_data.Count > 0)
                    {
                        ...
                        foreach (var item in ca)
                        {
                            ...
                            if (itsection != null && (itsection.ITSectionListType.LookupVal.ToUpper() == "EXTRAREBATE" || itsection.ITSectionListType.LookupVal.ToUpper() == "REBATE"))   //var A = item.ITSection.ITSectionListType.LookupVal.ToUpper() == "EXTRAREBATE";
                            {
                                        ...
                                        //{
                                            returndata.Add(new ITinvestmentPaymentChildDataClass
                                            {
                                                ...
                                                ActualInvestment = item.ActualInvestment != null ? item.ActualInvestment.ToString() : "",
                                                 ITSection = item.ITSection.ITSectionList.LookupVal != null ? item.ITSection.ITSectionList.LookupVal.ToString() : "",   ///d
                                ...
                                else
                                {
                                    ...
                                    {
                                        Id = item.Id.ToString(),                                        	
                                        ITInvestmentName = item.ITInvestment_Id!=null?item.ITInvestment.ITInvestmentName:db.LoanAdvanceHead.Where(e => item.LoanAdvanceHead_Id!=null && e.Id == item.LoanAdvanceHead_Id).FirstOrDefault().Name,
                                        InvestmentDate = item.InvestmentDate != null ? item.InvestmentDate.Value.ToString("dd/MM/yyyy") : "",
