...
{
    [AuthoriseManger]
    public class ITInvestmentPaymentSection80Dto80UController : Controller
    {
        //
        // GET: /ITInvestmentPaymentSection80Dto80U/
        // private DataBaseContext db = new DataBaseContext();
        public ActionResult Index()
        {
            return View("~/Views/Payroll/MainViews/ITInvestmentPaymentSection80Dto80U/Index.cshtml");
        }
        ...
        public ActionResult GetITInvestmentLKDetails(List<int> SkipIds)
        {
            ...
            using (DataBaseContext db = new DataBaseContext())
            {
                var fall = db.ITSection
                    ...
                    .Where(e => e.ITSectionListType.LookupVal.ToUpper() == "DEDUCT")
        ...
        public ActionResult GetITSectionByDefault(List<String> ITlist)
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                ...
                //    .Include(e => e.ITSectionList)
                //    .Include(e => e.ITSectionListType)
                //    //.Where(e => e.ITSectionListType.LookupVal.ToUpper() == "DEDUCT").ToList();
                //.Where(e => e.ITSectionListType.LookupVal.ToUpper() == "DEDUCT").FirstOrDefault();
        ...
        public ActionResult Create(ITInvestmentPayment ITP, FormCollection form, String forwarddata) //Create submit
        {
            ...
            using (DataBaseContext db = new DataBaseContext())
            {
                ...
                {
                    string Emp = form["Employee-Table"] == "0" ? "" : form["Employee-Table"];
                    string ITInvestmentlist = form["ITInvestmentlist"] == "0" ? "" : form["ITInvestmentlist"];
                    ...
                    {
                        var value = db.ITInvestment.Find(int.Parse(ITInvestmentlist));
                        ITP.ITInvestment = value;
                        ITP.ITSection = db.ITSection.Find(value.ITSection_Id);
                    }

                    ITP.ITSubInvestmentPayment = null;
                    List<ITSubInvestmentPayment> OBJ = new List<ITSubInvestmentPayment>();
                    if (ITSubInvestmentPaymentlist != null && ITSubInvestmentPaymentlist != "")
                    {
                        var ids1 = Utility.StringIdsToListIds(ITSubInvestmentPaymentlist);
                        foreach (var ca in ids1)
                        {
                            var OBJ_val = db.ITSubInvestmentPayment.Find(ca);
                            OBJ.Add(OBJ_val);
                            ITP.ITSubInvestmentPayment = OBJ;
                        }
                    }
                    ...
                    .Include(e => e.ITInvestmentPayment.Select(r => r.FinancialYear)).AsNoTracking().Where(e => e.Employee.Id == OEmployee).SingleOrDefault();
                            ...
                            using (TransactionScope ts = new TransactionScope())
                            {
                                try
                                {
                                    ...
                                    {
                                        var aa = db.EmployeePayroll.Include(q=>q.ITInvestmentPayment).Where(a=>a.Id==OEmployeePayroll.Id).SingleOrDefault();
                                    //    var temp=aa.ITInvestmentPayment.ToList();
                                        if (aa.ITInvestmentPayment.Count>0 && aa.ITInvestmentPayment!=null)
                                        {
                            ...
                            if (ModelState.IsValid)
                            {
                                ...
                                catch (DbUpdateConcurrencyException ex)
                                {
                                    ...
                                    if (databaseEntry == null)
                                    {
                                        //return Json(new Object[] { "", "", "Unable to save changes. The record was deleted by another user.", JsonRequestBehavior.AllowGet });
                                        List<string> Msgn = new List<string>();
                                        Msgn.Add(" Unable to save changes. The record was deleted by another user. ");
                                        return Json(new Utility.JsonReturnClass { success = false, responseText = Msgn }, JsonRequestBehavior.AllowGet);
                                    }
                                    else
                                    {
                                ...
                                Msgp.Add("Record modified by another user.So refresh it and try to save again.");
                                return Json(new Utility.JsonReturnClass { success = false, responseText = Msgp }, JsonRequestBehavior.AllowGet);
                                //return Json(new Object[] { "", "", "Record modified by another user.So refresh it and try to save again.", JsonRequestBehavior.AllowGet });
                            }
                        }
                        else
                ...
                try
                {
                    ...
                    if (db_data.Count > 0)
                    {
                        ...
                        foreach (var item in ca)
                        {
                            ...
                            if (item.ITSection != null && item.ITSection.ITSectionListType.LookupVal.ToUpper() == "DEDUCT")
                            {
                ...
                try
                {
                    ...
                    if (data != null)
                    {
                        ...
                        try
                        {
                            ...
                            db.Entry(db_data).State = System.Data.Entity.EntityState.Detached;
                            Msg.Add("  Record Updated");
                            return Json(new { status = true, data = db_data.ToString(), responseText = "Record Updated..!" }, JsonRequestBehavior.AllowGet);
