...
{
    public class ITSection24PaymentController : Controller
    {
        //private DataBaseContext db = new DataBaseContext();
        //
        // GET: /ITSection24Payment/
        public ActionResult Index()
        {
            return View("~/Views/Payroll/MainViews/ITSection24Payment/Index.cshtml");
        }
        ...
        public ActionResult GetITSectionByDefault()
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                var fall = db.ITSection.Include(e => e.ITInvestments)
                    ...
                    .Where(e => e.ITSectionListType.LookupVal.ToUpper() == "SECTION24").ToList();
                var returnpara = new
                {
        ...
        public ActionResult GetITSectionLKDetails(List<int> SkipIds)
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                var fall = db.ITSection.Include(e => e.ITSectionList).Include(e => e.ITSectionListType)
                       .Where(e => e.ITSectionList.LookupVal.ToUpper() == "SECTION24")
                       .ToList();
            ...
            using (DataBaseContext db = new DataBaseContext())
            {
                try
                {
                    ...
                    if (db_data.Count > 0)
                    {
                        ...
                        {
                            if (item.ITSection != null && item.ITSection.ITSectionList.LookupVal.ToUpper() == "SECTION24")
                            {
