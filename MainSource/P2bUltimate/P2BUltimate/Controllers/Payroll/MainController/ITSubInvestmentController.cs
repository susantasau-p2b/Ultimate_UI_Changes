       ...
       // private DataBaseContext db = new DataBaseContext();
        ...
        public ActionResult P2BGrid(P2BGrid_Parameters gp)
        {
            try
            {
                ...
                var ITSubInvestment = db.ITSubInvestment.ToList();
                IEnumerable<ITSubInvestment> IE;
                if (!string.IsNullOrEmpty(gp.searchField))
                {
                    IE = ITSubInvestment;
                    if (gp.searchOper.Equals("eq"))
                    {
                        jsonData = IE.Select(a => new Object[] { a.Id, a.SubInvestmentName }).Where(a => a.Contains(Convert.ToInt32(gp.searchString))).ToList();
                    }
                    if (pageIndex > 1)
                    {
                        int h = pageIndex * pageSize;
                        jsonData = IE.Skip(pageIndex).Take(pageSize).Select(a => new Object[] { a.Id, a.SubInvestmentName }).ToList();
                    }
                ...
                {
                    IE = ITSubInvestment;
                    Func<ITSubInvestment, string> orderfuc = (OBJ => gp.sidx == "ID" ? OBJ.Id.ToString() :
                                                               gp.sidx == "SubInvestmentName" ? OBJ.SubInvestmentName : "");

                    if (gp.sord == "asc")
                    {
                        IE = IE.OrderBy(orderfuc);
                        jsonData = IE.Select(a => new Object[] { a.Id, Convert.ToString(a.SubInvestmentName) }).ToList();
                    }
                    ...
                    totalRecords = ITSubInvestment.Count();
                }
                if (totalRecords > 0)
                {
