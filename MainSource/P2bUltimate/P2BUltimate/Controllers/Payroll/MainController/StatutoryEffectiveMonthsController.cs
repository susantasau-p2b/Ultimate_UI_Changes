...
{
    [AuthoriseManger]
    public class StatutoryEffectiveMonthsController : Controller
    {
        ...
        {
            return View("~/Views/Shared/Payroll/_Range.cshtml");
        }
        ...
        public ActionResult Create(StatutoryEffectiveMonths S, FormCollection form) //Create submit
        {
            ...
            using (DataBaseContext db = new DataBaseContext())
            {
                try
                {
                    ...
                    if (StatutoryWageRangelist != null)
                    {
                        var ids = Utility.StringIdsToListIds(StatutoryWageRangelist);
                        foreach (var ca in ids)
                        {
                            var Range_val = db.Range.Find(ca);
                            Range.Add(Range_val);
                            S.StatutoryWageRange = Range;
                        }
                    }
                    else
                    {
                        S.StatutoryWageRange = null;
                    }
                        ...
                        if (ModelState.IsValid)
                        {
                            using (TransactionScope ts = new TransactionScope())
                            {
                                ...
                                StatutoryEffectiveMonths SEM = new StatutoryEffectiveMonths()
                                {
                                    ...
                                    StatutoryWageRange = S.StatutoryWageRange,
                                    DBTrack = S.DBTrack,
                                    Gender = S.Gender
                                };
                                try
                                {
                                    db.StatutoryEffectiveMonths.Add(SEM);
                                    db.SaveChanges();
                                    //DBTrackFile.DBTrackSave("Core/P2b.Global", "C", corporate, null, "Corporate", null);
                                    ts.Complete();
                                    Msg.Add("  Data Saved successfully  ");
                                    return Json(new Utility.JsonReturnClass { Id = SEM.Id, Val = SEM.EffectiveMonth.LookupVal + " - " + (SEM.Gender != null ? SEM.Gender.LookupVal : ""), success = true, responseText = Msg }, JsonRequestBehavior.AllowGet);
        ...
        public ActionResult Edit(int data)
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                var Q = db.StatutoryEffectiveMonths
                ...
                List<StatutoryEffectiveMonthsWagesRangeList> StatutoryEffectiveMonthsWagesRangeList = new List<StatutoryEffectiveMonthsWagesRangeList>();
                var KS = db.StatutoryEffectiveMonths.Include(e => e.StatutoryWageRange).Where(e => e.Id == data).Select(e => e.StatutoryWageRange).ToList();
                foreach (var val in KS)
                {
                    StatutoryEffectiveMonthsWagesRangeList.Add(new StatutoryEffectiveMonthsWagesRangeList
                    {
                        StatutoryEffectiveMonthsWagesRange_Id = val.Select(e => e.Id.ToString()).ToArray(),
                        StatutoryEffectiveMonthsWagesRange_Fulldetails = val.Select(e => e.FullDetails).ToArray()
                    });
                }

                var Corp = db.StatutoryEffectiveMonths.Find(data);
                TempData["RowVersion"] = Corp.RowVersion;
                var Auth = Corp.DBTrack.IsModified;
                return Json(new Object[] { Q, StatutoryEffectiveMonthsWagesRangeList, Auth, JsonRequestBehavior.AllowGet });
            }
        }

        [HttpPost]
        public async Task<ActionResult> EditSave(StatutoryEffectiveMonths S, int data, FormCollection form) // Edit submit
        {
            List<string> Msg = new List<string>();
            using (DataBaseContext db = new DataBaseContext())
            {
                try
                {
                    ...
                    if (Auth == false)
                    {
                        ...
                        if (ModelState.IsValid)
                        {
                            try
                            {
                                ...
                                {
                                    StatutoryEffectiveMonths typedetails = null;
                                    List<Range> Range = new List<Range>();
                                    typedetails = db.StatutoryEffectiveMonths.Include(e => e.StatutoryWageRange).Where(e => e.Id == data).SingleOrDefault();
                                    if (StatutoryWageRangelist != null && StatutoryWageRangelist != "")
                                    {
                                        var ids = Utility.StringIdsToListIds(StatutoryWageRangelist);
                                        foreach (var ca in ids)
                                        {
                                            var StatutaryWagesRange_val = db.Range.Find(ca);
                                            Range.Add(StatutaryWagesRange_val);
                                            typedetails.StatutoryWageRange = Range;
                                        }
                                    }
                                    else
                                    {
                                        typedetails.StatutoryWageRange = null;
                                    }
                                    if (EffectiveMonth != null)
                                    {
                                        if (EffectiveMonth != "")
                                        {
                                            ...
                                            IList<StatutoryEffectiveMonths> typedetails1 = null;
                                            if (type.EffectiveMonth != null)
                                            {
                                                typedetails1 = db.StatutoryEffectiveMonths.Where(x => x.EffectiveMonth.Id == type.EffectiveMonth.Id && x.Id == data).ToList();
                                            }
                                            else
                                            {
                                                typedetails1 = db.StatutoryEffectiveMonths.Where(x => x.Id == data).ToList();
                                            }
                                    ...
                                    db.StatutoryEffectiveMonths.Attach(typedetails);
                                    ...
                                    StatutoryEffectiveMonths blog = null; // to retrieve old data
                                    DbPropertyValues originalBlogValues = null;

                                    using (var context = new DataBaseContext())
                                    {
                                        blog = context.StatutoryEffectiveMonths.Where(e => e.Id == data).SingleOrDefault();
                                    ...
                                    if (Gender != null && Gender != "")
                                    {
                                        ...
                                        var val = db.LookupValue.Find(int.Parse(Gender));
                                        S.Gender = val;
                                        S.Gender_Id = int.Parse(Gender);
                                        var type = db.StatutoryEffectiveMonths.Where(e => e.Id == data).SingleOrDefault();
                                        IList<StatutoryEffectiveMonths> typedetails1 = null;
                                        if (type.Gender != null)
                                        {
                                            typedetails1 = db.StatutoryEffectiveMonths.Where(x => x.Gender.Id == type.Gender.Id && x.Id == data).ToList();
                                        }
                                        else
                                        {
                                            typedetails1 = db.StatutoryEffectiveMonths.Where(x => x.Id == data).ToList();
                            ...
                            catch (DbUpdateConcurrencyException ex)
                            {
                                ...
                                var clientValues = (StatutoryEffectiveMonths)entry.Entity;
                    ...
                    else
                    {
                        using (TransactionScope ts = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                        {
                            ...
                            StatutoryEffectiveMonths blog = null; // to retrieve old data
                            DbPropertyValues originalBlogValues = null;
                            StatutoryEffectiveMonths Old_Corp = null;
                            ...
                            using (var context = new DataBaseContext())
                            {
                                ...
                                Old_Corp = context.StatutoryEffectiveMonths.Where(e => e.Id == data).SingleOrDefault();

                                //db.SaveChanges();
                            }
                            blog.DBTrack = S.DBTrack;
                            db.StatutoryEffectiveMonths.Attach(blog);
