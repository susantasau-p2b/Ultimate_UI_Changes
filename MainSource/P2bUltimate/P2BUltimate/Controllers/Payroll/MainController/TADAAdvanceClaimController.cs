...
namespace P2BUltimate.Controllers.Payroll.MainController
{
    public class TADAAdvanceClaimController : Controller
    {
        ...
        {
            return View("~/Views/Payroll/MainViews/TADAAdvanceClaim/Index.cshtml");
        }
        public ActionResult GridPartial()
        {
            return View("~/Views/Shared/Payroll/_TADAAdvClaimGridPartial.cshtml");
        }
        ...
        public ActionResult GridEditData(int data)
        {
            ...
            using (DataBaseContext db = new DataBaseContext())
            {
                if (data != 0)
                {
                    var retrundataList = db.TADAAdvanceClaim.Where(e => e.Id == data).FirstOrDefault();
        ...
        public JsonResult GetEmpSalStruct(string data)
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                ...
                //        .Where(s =>  s.SalaryHead.Type.LookupVal.ToUpper() == "EARNING"
        ...
        public ActionResult GridEditSave(TADAAdvanceClaim TADAAdvanceClaim, FormCollection form, string data)
        {
            List<string> Msgs = new List<string>();
            using (DataBaseContext db = new DataBaseContext())
            {
                int Id = int.Parse(data);
                var TADAAdvClaimData = db.TADAAdvanceClaim.Where(e => e.Id == Id).SingleOrDefault();
                string ProposedPlace = form["ProposedPlace"];

                double TADAAdvanceAmount = Convert.ToDouble(form["TADAAdvanceAmount"]);
                string Remark = form["Remark"];

                TADAAdvClaimData.ProposedPlace = ProposedPlace;
                TADAAdvClaimData.AdvAmt = TADAAdvanceAmount;
                TADAAdvClaimData.Remark = Remark;

                using (TransactionScope ts = new TransactionScope())
                {

                    TADAAdvClaimData.DBTrack = new DBTrack
                    {
                        CreatedBy = TADAAdvClaimData.DBTrack.CreatedBy == null ? null : TADAAdvClaimData.DBTrack.CreatedBy,
                        CreatedOn = TADAAdvClaimData.DBTrack.CreatedOn == null ? null : TADAAdvClaimData.DBTrack.CreatedOn,
                        Action = "M",
                    ...
                    try
                    {
                        db.TADAAdvanceClaim.Attach(TADAAdvClaimData);
                        db.Entry(TADAAdvClaimData).State = System.Data.Entity.EntityState.Modified;
                        db.SaveChanges();
                        db.Entry(TADAAdvClaimData).State = System.Data.Entity.EntityState.Detached;
                    ...
                    {
                        return RedirectToAction("Create", new { concurrencyError = true, id = TADAAdvClaimData.Id });
                    }
        ...
        public ActionResult Create(TADAAdvanceClaim TADAAdvClaim, FormCollection form)
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                ...
                //    if (test.TADAAdvanceClaim != null)
                //    {
                //        Msg.Add("Record Already Exists.");
                //        return Json(new Utility.JsonReturnClass { success = false, responseText = Msg }, JsonRequestBehavior.AllowGet);
                //    }

                    //if (TADAAdvClaim.LvNewReq.FromDate <= TADAAdvClaim.TravelStartDate && TADAAdvClaim.LvNewReq.ToDate >= TADAAdvClaim.TravelEndDate)
                    //{
        ...
        public async Task<ActionResult> Delete(int data)
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                TADAAdvanceClaim TADAAdvance = db.TADAAdvanceClaim.Where(e => e.Id == data).SingleOrDefault();
                ...
                using (TransactionScope ts = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                {
                    ...
                    try
                    {
                       // db.EmployeeDocuments.RemoveRange(selectedRegions);
                        db.TADAAdvanceClaim.Remove(TADAAdvance);
