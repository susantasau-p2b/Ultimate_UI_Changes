...
{
    public class VdaArrearCPIEntryFormController : Controller
    {
        //  private DataBaseContext db = new DataBaseContext();
        List<string> Vda_Arrearlist = new List<string>();

        public ActionResult Index()
        {
            // db.RefreshAllEntites(RefreshMode.StoreWins);//added by prashnat 13042017
            return View("~/Views/Payroll/MainViews/VdaArrearCPIEntryForm/Index.cshtml");
        }
        ...
        public ActionResult P2BGridDa(P2BGrid_Parameters gp)
        {
            try
            {
                using (DataBaseContext db = new DataBaseContext())
                {
                    ...
                    foreach (var z in BindEmpList)
                    {
                        if (z.SalaryArrearT != null && z.SalaryArrearT.Count > 0)
                        {
                            ...
                            foreach (var Sal in z.SalaryArrearT.Where(e => e.ArrearType.LookupVal.ToUpper() == "DAARREAR"))
                            {
        ...
        [HttpPost]
        public ActionResult CreateNotUse(CPIEntryT C, FormCollection form, String forwarddata)
        {
        ...
        public ActionResult EditSave(CPIEntryT C, int data, FormCollection form) // Edit submit
        {
            ...
            using (DataBaseContext db = new DataBaseContext())
            {
                ...
                string Emp = form["Employee-Table"] == "0" ? "" : form["Employee-Table"];
                string Paymonth = form["PayMonth"] == "0" ? "" : form["PayMonth"];
                //  string DaPaymonth = damonth;
                string DaPaymonth = form["DapayMonth"] == "0" ? "" : form["DapayMonth"];
                int pagree = Convert.ToInt32(PayScale);
                ...
                if (DaPaymonth == "" || DaPaymonth == null)
                {
                    Msg.Add(" Please select DA arrear Pay Month ");
                    return Json(new Utility.JsonReturnClass { success = false, responseText = Msg }, JsonRequestBehavior.AllowGet);
                }
                var daarrear = db.Lookup.Include(e => e.LookupValues).Where(e => e.Code == "417").FirstOrDefault().LookupValues.Where(e => e.LookupVal.ToUpper() == "DAARREAR").FirstOrDefault();
                if (daarrear == null)
                {
                    Msg.Add(" Please Define DAARREAR in lookup there code is 417. ");
                    return Json(new Utility.JsonReturnClass { success = false, responseText = Msg }, JsonRequestBehavior.AllowGet);
                }
                if (db.SalaryT.Any(o => o.PayMonth == C.PayMonth && o.ReleaseDate == null))
                {
                    Msg.Add(" Please Release The Salary Then enter Vda new Point For Vda Arrear Process ");
                    return Json(new Utility.JsonReturnClass { success = false, responseText = Msg }, JsonRequestBehavior.AllowGet);
                }
                if (db.SalaryT.Any(o => o.PayMonth == DaPaymonth))
                {
                    Msg.Add(" Please Delete The Salary Then enter Vda new Point For Vda Arrear Process ");
                    return Json(new Utility.JsonReturnClass { success = false, responseText = Msg }, JsonRequestBehavior.AllowGet);
                }

                var cpicheck = db.CPIEntryT.AsNoTracking().Where(o => o.PayMonth == C.PayMonth).FirstOrDefault();

                if (cpicheck == null)
                {
                    Msg.Add(" CPIEntry For This Month Not available ");
                    return Json(new Utility.JsonReturnClass { success = false, responseText = Msg }, JsonRequestBehavior.AllowGet);
                }
                ...
                if (Auth == false)
                {
                    if (ModelState.IsValid)
                    {
                        ...
                        //{
                        //    blog = context.CPIEntryT.Where(e => e.PayMonth == Paymonth).Include(e => e.PayScale)
                        //                            .SingleOrDefault();
                        ...
                        var OEmployeePayroll = db.EmployeePayroll.Include(e => e.Employee).Include(e => e.Employee.GeoStruct)
                           .Include(e => e.Employee.PayStruct)
                           .Include(e => e.Employee.FuncStruct)
                           .Include(e => e.Employee.ServiceBookDates)
                           .Where(x => EmployeePayroll_int.Contains(x.Id)).ToList();

                        foreach (var EmpPay in OEmployeePayroll)
                        {
                            var CPIEntry = db.CPIEntryT.Where(e => e.PayMonth == Paymonth && e.EmployeePayroll_Id == EmpPay.Id).AsParallel().SingleOrDefault();

                            if (CPIEntry != null)
                            {
                                if (CPIEntry.OldActualIndexPoint == 0)
                                {
                                    CPIEntry.OldActualIndexPoint = CPIEntry.ActualIndexPoint;
                                    CPIEntry.OldCalIndexPoint = CPIEntry.CalIndexPoint;
                                }
                                                                  ...
                                                                  new System.TimeSpan(0, 30, 0)))
                                {
                                    try
                                    {
                                        SalaryHeadGenProcess.EmployeeSalaryStructUpdateForCPIVdaArrear(EmpPay, Convert.ToDateTime(CPIEntry.PayMonth));
                                        // add data in salary arrear T
                                        SalaryArrearT OSalaryArrearT = new SalaryArrearT();
                                        DateTime fromdate = Convert.ToDateTime(CPIEntry.PayMonth);
                                        DateTime Todate = Convert.ToDateTime(CPIEntry.PayMonth).AddMonths(1).AddDays(-1);
                                        var OarrearT = db.EmployeePayroll.Where(t => t.Id == EmpPay.Id).Include(z => z.SalaryArrearT)
                                                      .Include(z => z.SalaryArrearT.Select(a => a.ArrearType))
                                                      .Select(e => e.SalaryArrearT.Where(r => r.IsRecovery == false && r.ArrearType.LookupVal.ToUpper() == daarrear.LookupVal.ToUpper() && (r.FromDate <= fromdate && r.ToDate >= Todate)).FirstOrDefault()).SingleOrDefault();
                                        if (OarrearT == null)
                                        {
                                            OSalaryArrearT = new SalaryArrearT();
                                            {
                                                OSalaryArrearT.ArrearType = db.Lookup.Include(e => e.LookupValues).Where(e => e.Code == "417").FirstOrDefault().LookupValues.Where(e => e.LookupVal.ToUpper() == "DAARREAR").FirstOrDefault();
                                                OSalaryArrearT.FromDate = fromdate;
                                                OSalaryArrearT.TotalDays = Todate.Day;
                                                OSalaryArrearT.ToDate = Todate;
                                                OSalaryArrearT.IsAuto = false;
                                                OSalaryArrearT.IsPaySlip = true;
                                                OSalaryArrearT.IsRecovery = false;
                                                OSalaryArrearT.PayMonth = DaPaymonth;
                                                OSalaryArrearT.GeoStruct = db.GeoStruct.Find(EmpPay.Employee.GeoStruct.Id);
                                                OSalaryArrearT.FuncStruct = db.FuncStruct.Find(EmpPay.Employee.FuncStruct.Id);
                                                OSalaryArrearT.PayStruct = db.PayStruct.Find(EmpPay.Employee.PayStruct.Id);
                                                OSalaryArrearT.DBTrack = new DBTrack { Action = "C", CreatedBy = SessionManager.UserName, IsModified = false };
                                                OSalaryArrearT.EmployeePayroll_Id = EmpPay.Id;
                                            }
                                    ...
                                    catch (Exception ex)
                                    {
                                        ...
                                        var EmpCode = EmpPay.Employee.EmpCode;
                                        ...
                                        if (LineNo != null)
                                        {
                                            linenumber = string.Join(",", LineNo);
                                        }
                                        string Error_Vda_Arrear = ("Vda Arrears error " + "  " + "EmpCode :: " + EmpCode + " " + " LineNo : " + linenumber);
                                        Vda_Arrearlist.Add(Error_Vda_Arrear);
                                        LineNo.Clear();
                                    }
                                    ts.Complete();
                                }
                            ...
                            //Vda Arrear Process Not Create Cpi code Comment Start
                            //else
                            //{
                ...
                string vda_arrear_errormsg = "";
                if (Vda_Arrearlist != null)
                {
                    vda_arrear_errormsg = string.Join(",", Vda_Arrearlist);
                }
                watch.Stop();
                Utility.DumpProcessStatus(watch.Elapsed.Hours + ":" + watch.Elapsed.Minutes + ":" + watch.Elapsed.Seconds + ":" + watch.Elapsed.Milliseconds);
                Msg.Add("  Data Updated successfully. Please go to arrearpayment click on create button then select employee and click on Process button." + " " + vda_arrear_errormsg);
                return Json(new Utility.JsonReturnClass { success = true, responseText = Msg }, JsonRequestBehavior.AllowGet);
            }
        }
        //catch (Exception ex)
        ...
        public ActionResult P2BGrid(P2BGrid_Parameters gp)
        {
            try
            {
                ...
                if (gp.IsAutho == true)
                {
                    CPIEntryT = db.CPIEntryT.Where(e => e.DBTrack.IsModified == true && e.OldActualIndexPoint != 0).GroupBy(e => e.PayMonth, (key, e) => e.FirstOrDefault());
                }
                else
                {
                    CPIEntryT = db.CPIEntryT.Where(e => e.OldActualIndexPoint != 0).GroupBy(e => e.PayMonth, (key, e) => e.FirstOrDefault());
                }
