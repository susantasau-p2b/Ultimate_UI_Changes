...
namespace P2BUltimate.Controllers.Payroll.MainController
{
    public class YearlyPaymentTController : Controller
    {
        ...
        {
            return View("~/Views/Payroll/MainViews/YearlyPaymentT/Index.cshtml");
        }
        ...
        public ActionResult Create(YearlyPaymentT Y, FormCollection form, String forwarddata) //Create submit
        {
            ...
            using (DataBaseContext db = new DataBaseContext())
            {
                try
                {
                    ...
                    foreach (var itememp in ids)
                    {
                        var employeepayrol = db.EmployeePayroll.Include(e => e.Employee).Include(e => e.YearlyPaymentT).Where(e => e.Employee_Id == itememp).SingleOrDefault();
                        string empcode = employeepayrol.Employee.EmpCode;
                        YearlyPaymentT YearlyPayTchk = db.YearlyPaymentT.Include(q => q.SalaryHead).Where(a => a.EmployeePayroll_Id == employeepayrol.Id && a.SalaryHead.Id == Y.SalaryHead.Id && a.FromPeriod == Y.FromPeriod && a.ToPeriod == Y.ToPeriod).SingleOrDefault();
                        if (YearlyPayTchk != null)
                        {
                            Msg.Add(empcode + ": Yearly Payment for this period already exist.");
                    ...
                    if (ModelState.IsValid)
                    {
                        if (ids != null)
                        {
                            foreach (var i in ids)
                            {
                                ...
                                if (OEmployee.ServiceBookDates.ServiceLastDate != null && Y.SalaryHead.InPayslip == true)
                                {
                                    Msg.Add(" employee is retired and in salaryhead inpayslip is true!");
                                    return Json(new Utility.JsonReturnClass { success = false, responseText = Msg }, JsonRequestBehavior.AllowGet);
                                }

                                OEmployeePayroll
                                = db.EmployeePayroll.Include(e => e.YearlyPaymentT)
                              .Where(e => e.Employee.Id == i).SingleOrDefault();
                                ...
                                using (TransactionScope ts = new TransactionScope())
                                {
                                    try
                                    {
                                        ...
                                        int Salheadid = Convert.ToInt32(SalaryHeadlist);
                                        ...
                                        if (SalHead.SalHeadOperationType.LookupVal.ToUpper() == "BONUS")
                                        {
                                            ...
                                            if (BonusAmt > 0)
                                            {
                                                var BonusSalhead = db.SalaryHead.Include(e => e.SalHeadOperationType).Where(e => e.SalHeadOperationType.LookupVal.ToUpper() == "BONUS").FirstOrDefault();
                                            ...
                                            if (ExGratiaAmt > 0)
                                            {
                                                var ExGratiaSalhead = db.SalaryHead.Include(e=>e.SalHeadOperationType).Where(e => e.SalHeadOperationType.LookupVal.ToUpper() == "EXGRATIA").FirstOrDefault();
                                        ...
                                        else if (SalHead.SalHeadOperationType.LookupVal.ToUpper() != "BONUS" && SalHead.ProcessType.LookupVal.ToUpper() == "STANDARD")
                                        {
                                        ...
                                        else if (SalHead.SalHeadOperationType.LookupVal.ToUpper() != "BONUS" && SalHead.ProcessType.LookupVal.ToUpper() == "FIXEDMONTH")
                                        {
                                            ...
                                            Double Fixamt = 0;
                                            DateTime? mEffectiveDate = Convert.ToDateTime(Y.ProcessMonth);
                                          ...
                                          .Include(e => e.EmpSalStructDetails.Select(q => q.SalaryHead))
                                                ...
                                                //.Include(e => e.EmpSalStructDetails.Select(q => q.SalaryHead.RoundingMethod))
                                                //.Include(e => e.EmpSalStructDetails.Select(q => q.SalaryHead.SalHeadOperationType))
                                            .Where(e => e.EmployeePayroll.Id == OEmployeePayroll.Id && e.EffectiveDate >= mEffectiveDate && e.EffectiveDate <= enddate);
                                        ...
                                        else if (SalHead.SalHeadOperationType.LookupVal.ToUpper() != "BONUS" && SalHead.ProcessType.LookupVal.ToUpper() == "EARNED")
                                        {
                                            ...
                                            List<string> mPeriod = new List<string>();
                                            EmployeePayroll OEmpPayroll = db.EmployeePayroll.Include(e => e.Employee).Include(e => e.Employee.ServiceBookDates).Where(e => e.Id == OEmployeePayroll.Id).FirstOrDefault();
                                            ...
                                            foreach (var item in mPeriod)
                                            {
                                                ...
                                                var salearndedt = db.SalaryT.Include(e => e.SalEarnDedT).Include(e => e.SalEarnDedT.Select(r=>r.SalaryHead)).Where(e => e.PayMonth == item && e.EmployeePayroll.Id == OEmployeePayroll.Id).AsNoTracking().FirstOrDefault();
                                                if (salearndedt != null)
                                                {
                                            ...
                                            ObjYPT.AmountPaid = TotCalAmount;
                                            db.YearlyPaymentT.Add(ObjYPT);
                                            db.SaveChanges();
        ...
        public async Task<ActionResult> EditSave(string forwarddata, FormCollection form) // Edit submit
        {
            ...
            using (DataBaseContext db = new DataBaseContext())
            {
                ...
                try
                {
                    ...
                    DateTime Paymonthdate = Convert.ToDateTime(PayMonth);
                    ...
                    try
                    {
                        ...
                        if (AmountProcessFlag == "true")
                        {
                            ...
                            foreach (var ca in ids)
                            {
                                ...
                                using (TransactionScope ts = new TransactionScope(TransactionScopeOption.Required, new System.TimeSpan(2, 0, 0)))
                                {
                                    ...
                                    YearlyPaymentT YearlyPayT = db.YearlyPaymentT.Include(q => q.EmployeePayroll).Include(q => q.LvEncashReq).Where(a => a.Id == ca).SingleOrDefault();
                                    //int empay = YearlyPayT.EmployeePayroll.Id;
                                    var SalT = db.SalaryT.Where(e => e.EmployeePayroll_Id == YearlyPayT.EmployeePayroll_Id && e.PayMonth == PayMonth).ToList();
                                    if (SalT.Count() > 0)
                                    {
                                        var chkappearinPaySlip = db.SalaryHead.Where(e => e.InPayslip == true && e.Id == YearlyPayT.SalaryHead_Id).Select(e => e.Name).SingleOrDefault();
                                        if (chkappearinPaySlip != null)
                                        {
                                            Msg.Add(chkappearinPaySlip + " head appear in Payslip, You have processed the salary, so please delete salary for " + PayMonth + "month and try again.");
                                            return Json(new Utility.JsonReturnClass { success = false, responseText = Msg }, JsonRequestBehavior.AllowGet);
                                        }
                                    }


                                    if (YearlyPayT.LvEncashReq != null)
                                    {
                                        int LvencashId = YearlyPayT.LvEncashReq.Id;
                                    ...
                                    YearlyPaymentT YearlyPayTLvencashpayment = db.YearlyPaymentT.Include(q => q.EmployeePayroll).Include(q => q.LvEncashReq).Where(a => a.Id == ca).SingleOrDefault();
                                    if (YearlyPayTLvencashpayment.LvEncashReq != null)
                                    {
                                        LvEncashPayment Lvencashpaymentdata = db.LvEncashPayment.Include(e => e.LvEncashReq).Where(e => e.LvEncashReq.Id == YearlyPayTLvencashpayment.LvEncashReq.Id).SingleOrDefault();
                                    ...
                                    if (YearlyPayT.ProcessMonth != null)
                                    {
                                        DateTime processmonthdate = Convert.ToDateTime(YearlyPayT.ProcessMonth);
                                        if (processmonthdate > Paymonthdate)
                                        {
                                            YearlyPayT.ProcessMonth = PayMonth;
                                        }  
                                    }
                                    
                                   
                                    YearlyPayT.TDSAmount = FinalTDSAmount;
                        ...
                        else
                        {
                            using (TransactionScope ts1 = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                            {
                                foreach (var ca in ids)
                                {
                                    YearlyPaymentT YearlyPaychk = db.YearlyPaymentT.Include(q => q.EmployeePayroll).Include(q => q.LvEncashReq).Where(a => a.Id == ca).SingleOrDefault();

                                    var SalT = db.SalaryT.Where(e => e.EmployeePayroll_Id == YearlyPaychk.EmployeePayroll_Id && e.PayMonth == PayMonth).ToList();
                                    if (SalT.Count() > 0)
                                    {
                                        var chkappearinPaySlip = db.SalaryHead.Where(e => e.InPayslip == true && e.Id == YearlyPaychk.SalaryHead_Id).Select(e => e.Name).SingleOrDefault();
                                        if (chkappearinPaySlip != null)
                                        {
                                            Msg.Add(chkappearinPaySlip + " head appear in Payslip, You have processed the salary, so please delete salary for " + PayMonth + "month and try again.");
                                            return Json(new Utility.JsonReturnClass { success = false, responseText = Msg }, JsonRequestBehavior.AllowGet);
                                    ...
                                    YearlyPaymentT YearlyPayT1 = db.YearlyPaymentT.Include(q => q.LvEncashReq).Include(q => q.EmployeePayroll).Where(a => a.Id == ca).SingleOrDefault();

                                    if (YearlyPayT1.LvEncashReq != null)
                                    {
                                        int LvencashId = YearlyPayT1.LvEncashReq.Id;
                                        LvEncashReq lvupdate = db.LvEncashReq.Include(e => e.WFStatus).Where(e => e.Id == LvencashId).SingleOrDefault();
                                        lvupdate.TrClosed = true;
                                        lvupdate.WFStatus = db.Lookup.Include(e => e.LookupValues).Where(e => e.Code == "1000").FirstOrDefault().LookupValues.Where(e => e.LookupVal == "3").FirstOrDefault();//db.LookupValue.Where(e => e.LookupVal == "3").SingleOrDefault();
        ...
        //}
        public JsonResult GetSalHeadProcessmonth(string data)
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                int Id = int.Parse(data);
                var qurey = db.SalaryHead.Include(e => e.Frequency).Include(e => e.SalHeadOperationType).Include(e => e.ProcessType).Where(e => e.Frequency.LookupVal.ToUpper() == "YEARLY" && e.SalHeadOperationType.LookupVal.ToUpper() != "PERK").Where(e => e.Id == Id).SingleOrDefault();
               
                bool selected = false;
                if (qurey.ProcessType.LookupVal.ToUpper() == "FIXEDMONTH")
                {
                    selected = true;
                }
                return Json(selected, JsonRequestBehavior.AllowGet);
        ...
        public JsonResult GetSalHead(string data)
        {
            ...
            {
                int Id = int.Parse(data);
                var qurey = db.SalaryHead.Include(e => e.Frequency).Include(e => e.SalHeadOperationType).Where(e => e.Frequency.LookupVal.ToUpper() == "YEARLY" && e.SalHeadOperationType.LookupVal.ToUpper() != "PERK").Where(e => e.Id == Id).SingleOrDefault();
                var a = db.SalaryHead.Include(e => e.SalHeadOperationType).Where(e => e.Id == Id).SingleOrDefault();
                bool selected = false;
                if (qurey.SalHeadOperationType.LookupVal.ToUpper() == "LVENCASH")
                {
                    selected = true;
                }
                return Json(selected, JsonRequestBehavior.AllowGet);
        ...
        public class P2bYearlypaymentGridData
        {
            ...
            public string EncashLvhead { get; set; }
        ...
        public ActionResult P2BGridRelease(P2BGrid_Parameters gp)
        {
            try
            {
                ...
                foreach (var z in OEmployee)
                {
                    ...
                    //                    .SingleOrDefault();
                    var ObjYearlyPaymentT = db.EmployeePayroll.Include(q => q.YearlyPaymentT).Include(q => q.YearlyPaymentT.Select(s => s.SalaryHead))
                        .Include(q => q.YearlyPaymentT.Select(x => x.LvEncashReq.LvHead))
                        .Where(e => e.Id == z.Id).SingleOrDefault();

                    //DateTime? Eff_Date = null;
                    //PayScaleAgreement PayScaleAgr = null;
                    foreach (var a in ObjYearlyPaymentT.YearlyPaymentT.Where(e => e.ReleaseFlag == false))
                    {
                        ...
                        view = new P2bYearlypaymentGridData()
                        {
                            ...
                            EncashLvhead = a.LvEncashReq != null && a.LvEncashReq.LvHead != null ? a.LvEncashReq.LvHead.LvName : "",
                ...
                if (!string.IsNullOrEmpty(gp.searchField) && !string.IsNullOrEmpty(gp.searchString))
                {
                    ...
                    if (gp.searchOper.Equals("eq"))
                    {
                        jsonData = IE.Where(e => (e.Employee.EmpCode.ToString().Contains(gp.searchString))
                            ...
                            || (e.EncashLvhead.ToString().Contains(gp.searchString))
                            || (e.Id.ToString().Contains(gp.searchString))
                            ).Select(a => new Object[] { a.Employee.EmpCode, a.Employee.EmpName.FullNameFML, a.SalaryHead.Name, a.FromPeriod, a.ToPeriod, a.AmountPaid, a.OtherDeduction, a.ProcessMonth, a.TDSAmount, a.Narration, a.EncashLvhead, a.Id }).ToList();
                        //jsonData = IE.Select(a => new Object[] { a.Id, Convert.ToString(a.Code), Convert.ToString(a.Name), Convert.ToString(a.BusinessType) != null ? Convert.ToString(a.BusinessType.Id) : "" }).Where(a => a.Contains(Convert.ToInt32(gp.searchString))).ToList();
                    }
                    if (pageIndex > 1)
                    {
                        int h = pageIndex * pageSize;
                        jsonData = IE.Skip(pageIndex).Take(pageSize).Select(a => new Object[] { a.Employee.EmpCode, a.Employee.EmpName.FullNameFML, a.SalaryHead.Name, a.FromPeriod, a.ToPeriod, a.AmountPaid, a.OtherDeduction, a.ProcessMonth, a.TDSAmount, a.Narration, a.EncashLvhead, a.Id }).ToList();
                    }
                ...
                else
                {
                    ...
                    else
                    {
                        orderfuc = (c => gp.sidx == "EmpCode" ? c.Employee.EmpCode.ToString() :
                                         ...
                                         gp.sidx == "EncashLvhead" ? c.EncashLvhead.ToString() : ""
                                          );
                    }
        ...
        public class YearlyPaymentChildDataClass
        {
            ...
            public DateTime? PayMonthdate { get; set; }
        }
                    ...
                    if (db_data.Count > 0)
                    {
                        ...
                        foreach (var item in db_data.SelectMany(e => e.YearlyPaymentT))
                        {
                            returndata.Add(new YearlyPaymentChildDataClass
                            {
                                ...
                                ProcessMonth = item.ProcessMonth != null ? item.ProcessMonth.ToString() : "",
                                ReleaseDate = item.ReleaseDate != null ? item.ReleaseDate.Value.ToShortDateString() : "",
                                ReleaseFlag = item.ReleaseFlag,
                                PayMonthdate = item.PayMonth != null ? Convert.ToDateTime("01/" + item.PayMonth.ToString()) : DateTime.Now,
                            });
                        }
                        var Result = returndata.OrderByDescending(grp => grp.PayMonthdate).ToList();
                        return Json(Result, JsonRequestBehavior.AllowGet);
                    }
                    else
                    {
