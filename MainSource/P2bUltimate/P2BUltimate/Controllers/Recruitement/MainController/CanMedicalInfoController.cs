...
{
    public class CanMedicalInfoController : Controller
    {
        //
        // GET: /CanMedicalInfo/
        ...
        {
            return View("~/Views/Recruitement/MainView/CanMedicalInfo/Index.cshtml");
        }
        ...
        public ActionResult GetAllergyDetails(string data)
        {
            ...
            {
                var fall = db.Allergy.ToList();
                IEnumerable<Allergy> all;
                if (!string.IsNullOrEmpty(data))
                {
                    all = db.Allergy.ToList().Where(d => d.FullDetails.Contains(data));
                }
                else
                {
        ...
        public ActionResult GetDiseaseDetails(string data)
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                var fall = db.Disease.ToList();
                IEnumerable<Disease> all;
                if (!string.IsNullOrEmpty(data))
                {
                    all = db.Disease.ToList().Where(d => d.FullDetails.Contains(data));
                }
                else
                {
                    var r = (from ca in fall select new { srno = ca.Id, lookupvalue = ca.FullDetails }).Distinct();
        ...
        public ActionResult GetEmergencyContactDetails(string data)
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                var fall = db.EmergencyContact.ToList();
                IEnumerable<EmergencyContact> all;
                if (!string.IsNullOrEmpty(data))
                {
                    all = db.EmergencyContact.ToList().Where(d => d.FullDetails.Contains(data));
                }
                else
                {
                    //var r = (from ca in fall select new { srno = ca.Id, lookupvalue = ca.FullDetails }).Distinct();
                    var r = (from ca in fall select new { srno = ca.Id, lookupvalue = ca.Id }).Distinct();
                    return Json(r, JsonRequestBehavior.AllowGet);
                }
                var result = (from c in all
                              select new { c.Id, c.FullDetails }).Distinct();
                    ...
                    if (ModelState.IsValid)
                    {
                        ...
                        using (TransactionScope ts = new TransactionScope())
                        {
                            ...
                            try
                            {
                                ...
                                if (CanData != null)
                                {
                                    CanData.CanMedicalInfo = EmpMedicalInfo;
                                    db.Candidate.Attach(CanData);
                                    db.Entry(CanData).State = System.Data.Entity.EntityState.Modified;
                                    db.SaveChanges();
                                    db.Entry(CanData).State = System.Data.Entity.EntityState.Detached;
                                }
        ...
        public ActionResult Edit(int data)
        {
            ...
            using (DataBaseContext db = new DataBaseContext())
            {
                ...
                var Q = db.Candidate
                    ...
                    .Include(e => e.CanMedicalInfo.Disease.Select(r => r.DiseaseType))
                    .Include(e => e.CanMedicalInfo.Doctor)
                    .Include(e => e.CanMedicalInfo.Doctor.Select(r => r.Name))
                    .Include(e => e.CanMedicalInfo.EmergencyContact)
                     .Where(e => e.Id == data)
                   .Select
                    (e => new
                    {
                        PreferredHospital = e.CanMedicalInfo.PreferredHospital,
                        IDMark = e.CanMedicalInfo.IDMark,
                        Height = e.CanMedicalInfo.Height,
                        Weight = e.CanMedicalInfo.Weight,
                        BloodGroup_Id = e.CanMedicalInfo.BloodGroup.Id == null ? 0 : e.CanMedicalInfo.BloodGroup.Id,
                ...
                var add_data = db.Candidate
                    ...
                    .Include(e => e.CanMedicalInfo.Doctor)
                    .Include(e => e.CanMedicalInfo.Doctor.Select(r => r.Name))
                    .Include(e => e.CanMedicalInfo.Doctor.Select(r => r.Specialization))
                    .Include(e => e.CanMedicalInfo.EmergencyContact)
                    .Include(e => e.CanMedicalInfo.EmergencyContact.Select(r => r.Relation))
                    .Where(e => e.Id == data).ToList();

                var add_data1 = db.Candidate
                 .Include(e => e.CanMedicalInfo)
                 .Include(e => e.CanMedicalInfo.HospitalContactDetails)
                 .Include(e => e.CanMedicalInfo.HospitalAddress)
                 .Include(e => e.CanMedicalInfo.Allergy)
                ...
                foreach (var ca in add_data)
                {
                    ...
                    {
                        Allergy_id = ca.CanMedicalInfo.Allergy.Select(e => e.Id.ToString()).ToArray(),
                        Allergy_val = ca.CanMedicalInfo.Allergy.Select(e => e.FullDetails.ToString()).ToArray(),
                        Disease_id = ca.CanMedicalInfo.Disease.Select(e => e.Id.ToString()).ToArray(),
                        Disease_val = ca.CanMedicalInfo.Disease.Select(e => e.FullDetails.ToString()).ToArray(),
                        Doctor_id = ca.CanMedicalInfo.Doctor.Select(e => e.Id.ToString()).ToArray(),
                        Doctor_val = ca.CanMedicalInfo.Doctor.Select(e => e.FullDetails.ToString()).ToArray(),
                        EMCN_id = ca.CanMedicalInfo.EmergencyContact.Select(e => e.Id.ToString()).ToArray(),
                        EMCN_val = ca.CanMedicalInfo.EmergencyContact.Select(e => e.FullDetails.ToString()).ToArray(),
                        HospitalAddress_Id = ca.CanMedicalInfo.HospitalAddress.Id.ToString(),
                        HospitalAddress_FullAddress = ca.CanMedicalInfo.HospitalAddress.FullAddress,
                        HospitalContactDetails_Id = ca.CanMedicalInfo.HospitalContactDetails.Id.ToString(),
                        FullContactDetails = ca.CanMedicalInfo.HospitalContactDetails.FullContactDetails,
                    });
                }
                ...
                var Corp = db.EmpMedicalInfo.Find(add_data1.CanMedicalInfo.Id);
        ...
        //        db_Data.CanMedicalInfo.Allergy = null;
        //        db_Data.CanMedicalInfo.Disease = null;
        //        db_Data.CanMedicalInfo.Doctor = null;
        //        db_Data.CanMedicalInfo.EmergencyContact = null;
        ...
        //                db_Data.CanMedicalInfo.Allergy = AllergyVal;
        //            }
        //        }
        //        List<Disease> DiseaseVal = new List<Disease>();
        ...
        //                db_Data.CanMedicalInfo.Disease = DiseaseVal;
        //            }
        //        }
        //        List<EmergencyContact> EmergencyContactVal = new List<EmergencyContact>();
        ...
        //                db_Data.CanMedicalInfo.EmergencyContact = EmergencyContactVal;
        //            }
        //        }
        //        List<Doctor> DoctorVal = new List<Doctor>();
        ...
        //                db_Data.CanMedicalInfo.Doctor = DoctorVal;
        //            }
        //        }

        //        var alrgy = db_Data.CanMedicalInfo.Allergy;
        //        var Dises = db_Data.CanMedicalInfo.Disease;
        //        var Dctr = db_Data.CanMedicalInfo.Doctor;
        //        var EmrgContact = db_Data.CanMedicalInfo.EmergencyContact;
        ...
        //                    {
        //                        db.EmpMedicalInfo.Attach(db_Data.CanMedicalInfo);
        //                        db.Entry(db_Data.CanMedicalInfo).State = System.Data.Entity.EntityState.Modified;
        //                        db.SaveChanges();
        //                        TempData["RowVersion"] = db_Data.RowVersion;
        //                        db.Entry(db_Data.CanMedicalInfo).State = System.Data.Entity.EntityState.Detached;

        //                        var Curr_OBJ = db.Candidate.Find(data);
        ...
        //                            {
        //                                blog = context.Candidate.Where(e=>e.CanMedicalInfo.Id == data).Select(e=>e.CanMedicalInfo).SingleOrDefault(); 
        ...
        //                            var eee = db.Candidate.Include(e=>e.CanMedicalInfo)
        //                                .Include(e=>e.CanMedicalInfo.Allergy)
        //                                .Include(e => e.CanMedicalInfo.Disease)
        //                                .Include(e => e.CanMedicalInfo.Doctor)
        //                                .Include(e => e.CanMedicalInfo.EmergencyContact)
        ...
        //                                eee.Id = data;
        //                                eee.CanMedicalInfo.Allergy = alrgy;
        //                                eee.CanMedicalInfo.Disease = Dises;
        //                                eee.CanMedicalInfo.Doctor = Dctr;
        //                                eee.CanMedicalInfo.EmergencyContact = EmrgContact;
        //                                eee.CanMedicalInfo.Height = c.Height;
        //                                eee.CanMedicalInfo.Weight = c.Weight;
        //                                eee.CanMedicalInfo.HospitalAddress = c.HospitalAddress;
        //                                eee.CanMedicalInfo.HospitalContactDetails = c.HospitalContactDetails;
        //                                eee.CanMedicalInfo.PreferredHospital = c.PreferredHospital;
                                        
        //                            };


        //                            db.EmpMedicalInfo.Attach(eee.CanMedicalInfo);
        //                            db.Entry(eee.CanMedicalInfo).State = System.Data.Entity.EntityState.Modified;

        //                            // db.Entry(c.DBTrack).State = System.Data.Entity.EntityState.Modified;
        //                            //db.SaveChanges();
        //                            db.Entry(eee.CanMedicalInfo).OriginalValues["RowVersion"] = TempData["RowVersion"];
        ...
        //                            db.Entry(eee.CanMedicalInfo).State = System.Data.Entity.EntityState.Detached;
        //                            ts.Complete();
        //                            Msg.Add("  Record Updated");
        //                            return Json(new Utility.JsonReturnClass { Id = eee.Id, Val = eee.CanMedicalInfo.PreferredHospital, success = true, responseText = Msg }, JsonRequestBehavior.AllowGet);
        ...
        //                {
        //                    Allergy = db_Data.CanMedicalInfo.Allergy,
        //                    Disease = db_Data.CanMedicalInfo.Disease,
        //                    Doctor = db_Data.CanMedicalInfo.Doctor,
        //                    EmergencyContact = db_Data.CanMedicalInfo.EmergencyContact,
        ...
        //                    Old_OBJ = context.EmpMedicalInfo.Where(e => e.Id == data).Include(e => e.BloodGroup)
        //                        .Include(e => e.HospitalAddress).Include(e => e.HospitalContactDetails).SingleOrDefault();
        //                    DT_EmpMedicalInfo DT_OBJ = (DT_EmpMedicalInfo)obj;
        ...
        //                ts.Complete();
        //                Msg.Add("  Record Updated");
        //                return Json(new Utility.JsonReturnClass { Id = blog.Id, Val = c.PreferredHospital, success = true, responseText = Msg }, JsonRequestBehavior.AllowGet);
        ...
        public async Task<ActionResult> EditSave(EmpMedicalInfo c, int data, FormCollection form) // Edit submit
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                try
                {
                    ...
                    var db_Data = db.Candidate.Include(e => e.CanMedicalInfo)
                        .Include(e => e.CanMedicalInfo.HospitalContactDetails)
                            .Include(e => e.CanMedicalInfo.HospitalAddress)
                        .Include(e => e.CanMedicalInfo.Allergy)
                        .Include(e => e.CanMedicalInfo.Disease)
                    ...
                    int AID = db_Data.CanMedicalInfo.Id;
                    ...
                    db_Data.CanMedicalInfo.Doctor = null;
                    db_Data.CanMedicalInfo.EmergencyContact = null;
                    db_Data.CanMedicalInfo.HospitalContactDetails = null;
                    db_Data.CanMedicalInfo.HospitalAddress = null;
                    List<Allergy> AllergyVal = new List<Allergy>();
                    string AllergyV = form["Alergylist"];
                    if (AllergyV != null)
                    {
                    ...
                    if (Addrs != null)
                    {
                        if (Addrs != "")
                        {
                            ...
                            var val = db.Address.Include(e => e.Area)
                                                ...
                                                .Where(e => e.Id == AddId).SingleOrDefault();
                            c.HospitalAddress = val;
                            db_Data.CanMedicalInfo.HospitalAddress = c.HospitalAddress;
                        }
                    }
                    ...
                    if (HospitalContactDetails != null)
                    {
                        if (HospitalContactDetails != "")
                        {
                            ...
                            var val = db.ContactDetails.Include(e => e.ContactNumbers)
                                                .Where(e => e.Id == ContId).SingleOrDefault();
                            c.HospitalContactDetails = val;
                            db_Data.CanMedicalInfo.HospitalContactDetails = c.HospitalContactDetails;
                        }
                    }
                        ...
                        if (ModelState.IsValid)
                        {
                            try
                            {
                                ...
                                using (TransactionScope ts = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                                {
                                    ...
                                    db.Entry(db_Data.CanMedicalInfo).State = System.Data.Entity.EntityState.Modified;
                                    db.SaveChanges();
                                    TempData["RowVersion"] = db_Data.CanMedicalInfo.RowVersion;
                                    db.Entry(db_Data.CanMedicalInfo).State = System.Data.Entity.EntityState.Detached;
        ...
        public ActionResult P2BGrid(P2BGrid_Parameters gp)
        {
            try
            {
                ...
                if (gp.IsAutho == true)
                {
                    Candidate = db.Candidate.Include(e => e.CanMedicalInfo).Include(e => e.CanName).Where(e => e.DBTrack.IsModified == true).ToList();
                   
                }
                else
                {
                    Candidate = db.Candidate.Include(e => e.CanMedicalInfo).Include(e => e.CanName).ToList();
                    Candidate = db.Candidate.Include(q => q.CanMedicalInfo).Include(q => q.CanName).Where(q => q.CanMedicalInfo != null).ToList();
                }
                ...
                if (!string.IsNullOrEmpty(gp.searchField))
                {
                    ...
                    {
                        jsonData = IE.Select(a => new { a.Id, a.CanName.FullNameFML, a.CanMedicalInfo.Height, a.CanMedicalInfo.Weight }).Where((e => (e.Id.ToString() == gp.searchString) || (e.FullNameFML.ToLower() == gp.searchString.ToLower()) || (e.Height.ToString() == gp.searchString.ToLower()) || (e.Weight.ToString() == gp.searchString.ToLower()))).ToList();
                    ...
                    if (pageIndex > 1)
                    {
                        ...
                        jsonData = IE.Skip(pageIndex).Take(pageSize).Select(a => new Object[] { a.Id, a.CanName.FullNameFML, a.CanMedicalInfo != null ? a.CanMedicalInfo.Height : 0, a.CanMedicalInfo != null ? a.CanMedicalInfo.Weight : 0 }).ToList();
                    }
                ...
                else
                {
                    ...
                    if (gp.sord == "asc")
                    {
                        ...
                        jsonData = IE.Select(a => new Object[] { a.Id, a.CanName.FullNameFML, a.CanMedicalInfo != null ? a.CanMedicalInfo.Height : 0, a.CanMedicalInfo != null ? a.CanMedicalInfo.Weight : 0 }).ToList();
                    }
