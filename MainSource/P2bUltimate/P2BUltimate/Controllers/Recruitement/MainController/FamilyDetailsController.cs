...
namespace P2BUltimate.Controllers.recruitment.MainController
{
    ...
    public class FamilyDetailsController : Controller
    {
        ...
        {
            return View("~/Views/Shared/Recruitement/_FamilyDetails.cshtml");
        }
                ...
                try
                {
                    ...
                    if (ModelState.IsValid)
                    {
                        using (TransactionScope ts = new TransactionScope())
                        {
                            ...
                            var EmpSocialInfo = db.Candidate.Include(e => e.CanFamilyDetails)
                                               .Include(e => e.CanFamilyDetails.Select(lm => lm.ContactDetails))
                                               .Include(e => e.CanFamilyDetails.Select(lm => lm.Address))
                                               .Include(e => e.CanFamilyDetails.Select(lm => lm.Relation))
                                               .Include(e => e.CanFamilyDetails.Select(lm => lm.MemberName))
        ...
        public ActionResult Edit(int data)
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                try
                {
                    ...
                    var EmpSocialInfo = db.FamilyDetails
                      .Include(e => e.ContactDetails)
                      .Include(e => e.Address)
                      .Include(e => e.Relation)
        ...
        public ActionResult GetFamilyDetails(List<int> SkipIds)
        {
            ...
            using (DataBaseContext db = new DataBaseContext())
            {
                ...
                if (SkipIds != null)
                {
                    ...
                    {
                        if (fall == null)
                            fall = db.Candidate.Include(e => e.CanFamilyDetails).Where(e => !e.Id.ToString().Contains(a.ToString())).SelectMany(h => h.CanFamilyDetails).ToList();
                        else
                            fall = fall.Where(e => !e.Id.ToString().Contains(a.ToString())).ToList();
