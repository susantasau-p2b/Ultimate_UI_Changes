...
{
    public class JobAgencyController : Controller
    {
        ...
        {
            return View("~/Views/Recruitement/MainView/JobAgency/Index.cshtml");
        }

        public ActionResult Partial()
        {
            return View("~/Views/Shared/Recruitement/_JobAgency.cshtml");
        }
        ...
        public ActionResult Create(JobAgency c, FormCollection form) //Create submit
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                string AgencyAddr = form["AgencyAddressList"] == "0" ? "" : form["AgencyAddressList"];
                string ContactDetails = form["ContactDetailslistJA"] == "0" ? "" : form["ContactDetailslistJA"];
                //string namesingle = form["ContactPersonlistJA"] == "0" ? "" : form["ContactPersonlistJA"];
                ...
                try
                {
                    ...
                    if (AgencyAddr != null)
                    {
                        if (AgencyAddr != "")
                        {
                            int AddId = Convert.ToInt32(AgencyAddr);
                    ...
                    if (ModelState.IsValid)
                    {
                        using (TransactionScope ts = new TransactionScope())
                        {
                            ...
                            JobAgency corporate = new JobAgency()
                            {
                            ...
                            db.JobAgency.Add(corporate);
                            var rtn_Obj = DBTrackFile.DBTrackSave("Recruitment/Recruitment", null, db.ChangeTracker, c.DBTrack);
                            //DT_JobAgency DT_Corp = (DT_JobAgency)rtn_Obj;
                            //DT_Corp.AgencyAddress_Id = c.AgencyAddress == null ? 0 : c.AgencyAddress.Id;
                            ...
                            {
                                List<JobAgency> JobAgencylist = new List<JobAgency>();
                                JobAgencylist.Add(corporate);
                                companyRecruitment.JobAgency = JobAgencylist;
        ...
        public ActionResult Edit(int data)
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                ...
                var add_data = db.JobAgency
                  .Include(e => e.ContactDetails)
                    .Include(e => e.AgencyAddress)

                    .Include(e => e.ContactPerson)
                    .Include(e => e.AgencyAddress.Area)
                    .Include(e => e.AgencyAddress.City)
                    .Include(e => e.AgencyAddress.Country)
                    .Include(e => e.AgencyAddress.District)
                    .Include(e => e.AgencyAddress.State)
                    .Include(e => e.AgencyAddress.StateRegion)
                    .Include(e => e.AgencyAddress.Taluka)
                    .Where(e => e.Id == data)
                    .Select(e => new
                    {
                        Address_FullAddress = e.AgencyAddress.FullAddress == null ? "" : e.AgencyAddress.FullAddress,
                        Add_Id = e.AgencyAddress.Id == null ? "" : e.AgencyAddress.Id.ToString(),
                ...
                //         Address_Val = e. AgencyAddress_Id== 0 ? "" : db.AgencyAddress.Where(x => x.Id == e.AgencyAddress_Id).Select(x => x.FullAddress).FirstOrDefault(),
                ...
                //           ContactPerson_Val = e.ContactPerson_Id == 0 ? "" : db.ContactPerson.Where(x => x.Id == e.ContactPerson_Id).Select(x => x.FullDetails).FirstOrDefault()
                //     }).OrderByDescending(e => e.DT_Id).FirstOrDefault();

                var Corp = db.JobAgency.Find(data);
        ...
        [HttpPost]
        public async Task<ActionResult> EditSave(JobAgency c, int data, FormCollection form) // Edit submit
        {
            List<String> Msg = new List<String>();
            using (DataBaseContext db = new DataBaseContext())
            {
                try
                {
                    //  string ContPer = form["ContactPersonlistJA"] == "0" ? "" : form["ContactPersonlistJA"];
                    int EmpName_FName = form["EmpName_id"] == "" ? 0 : Convert.ToInt32(form["EmpName_id"]);
                    string Addrs = form["AgencyAddressList"] == "0" ? "" : form["AgencyAddressList"];
                    string ContactDetails = form["ContactDetailslistJA"] == "0" ? "" : form["ContactDetailslistJA"];
                    ...
                    if (Addrs != null)
                    {
                        if (Addrs != "")
                        {
                            ...
                            var add = db.JobAgency.Include(e => e.AgencyAddress).Where(e => e.Id == data).SingleOrDefault();
                            IList<JobAgency> addressdetails = null;
                            if (add.AgencyAddress != null)
                            {
                                addressdetails = db.JobAgency.Where(x => x.AgencyAddress.Id == add.AgencyAddress.Id && x.Id == data).ToList();
                            }
                            else
                            {
                                addressdetails = db.JobAgency.Where(x => x.Id == data).ToList();
                            }
                    ...
                    {
                        var addressdetails = db.JobAgency.Include(e => e.AgencyAddress).Where(x => x.Id == data).ToList();
                        foreach (var s in addressdetails)
                        {
                    ...
                    if (ContactDetails != null)
                    {
                        if (ContactDetails != "")
                        {
                            ...
                            var add = db.JobAgency.Include(e => e.ContactDetails).Where(e => e.Id == data).SingleOrDefault();
                            IList<JobAgency> contactsdetails = null;
                            if (add.ContactDetails != null)
                            {
                                contactsdetails = db.JobAgency.Where(x => x.ContactDetails.Id == add.ContactDetails.Id && x.Id == data).ToList();
                            }
                            else
                            {
                                contactsdetails = db.JobAgency.Where(x => x.Id == data).ToList();
                            }
                    ...
                    {
                        var contactsdetails = db.JobAgency.Include(e => e.ContactDetails).Where(x => x.Id == data).ToList();
                        foreach (var s in contactsdetails)
                        {
                    ...
                    if (EmpName_FName != 0)
                    {
                        ...
                        var add = db.JobAgency.Include(e => e.ContactPerson).Where(e => e.Id == data).SingleOrDefault();
                        IList<JobAgency> ContactPerson = null;
                        if (add.ContactPerson != null)
                        {
                            ContactPerson = db.JobAgency.Where(x => x.ContactPerson.Id == add.ContactPerson.Id && x.Id == data).ToList();
                        }
                        else
                        {
                            ContactPerson = db.JobAgency.Where(x => x.Id == data).ToList();
                        }
                    ...
                    {
                        var ContactPerson = db.JobAgency.Include(e => e.ContactPerson).Where(x => x.Id == data).ToList();
                        foreach (var s in ContactPerson)
                        {
                    ...
                    if (Auth == false)
                    {
                        ...
                        if (ModelState.IsValid)
                        {
                            ...
                            {
                                JobAgency blog = null; // to retrieve old data
                                DbPropertyValues originalBlogValues = null;

                                using (var context = new DataBaseContext())
                                {
                                    blog = context.JobAgency.Where(e => e.Id == data)
                                ...
                                var CurCorp = db.JobAgency.Find(data);
                                TempData["CurrRowVersion"] = CurCorp.RowVersion;
                                db.Entry(CurCorp).State = System.Data.Entity.EntityState.Detached;
                                if (DBTrackFile.ByteArrayCompare((byte[])TempData["CurrRowVersion"], (byte[])TempData["RowVersion"]) == true)
                                {
                                    JobAgency corp = new JobAgency()
                                    {
                                        Name = c.Name,
                                        PANNo = c.PANNo,
                                        Id = data,
                                ...
                                using (var context = new DataBaseContext())
                                {
                                    ...
                                    //DT_JobAgency DT_Corp = (DT_JobAgency)obj;
                                    //DT_Corp.AgencyAddress_Id = blog.AgencyAddress == null ? 0 : blog.AgencyAddress.Id;
                    ...
                    else
                    {
                        using (TransactionScope ts = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                        {
                            ...
                            JobAgency blog = null; // to retrieve old data
                            DbPropertyValues originalBlogValues = null;
                            JobAgency Old_Corp = null;

                            using (var context = new DataBaseContext())
                            {
                                blog = context.JobAgency.Where(e => e.Id == data).SingleOrDefault();
                                originalBlogValues = context.Entry(blog).OriginalValues;
                            }
                            c.DBTrack = new DBTrack
                            {
                            ...
                            using (var context = new DataBaseContext())
                            {
                                var obj = DBTrackFile.ModifiedDataHistory("Recruitement/Recruitement", "M", blog, corp, "Corporate", c.DBTrack);
                                // var obj = DBTrackFile.DBTrackSave("Core/P2b.Global", originalBlogValues, db.ChangeTracker, c.DBTrack);

                                Old_Corp = context.JobAgency.Where(e => e.Id == data)
                                    .Include(e => e.AgencyAddress).Include(e => e.ContactDetails).Include(e => e.ContactPerson).SingleOrDefault();
                                //DT_JobAgency DT_Corp = (DT_JobAgency)obj;
                                //DT_Corp.AgencyAddress_Id = DBTrackFile.ValCompare(Old_Corp.AgencyAddress, c.AgencyAddress);//Old_Corp.Address == c.Address ? 0 : Old_Corp.Address == null && c.Address != null ? c.Address.Id : Old_Corp.Address.Id;
                ...
                catch (DbUpdateConcurrencyException ex)
                {
                    ...
                    var clientValues = (JobAgency)entry.Entity;
                    ...
                    {
                        var databaseValues = (JobAgency)databaseEntry.ToObject();
        ...
        public int EditS(string Addrs, string ContactDetails, string ContPer, int data, JobAgency c, DBTrack dbT)
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                if (Addrs != null)
                ...
                if (ContPer != null)
                {
                    if (ContPer != "")
                    {
                        ...
                        if (add.ContactPerson != null)
                        {
                            contactsdetails = db.JobAgency.Where(x => x.ContactPerson.Id == add.ContactPerson.Id && x.Id == data).ToList();
                        }
                        else
                        {
                            contactsdetails = db.JobAgency.Where(x => x.Id == data).ToList();
                ...
                {
                    var contactsdetails = db.JobAgency.Include(e => e.ContactPerson).Where(x => x.Id == data).ToList();
                    foreach (var s in contactsdetails)
                    {
                        s.ContactPerson = null;
                        db.JobAgency.Attach(s);
        ...
        public async Task<ActionResult> Delete(int data)
        {
            ...
            using (DataBaseContext db = new DataBaseContext())
            {
                try
                {
                    JobAgency corporates = db.JobAgency.Include(e => e.AgencyAddress)
                                                       .Include(e => e.ContactDetails)
                                                       .Include(e => e.ContactPerson).Where(e => e.Id == data).SingleOrDefault();

                    //Address add = corporates.AgencyAddress;
                    ...
                    if (corporates.DBTrack.IsModified == true)
                    {
                        using (TransactionScope ts = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                        {
                            ...
                            //DT_JobAgency DT_Corp = (DT_JobAgency)rtn_Obj;
                            //DT_Corp.AgencyAddress_Id = corporates.AgencyAddress == null ? 0 : corporates.AgencyAddress.Id;
                    ...
                    else
                    {
                        ...
                        using (TransactionScope ts = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                        {
                            ...
                            //DT_JobAgency DT_Corp = (DT_JobAgency)rtn_Obj;
                            //DT_Corp.AgencyAddress_Id = add == null ? 0 : add.Id;
        ...
        public ActionResult GetContactDetLKDetails(List<int> SkipIds)
        {
            ...
            using (DataBaseContext db = new DataBaseContext())
            {
                ...
                var list1 = db.JobAgency.Include(q => q.ContactDetails).ToList().Select(e => e.ContactDetails);
        ...
        public ActionResult GetAddressLKDetails(List<int> SkipIds)
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                ...
                var list1 = db.JobAgency.ToList().Select(e => e.AgencyAddress);
                var list2 = fall.Except(list1);
        ...
        public ActionResult P2BGrid(P2BGrid_Parameters gp)
        {
            ...
            try
            {
                ...
                IEnumerable<JobAgency> JobAgency = null;
                if (gp.IsAutho == true)
                {
                    JobAgency = db.JobAgency.Include(e => e.ContactPerson)
                        .Include(e => e.AgencyAddress)
                        .Include(e => e.AgencyAddress.City)
                        .Include(e => e.ContactDetails)
                        .Include(e => e.ContactPerson)
                ...
                if (!string.IsNullOrEmpty(gp.searchField))
                {
                    ...
                    if (gp.searchOper.Equals("eq"))
                    {
                        jsonData = IE.Where(e => (e.Name.ToUpper().ToString().Contains(gp.searchString.ToUpper()))
                               || (e.ContactPerson != null && e.ContactPerson.FullNameFML != null ? e.ContactPerson.FullNameFML.ToUpper().ToString().Contains(gp.searchString.ToUpper()) : false)
                               || (e.AgencyAddress != null && e.AgencyAddress.City != null ? e.AgencyAddress.City.Name.ToUpper().ToString().Contains(gp.searchString.ToUpper()) : false)
                               || (e.ContactDetails != null ? e.ContactDetails.EmailId.ToUpper().ToString().Contains(gp.searchString.ToUpper()) : false)
                               || (e.Id.ToString().Contains(gp.searchString))
                               ).Select(a => new Object[] { a.Name, a.ContactPerson != null && a.ContactPerson.FullNameFML != null ? a.ContactPerson.FullNameFML : "", a.AgencyAddress != null ? a.AgencyAddress.City.Name : "", a.ContactDetails != null ? a.ContactDetails.EmailId : "", a.Id }).ToList();
                    ...
                    if (pageIndex > 1)
                    {
                        ...
                        jsonData = IE.Skip(pageIndex).Take(pageSize).Select(a => new Object[] { a.Name, a.ContactPerson != null && a.ContactPerson.FullNameFML != null ? a.ContactPerson.FullNameFML : "", a.AgencyAddress == null ? "" : a.AgencyAddress.City.Name, a.ContactDetails == null ? "" : a.ContactDetails.EmailId, a.Id }).ToList();
                    }
                ...
                {
                    IE = JobAgency;
                    Func<JobAgency, dynamic> orderfuc;
                    if (gp.sidx == "Id")
                    {
                    ...
                    else
                    {
                        orderfuc = (c => gp.sidx == "Name" ? c.Name :
                                         ...
                                         gp.sidx == "City" ? c.AgencyAddress == null ? "" : c.AgencyAddress.City == null ? "" : c.AgencyAddress.City.Name :
                                         gp.sidx == "Email" ? c.ContactDetails == null ? "" : c.ContactDetails.EmailId : "");
                    }
                    if (gp.sord == "asc")
                    {
                        IE = IE.OrderBy(orderfuc);
                        jsonData = IE.Select(a => new Object[] { a.Name, a.ContactPerson != null && a.ContactPerson.FullNameFML != null ? a.ContactPerson.FullNameFML : "", a.AgencyAddress == null ? "" : a.AgencyAddress.City == null ? "" : a.AgencyAddress.City.Name, a.ContactDetails == null ? "" : a.ContactDetails.EmailId, a.Id }).ToList();
                    }
                    else if (gp.sord == "desc")
                    {
                        IE = IE.OrderByDescending(orderfuc);
                        jsonData = IE.Select(a => new Object[] { Convert.ToString(a.Name), a.ContactPerson != null && a.ContactPerson.FullNameFML != null ? a.ContactPerson.FullNameFML : "", a.AgencyAddress == null ? "" : a.AgencyAddress.City == null ? "" : a.AgencyAddress.City.Name, a.ContactDetails == null ? "" : a.ContactDetails.EmailId, a.Id }).ToList();
                    }
                    if (pageIndex > 1)
                    {
                        int h = pageIndex * pageSize;
                        jsonData = IE.Skip(pageIndex).Take(pageSize).Select(a => new Object[] { a.Name, a.ContactPerson != null && a.ContactPerson.FullNameFML != null ? a.ContactPerson.FullNameFML : "", a.AgencyAddress == null ? "" : a.AgencyAddress.City == null ? "" : a.AgencyAddress.City.Name, a.ContactDetails == null ? "" : a.ContactDetails.EmailId, a.Id }).ToList();
                    }
                    totalRecords = JobAgency.Count();
                }
                if (totalRecords > 0)
                {
        ...
        public ActionResult GetLookupJobAgency_EmpName()
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                var rall = db.JobAgency.Select(p => new
                {
                    ContactPersonId = p.ContactPerson.Id
                }).ToList();
                var JobAgencyEmpNameId = rall.Select(w => w.ContactPersonId).ToList();
                var lall = db.NameSingle.Select(q => new
                {
                    FullNameFML = q.FullNameFML,
                    NameSingleId = q.Id
                }).Where(e => !JobAgencyEmpNameId.Contains(e.NameSingleId)).ToList();
