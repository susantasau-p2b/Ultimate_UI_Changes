...
namespace P2BUltimate.Controllers.Recruitment.MainController
{
    public class JobPortalController : Controller
    {
        ...
        {
            return View("~/Views/Recruitement/MainView/JobPortal/Index.cshtml");
        }

        public ActionResult Partial()
        {
            return View("~/Views/Shared/Recruitement/_JobPortal.cshtml");
        }


        [HttpPost]
        // [ValidateAntiForgeryToken]
        public ActionResult Create(JobPortal c, FormCollection form) //Create submit
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                string PortalAddr = form["PortalAddressList"] == "0" ? "" : form["PortalAddressList"];
                string ContactDetails = form["ContactDetailslistJP"] == "0" ? "" : form["ContactDetailslistJP"];
                //string namesingle = form["ContactPersonlistJP"] == "0" ? "" : form["ContactPersonlistJP"];
                ...
                try
                {
                    ...
                    if (ModelState.IsValid)
                    {
                        using (TransactionScope ts = new TransactionScope())
                        {
                            ...
                            JobPortal corporate = new JobPortal()
                            {
                            ...
                            {
                                List<JobPortal> JobPortallist = new List<JobPortal>();
                                JobPortallist.Add(corporate);
                                companyRecruitment.JobPortal = JobPortallist;
        ...
        public ActionResult Edit(int data)
        {
            ...
            using (DataBaseContext db = new DataBaseContext())
            {
                ...
                var add_data = db.JobPortal
                  .Include(e => e.ContactDetails)
                    ...
                    .Select(e => new
                    {
                        ...
                        FullContactersonDetail = e.ContactPerson.FullDetails == null ? "" : e.ContactPerson.FullDetails
                    }).ToList();


                //var W = db.DT_JobPortal
                ...
                //         Portal_Address_Val = e.PortalAddress_Id == 0 ? "" : db.Address.Where(x => x.Id == e.PortalAddress_Id).Select(x => x.FullAddress).FirstOrDefault(),
                ...
                var Corp = db.JobPortal.Find(data);
        ...
        [HttpPost]
        public async Task<ActionResult> EditSave(JobPortal c, int data, FormCollection form) // Edit submit
        {
            List<String> Msg = new List<String>();
            using (DataBaseContext db = new DataBaseContext())
            {
                try
                {
                    string ContPer = form["ContactPersonlistJP"] == "0" ? "" : form["ContactPersonlistJP"];
                    ...
                    if (Addrs != null)
                    {
                        if (Addrs != "")
                        {
                            ...
                            var add = db.JobPortal.Include(e => e.PortalAddress).Where(e => e.Id == data).SingleOrDefault();
                            IList<JobPortal> addressdetails = null;
                            if (add.PortalAddress != null)
                            {
                                addressdetails = db.JobPortal.Where(x => x.PortalAddress.Id == add.PortalAddress.Id && x.Id == data).ToList();
                            }
                            else
                            {
                                addressdetails = db.JobPortal.Where(x => x.Id == data).ToList();
                            }
                    ...
                    {
                        var addressdetails = db.JobPortal.Include(e => e.PortalAddress).Where(x => x.Id == data).ToList();
                        foreach (var s in addressdetails)
                        {
                    ...
                    if (ContactDetails != null)
                    {
                        if (ContactDetails != "")
                        {
                            ...
                            var add = db.JobPortal.Include(e => e.ContactDetails).Where(e => e.Id == data).SingleOrDefault();
                            IList<JobPortal> contactsdetails = null;
                            if (add.ContactDetails != null)
                            {
                                contactsdetails = db.JobPortal.Where(x => x.ContactDetails.Id == add.ContactDetails.Id && x.Id == data).ToList();
                            }
                            else
                            {
                                contactsdetails = db.JobPortal.Where(x => x.Id == data).ToList();
                            }
                    ...
                    {
                        var contactsdetails = db.JobPortal.Include(e => e.ContactDetails).Where(x => x.Id == data).ToList();
                        foreach (var s in contactsdetails)
                        {
                    ...
                    if (EmpName_FName != null)
                    {
                            ...
                            var add = db.JobPortal.Include(e => e.ContactPerson).Where(e => e.Id == data).SingleOrDefault();
                            IList<JobPortal> contactsdetails = null;
                            if (add.ContactPerson != null)
                            {
                                contactsdetails = db.JobPortal.Where(x => x.ContactPerson.Id == add.ContactPerson.Id && x.Id == data).ToList();
                            }
                            else
                            {
                                contactsdetails = db.JobPortal.Where(x => x.Id == data).ToList();
                    ...
                    {
                        var contactsdetails = db.JobPortal.Include(e => e.ContactPerson).Where(x => x.Id == data).ToList();
                        foreach (var s in contactsdetails)
                        {
                    ...
                    if (Auth == false)
                    {
                        ...
                        if (ModelState.IsValid)
                        {
                            ...
                            {
                                JobPortal blog = null; // to retrieve old data
                                DbPropertyValues originalBlogValues = null;

                                using (var context = new DataBaseContext())
                                {
                                    blog = context.JobPortal.Where(e => e.Id == data)
                               ...
                               // int a = EditS(Addrs, ContactDetails, ContPer, data, c, c.DBTrack);
                                ...
                                var CurCorp = db.JobPortal.Find(data);
                                TempData["CurrRowVersion"] = CurCorp.RowVersion;
                                db.Entry(CurCorp).State = System.Data.Entity.EntityState.Detached;
                                if (DBTrackFile.ByteArrayCompare((byte[])TempData["CurrRowVersion"], (byte[])TempData["RowVersion"]) == true)
                                {
                                    JobPortal corp = new JobPortal()
                                    {
                                        Name = c.Name,
                                        PANNo = c.PANNo,
                                        Id = data,
                                ...
                                using (var context = new DataBaseContext())
                                {
                                    ...
                                    //DT_JobPortal DT_Corp = (DT_JobPortal)obj;
                    ...
                    else
                    {
                        using (TransactionScope ts = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                        {
                            ...
                            JobPortal blog = null; // to retrieve old data
                            DbPropertyValues originalBlogValues = null;
                            JobPortal Old_Corp = null;

                            using (var context = new DataBaseContext())
                            {
                                blog = context.JobPortal.Where(e => e.Id == data).SingleOrDefault();
                                originalBlogValues = context.Entry(blog).OriginalValues;
                            }
                            c.DBTrack = new DBTrack
                            {
                            ...
                            using (var context = new DataBaseContext())
                            {
                                var obj = DBTrackFile.ModifiedDataHistory("Recruitement/Recruitement", "M", blog, corp, "JobPortal", c.DBTrack);
                                // var obj = DBTrackFile.DBTrackSave("Core/P2b.Global", originalBlogValues, db.ChangeTracker, c.DBTrack);

                                Old_Corp = context.JobPortal.Where(e => e.Id == data)
                ...
                catch (DbUpdateConcurrencyException ex)
                {
                    ...
                    var clientValues = (JobPortal)entry.Entity;
                    ...
                    {
                        var databaseValues = (JobPortal)databaseEntry.ToObject();
        ...
        public int EditS(string Addrs, string ContactDetails, string ContPer, int data, JobPortal c, DBTrack dbT)
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                if (Addrs != null)
        ...
        public ActionResult P2BGrid(P2BGrid_Parameters gp)
        {
            ...
            try
            {
                ...
                IEnumerable<JobPortal> JobPortal = null;
                if (gp.IsAutho == true)
                {
                    JobPortal = db.JobPortal.Include(e => e.ContactPerson)
                        .Include(e => e.PortalAddress)
                        .Include(e => e.PortalAddress.City)
                        .Include(e => e.ContactDetails)
                ...
                if (!string.IsNullOrEmpty(gp.searchField))
                {
                    ...
                    if (gp.searchOper.Equals("eq"))
                    {
                         jsonData = IE.Where(e => (e.Name.ToUpper().ToString().Contains(gp.searchString.ToUpper()))
                               ...
                               || (e.PortalAddress != null ? e.PortalAddress.City.Name.ToUpper().ToString().Contains(gp.searchString.ToUpper()) : false)
                               || (e.ContactDetails != null ? e.ContactDetails.EmailId.ToUpper().ToString().Contains(gp.searchString.ToUpper()) : false)
                               || (e.Id.ToString().Contains(gp.searchString))
                               ).Select(a => new Object[] { a.Name, a.ContactPerson != null ? a.ContactPerson.FullNameFML : "", a.PortalAddress != null ? a.PortalAddress.City.Name : "", a.ContactDetails != null ? a.ContactDetails.EmailId : "", a.Id }).ToList();
                    ...
                    if (pageIndex > 1)
                    {
                        ...
                        jsonData = IE.Skip(pageIndex).Take(pageSize).Select(a => new Object[] { a.Name, a.ContactPerson.FullNameFML, a.PortalAddress == null ? "" : a.PortalAddress.City == null ? "" : a.PortalAddress.City.Name, a.ContactDetails == null ? "" : a.ContactDetails.EmailId, a.Id }).ToList();
                    }
                ...
                {
                    IE = JobPortal;
                    Func<JobPortal, dynamic> orderfuc;
                    if (gp.sidx == "Id")
                    {
        ...
        public async Task<ActionResult> Delete(int data)
        {
            ...
            using (DataBaseContext db = new DataBaseContext())
            {
                try
                {
                    JobPortal corporates = db.JobPortal.Include(e => e.PortalAddress)
                                                       .Include(e => e.ContactDetails)
                    ...
                    if (corporates.DBTrack.IsModified == true)
                    {
                        using (TransactionScope ts = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                        {
                            ...
                            var rtn_Obj = DBTrackFile.DBTrackSave("Recruitement/Recruitement", null, db.ChangeTracker, corporates.DBTrack);
                            DT_JobPortal DT_Corp = (DT_JobPortal)rtn_Obj;
        ...
        public ActionResult GetContactDetLKDetails(List<int> SkipIds)
        {
            ...
            using (DataBaseContext db = new DataBaseContext())
            {
                ...
                var list1 = db.JobPortal.ToList().Select(e => e.ContactDetails);
        ...
        public ActionResult GetAddressLKDetails(List<int> SkipIds)
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                ...
                var list1 = db.JobPortal.ToList().Select(e => e.PortalAddress);
                var list2 = fall.Except(list1);
        ...
        public ActionResult GetLookupJobPortal_Name()
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                
                var rall = db.JobPortal.Select(p => new
                {
                   ContactPersonId= p.ContactPerson.Id
                }).ToList();
                var JobPortalEmpNameId=rall.Select(w=>w.ContactPersonId).ToList();
                var lall = db.NameSingle.Select(q => new
                {
                    FullNameFML = q.FullNameFML,
                    NameSingleId = q.Id
                }).Where(e => !JobPortalEmpNameId.Contains(e.NameSingleId)).ToList();
