...
namespace P2BUltimate.Controllers.Recruitement.MainController
{
    ...
    public class PostDetailsController : Controller
    {
        ...
        {
            return View("~/Views/Recruitement/MainView/PostDetails/Index.cshtml");
        }


        [HttpPost]
        // [ValidateAntiForgeryToken]
        public ActionResult Create(PostDetails p, FormCollection form) //Create submit
        {
                ...
                try
                {
                    ...
                    if (ModelState.IsValid)
                    {
                        using (TransactionScope ts = new TransactionScope())
                        {
                            ...
                            //db.Create(DT_Post);
                            db.SaveChanges();
                            postdata.Add(db.PostDetails.Find(Postdetails.Id));
                            ...
                            var companyRecruitment = db.CompanyRecruitment
                                .Include(e => e.RecruitYearlyCalendar)
                                .Include(e => e.RecruitYearlyCalendar.Select(t => t.RecruitmentCalendar))
                                .Include(e => e.RecruitYearlyCalendar.Select(t => t.PostDetails))
                                .Where(e => e.Company.Id == compid).SingleOrDefault();

                            Calendar caldata = db.Calendar.Include(e => e.Name).Where(e => e.Name.LookupVal.ToString().ToUpper() == "RECRUITMENTCALENDAR" && e.Default == true).FirstOrDefault();
                            RecruitYearlyCalendar recruitdata = companyRecruitment.RecruitYearlyCalendar.Where(e => e.RecruitmentCalendar.Id == caldata.Id).SingleOrDefault();
                            if (recruitdata == null)
                            {
                                RecruitYearlyCalendar Recruitcalendar = new RecruitYearlyCalendar()
                                {
                                    RecruitmentCalendar = caldata,
                                    DBTrack = p.DBTrack,
                                ...
                                db.SaveChanges();
                                RecruitYearlyCalendarlst.Add(db.RecruitYearlyCalendar.Find(Recruitcalendar.Id));
                            }
                            else
                            {
                                if (recruitdata.PostDetails != null)
                                {
                                    postdata.AddRange(recruitdata.PostDetails);
                                }
                                recruitdata.PostDetails = postdata;
                                db.Entry(recruitdata).State = System.Data.Entity.EntityState.Modified;
                                db.SaveChanges();
                                db.Entry(recruitdata).State = System.Data.Entity.EntityState.Detached;
                            }

                            if (companyRecruitment.RecruitYearlyCalendar != null)
                            {
                                RecruitYearlyCalendarlst.AddRange(companyRecruitment.RecruitYearlyCalendar);
                            }
        ...
        public ActionResult Edit(int data)
        {
            ...
            using (DataBaseContext db = new DataBaseContext())
            {
                var Q = db.PostDetails.Include(e => e.FuncStruct)
                ...
                var add_data = db.PostDetails.Include(e => e.FuncStruct)
                                                       .Include(e => e.FuncStruct.JobPosition)
                                                       .Include(e => e.FuncStruct.Job)
                                                       .Include(e => e.ExpFilter)
                                                       .Include(e => e.RangeFilter)
                ...
                var Corp = db.PostDetails.Find(data);
        ...
        public ActionResult P2BGrid(P2BGrid_Parameters gp)
        {
            ...
            try
            {
                ...
                if (gp.IsAutho == true)
                {
                    PostDetails = db.PostDetails.Include(e => e.FuncStruct).Include(e => e.FuncStruct.JobPosition)
                                                .Include(e => e.FuncStruct.Job)
                                                  .Include(e => e.ExpFilter)
                                                  .Include(e => e.RangeFilter)
                                                  .Include(e => e.Qualification)
                                                  ...
                                                  .Include(e => e.MaritalStatus)
                                                  .Include(e => e.CategoryPost)
                                                  .Include(e => e.CategorySplPost).AsNoTracking().ToList();
                }
                else
                {
                    PostDetails = db.PostDetails.Include(e => e.FuncStruct).Include(e => e.FuncStruct.JobPosition)
                ...
                if (!string.IsNullOrEmpty(gp.searchField) && !string.IsNullOrEmpty(gp.searchString))
                {
                    ...
                    if (gp.searchOper.Equals("eq"))
                    {
                        jsonData = IE.Where(e => (e.PostCode.ToString().Contains(gp.searchString))
                                || (e.FuncStruct != null ? e.FuncStruct.Job.Name.ToUpper().ToString().Contains(gp.searchString.ToUpper()) : false)
                                || (e.RequestVacancies.ToString().Contains(gp.searchString))
                                || (e.Id.ToString().Contains(gp.searchString))
                                ).Select(a => new Object[] { a.PostCode, a.FuncStruct == null ? "" : a.FuncStruct.Job == null ? "" : a.FuncStruct.Job.Name, a.RequestVacancies, a.Id }).ToList();
                    ...
                    {
                        int h = pageIndex * pageSize;
                        jsonData = IE.Skip(pageIndex).Take(pageSize).Select(a => new Object[] { a.PostCode, a.FuncStruct == null ? "" : a.FuncStruct.Job == null ? "" : a.FuncStruct.Job.Name, a.RequestVacancies, a.Id }).ToList();
                    }
                    totalRecords = IE.Count();
                }
                else
                {
                    ...
                    {
                        orderfuc = (c => gp.sidx == "PostCode" ? c.PostCode.ToString() :
                                         gp.sidx == "Jobdescription" ? c.FuncStruct == null ? "" : c.FuncStruct.JobPosition == null ? "" : c.FuncStruct == null ? "" : c.FuncStruct.Job == null ? "" : c.FuncStruct.Job.Name :
                    ...
                    {
                        IE = IE.OrderBy(orderfuc);
                        jsonData = IE.Select(a => new Object[] { a.PostCode, a.FuncStruct == null ? "" : a.FuncStruct.Job == null ? "" : a.FuncStruct.Job.Name,/* a.FuncStruct.JobPosition.JobPositionDesc == null ? "0" : Convert.ToString(a.FuncStruct.JobPosition.JobPositionDesc)*/a.RequestVacancies, a.Id }).ToList();
                    }
                    else if (gp.sord == "desc")
                    {
                        IE = IE.OrderByDescending(orderfuc);
                    ...
                    {
                        int h = pageIndex * pageSize;
                        jsonData = IE.Skip(pageIndex).Take(pageSize).Select(a => new Object[] { /* a.FuncStruct.JobPosition.JobPositionDesc == null ? "0" : Convert.ToString(a.FuncStruct.JobPosition.JobPositionDesc)*/a.Narration == null ? "" : Convert.ToString(a.Narration), a.Id }).ToList();
                    }
                    totalRecords = PostDetails.Count();
                }
                if (totalRecords > 0)
        ...
        public async Task<ActionResult> Delete(int data)
        {
            ...
            using (DataBaseContext db = new DataBaseContext())
            {
                try
                {
                    PostDetails Postdetails = db.PostDetails.Include(e => e.FuncStruct.JobPosition)
                                                       .Include(e => e.FuncStruct.Job)
                                                       .Include(e => e.ExpFilter)
                                                       .Include(e => e.RangeFilter)
                                                       .Include(e => e.Qualification)
                    ...
                    else
                    {
                        ...
                        using (TransactionScope ts = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                        {
                            ...
                            if (Postdetails.Skill.Count() > 0 || Postdetails.Qualification.Count() > 0 || Postdetails.CategoryPost.Count() > 0 || Postdetails.CategorySplPost.Count() > 0)
                            {
        ...
        [HttpPost]
        public async Task<ActionResult> EditSave(PostDetails L, int data, FormCollection form) // Edit submit
        {
            List<string> Msg = new List<string>();
            using (DataBaseContext db = new DataBaseContext())
                try
                {
                    ...
                    string Gender = form["Gender_drop"] == "0" ? "" : form["Gender_drop"];
                    string MStatus = form["MaritalStatus_drop"] == "0" ? "" : form["MaritalStatus_drop"];
                    var blog1 = db.PostDetails.Where(e => e.Id == data).Include(e => e.FuncStruct)
                                                    .Include(e => e.FuncStruct.JobPosition)
                                                    .Include(e => e.FuncStruct.Job)
                                                    .Include(e => e.ExpFilter)
                                                    .Include(e => e.RangeFilter)
                    ...
                    PostDetails pd = null;
                    pd = db.PostDetails.Include(e => e.Skill).Where(e => e.Id == data).SingleOrDefault();
                    ...
                    PostDetails pd1 = null;
                    pd1 = db.PostDetails.Include(e => e.Qualification).Where(e => e.Id == data).SingleOrDefault();
                            ...
                            //  using (DataBaseContext db = new DataBaseContext())
                            {
                                ...
                                using (TransactionScope ts = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                                {
                                    ...
                                    if (ExpFilter_Id != null)
                                    {
                                        ...
                                        {
                                            var WFTypeDetails = db.PostDetails.Include(e => e.ExpFilter).Where(x => x.Id == data).ToList();
                                    ...
                                    {
                                        var CreditdateypeDetails = db.PostDetails.Include(e => e.ExpFilter).Where(x => x.Id == data).ToList();
                                    ...
                                    if (RangeFilter_Id != null)
                                    {
                                        ...
                                        {
                                            var WFTypeDetails = db.PostDetails.Include(e => e.RangeFilter).Where(x => x.Id == data).ToList();
                                            foreach (var s in WFTypeDetails)
                                            {
                                                s.RangeFilter = null;
                                                db.PostDetails.Attach(s);
                                    ...
                                    {
                                        var CreditdateypeDetails = db.PostDetails.Include(e => e.RangeFilter).Where(x => x.Id == data).ToList();
                                        foreach (var s in CreditdateypeDetails)
                                        {
                                            s.RangeFilter = null;
                                            db.PostDetails.Attach(s);
                                    ...
                                    if (Gender != null)
                                    {
                                        ...
                                        {
                                            var WFTypeDetails = db.PostDetails.Include(e => e.Gender).Where(x => x.Id == data).ToList();
                                            foreach (var s in WFTypeDetails)
                                            {
                                                s.Gender = null;
                                                db.PostDetails.Attach(s);
                                    ...
                                    {
                                        var CreditdateypeDetails = db.PostDetails.Include(e => e.Gender).Where(x => x.Id == data).ToList();
                                        foreach (var s in CreditdateypeDetails)
                                        {
                                            s.Gender = null;
                                            db.PostDetails.Attach(s);
                                    ...
                                    if (MStatus != null)
                                    {
                                        ...
                                        {
                                            var WFTypeDetails = db.PostDetails.Include(e => e.MaritalStatus).Where(x => x.Id == data).ToList();
                                            foreach (var s in WFTypeDetails)
                                            {
                                                s.MaritalStatus = null;
                                                db.PostDetails.Attach(s);
                                    ...
                                    {
                                        var CreditdateypeDetails = db.PostDetails.Include(e => e.MaritalStatus).Where(x => x.Id == data).ToList();
                                        foreach (var s in CreditdateypeDetails)
                                        {
                                            s.MaritalStatus = null;
                                            db.PostDetails.Attach(s);
                                    ...
                                    if (JobPosition != null)
                                    {
                                        if (JobPosition != "")
                                        {
                                            ...
                                            var type = db.PostDetails.Include(e => e.FuncStruct).Include(e => e.FuncStruct.Job).Include(e => e.FuncStruct.JobPosition).Where(e => e.Id == data).SingleOrDefault();
                                            IList<PostDetails> typedetails = null;
                                            if (type.FuncStruct != null)
                                            {
                                                typedetails = db.PostDetails.Where(x => x.FuncStruct.Id == type.FuncStruct.Id && x.Id == data).ToList();
                                        ...
                                        {
                                            var WFTypeDetails = db.PostDetails.Include(e => e.FuncStruct).Include(e => e.FuncStruct.Job).Include(e => e.FuncStruct.JobPosition).Where(x => x.Id == data).ToList();
                                            foreach (var s in WFTypeDetails)
                                            {
                                                s.FuncStruct = null;
                                                db.PostDetails.Attach(s);
                                    ...
                                    {
                                        var CreditdateypeDetails = db.PostDetails.Include(e => e.FuncStruct).Include(e => e.FuncStruct.Job).Include(e => e.FuncStruct.JobPosition).Where(x => x.Id == data).ToList();
                                        foreach (var s in CreditdateypeDetails)
                                        {
                                            s.FuncStruct = null;
                                            db.PostDetails.Attach(s);
                                    ...
                                    if (DBTrackFile.ByteArrayCompare((byte[])TempData["CurrRowVersion"], (byte[])TempData["RowVersion"]) == true)
                                    {
                                        ...
                                        PostDetails post = new PostDetails()
                                        {
                                            //NoOfVacancies = blog1.NoOfVacancies,
                                            ...
                                            CreationDate = blog1.CreationDate,
        ...
        public ActionResult GetCalendarDetails(string data)
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                var fall = db.Calendar.Include(e => e.Name).Where(e => e.Name.LookupVal.ToUpper() == "RECRUITMENTCALENDAR").ToList();
