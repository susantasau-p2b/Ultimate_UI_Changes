...
{
    [AuthoriseManger]
    public class PreviousCompanyExpController : Controller
    {
        //
        // GET: /PreCompanyExp/
        public ActionResult Partial()
        {
            return View("~/Views/Shared/Recruitement/_PreviousCompanyExp.cshtml");
                    ...
                    if (ModelState.IsValid)
                    {
                        using (TransactionScope ts = new TransactionScope())
                        {
                            ...
                            try
                            {
                                ...
                                return Json(new Utility.JsonReturnClass { Id = pce.Id, Val = pce.FullDetails, success = true, responseText = Msg }, JsonRequestBehavior.AllowGet);
        ...
        public ActionResult Edit(int data)
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                ...
                var prevcomp = db.PrevCompExp.Include(e => e.CompAddress)
                    .Include(e => e.ContactDetails).Where(e => e.Id == data)
                                    .SingleOrDefault();
                int AID = prevcomp.Id;
                        ...
                        if (ModelState.IsValid)
                        {
                            try
                            {
                                ...
                                using (TransactionScope ts = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                                {
                                    ...
                                    var prevcomp1 = db.PrevCompExp.Include(e => e.CompAddress)
                                 .Include(e => e.ContactDetails)
                                                 .Where(e => e.Id == data).SingleOrDefault();
                                    ts.Complete();

                                    Msg.Add("  Record Updated");
                                    return Json(new Utility.JsonReturnClass { Id = prevcomp1.Id, Val = prevcomp1.FullDetails, success = true, responseText = Msg }, JsonRequestBehavior.AllowGet);
                                }
                            }
        ...
        public ActionResult GetPrevCompanyExp(List<int> SkipIds)
        {
            List<string> Msg = new List<string>();
            using (DataBaseContext db = new DataBaseContext())
            {
                var fall = new List<PrevCompExp>();
                if (SkipIds != null)
                {
                    foreach (var a in SkipIds)
                    {
                        if (fall == null)
                            fall = db.Candidate.Include(e => e.CanPreCompExp).Where(e => !e.Id.ToString().Contains(a.ToString())).SelectMany(h => h.CanPreCompExp).ToList();
                        else
                            fall = fall.Where(e => !e.Id.ToString().Contains(a.ToString())).ToList();

                    }
                }
                var list1 = db.PrevCompExp.ToList();
