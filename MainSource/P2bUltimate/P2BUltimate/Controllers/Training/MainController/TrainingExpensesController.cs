...
{
    public class TrainingExpensesController : Controller
    {
        ...
        {
            return View("~/Views/Training/MainViews/TrainingExpenses/Index.cshtml");
        }
        ...
        public ActionResult Create(TrainingExpenses COBJ, FormCollection form)
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                ...
                try
                {
                    ...
                    if (ModelState.IsValid)
                    {
                        ...
                        {
                            //if (db.TrainingExpenses.Any(o => o.MiscFees == COBJ.MiscFees) && db.TrainingExpenses.Any(o => o.FoodFees == COBJ.FoodFees))
                            //{
                            ...
                            TrainingExpenses TrainingExpenses = new TrainingExpenses()
                            {
                                ...
                                FoodFees = COBJ.FoodFees,
                                MiscFees = COBJ.MiscFees,
                                StayFees = COBJ.StayFees,
                                TrainingFees = COBJ.TrainingFees,
                                TravelFees = COBJ.TravelFees,
                                DBTrack = COBJ.DBTrack
                            };
                            try
                            {
                                db.TrainingExpenses.Add(TrainingExpenses);
                                ...
                                return Json(new Utility.JsonReturnClass { Id = TrainingExpenses.Id, Val = TrainingExpenses.FullDetails, success = true, responseText = Msg }, JsonRequestBehavior.AllowGet);
        ...
        public ActionResult Edit(int data)
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                ...
                var TrainingExpenses = db.TrainingExpenses
                ...
                var r = (from ca in TrainingExpenses
                         select new
                         {
                             ...
                             Id = ca.Id,
                             FoodFees = ca.FoodFees,
                             MiscFees = ca.MiscFees,
                             StayFees = ca.StayFees,
                             TrainingFees = ca.TrainingFees,
                             TravelFees = ca.TravelFees,
                ...
                var LKup = db.TrainingExpenses.Find(data);
        ...
        //[HttpPost]
        ////[ValidateAntiForgeryToken]
        //public async Task<ActionResult> EditSave(TrainingExpenses ESOBJ, FormCollection form, int data)
        //{
        //    using (DataBaseContext db = new DataBaseContext())
        //    {
        //        List<string> Msg = new List<string>();
        ...
        //                        {
        //                            TrainingExpenses blog = null; // to retrieve old data
        //                            DbPropertyValues originalBlogValues = null;

        //                            using (var context = new DataBaseContext())
        //                            {
        //                                blog = context.TrainingExpenses.Where(e => e.Id == data)
        //                                                        .SingleOrDefault();
        ...
        //                                DT_TrainingExpenses DT_OBJ = (DT_TrainingExpenses)obj;
        //                                db.Create(DT_OBJ);
        //                                db.SaveChanges();
        //                            }
        ...
        //                            return Json(new Utility.JsonReturnClass { Id = ESOBJ.Id, Val = ESOBJ.FoodFees.ToString(), success = true, responseText = Msg }, JsonRequestBehavior.AllowGet);
        ...
        //                    TrainingExpenses blog = null; // to retrieve old data
        //                    DbPropertyValues originalBlogValues = null;
        //                    TrainingExpenses Old_Obj = null;

        //                    using (var context = new DataBaseContext())
        //                    {
        //                        blog = context.TrainingExpenses.Where(e => e.Id == data).SingleOrDefault();
        //                        originalBlogValues = context.Entry(blog).OriginalValues;
        //                    }
        //                    ESOBJ.DBTrack = new DBTrack
        //                    {
        ...
        //                        ModifiedOn = DateTime.Now
        //                    };
        //                    TrainingExpenses corp = new TrainingExpenses()
        //                    {
        ...
        //                    using (var context = new DataBaseContext())
        //                    {
        //                        var obj = DBTrackFile.ModifiedDataHistory("Training/Training", "M", blog, corp, "TrainingExpenses", ESOBJ.DBTrack);
        //                        // var obj = DBTrackFile.DBTrackSave("Core/P2b.Global", originalBlogValues, db.ChangeTracker, c.DBTrack);

        //                        Old_Obj = context.TrainingExpenses.Where(e => e.Id == data)
        //                            .SingleOrDefault();
        //                        DT_TrainingExpenses DT_Corp = (DT_TrainingExpenses)obj;
        //                        db.Create(DT_Corp);
        //                        //db.SaveChanges();
        //                    }
        //                    blog.DBTrack = ESOBJ.DBTrack;
        //                    db.TrainingExpenses.Attach(blog);
        ...
        //                    Msg.Add("  Record Updated");
        //                    return Json(new Utility.JsonReturnClass { Id = blog.Id, Val = ESOBJ.FoodFees.ToString(), success = true, responseText = Msg }, JsonRequestBehavior.AllowGet);
        ...
        [HttpPost]
        public async Task<ActionResult> EditSave(TrainingExpenses L, int data, FormCollection form) // Edit submit
        {
            List<string> Msg = new List<string>();
            using (DataBaseContext db = new DataBaseContext())
            {
                try
                {
                    var blog1 = db.TrainingExpenses.Where(e => e.Id == data).SingleOrDefault();
                    ...
                    if (ModelState.IsValid)
                    {
                        try
                        {
                            ...
                            using (TransactionScope ts = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                            {
                                var Curr_Lookup = db.TrainingExpenses.Find(data);
                                ...
                                if (DBTrackFile.ByteArrayCompare((byte[])TempData["CurrRowVersion"], (byte[])TempData["RowVersion"]) == true)
                                {
                                    ...
                                    TrainingExpenses post = new TrainingExpenses()
                                    {
                                        FoodFees = L.FoodFees,
                                        MiscFees = L.MiscFees,
                                        StayFees = L.StayFees,
                                        TrainingFees = L.TrainingFees,
                                        TravelFees = L.TravelFees,
                                        Id = data,
                                        DBTrack = blog1.DBTrack,
                                        //  RowVersion = (Byte[])TempData["RowVersion"]
                                    };

                                    db.TrainingExpenses.Attach(post);
        ...
        public ActionResult Delete(int? data)
        {
            ...
            using (DataBaseContext db = new DataBaseContext())
            {
                ...
                TrainingExpenses TrainingExpenses = db.TrainingExpenses.Where(e => e.Id == data).SingleOrDefault();
                try
                {
                    //var selectedValues = TrainingExpenses.SocialActivities;
                    //var lkValue = new HashSet<int>(TrainingExpenses.SocialActivities.Select(e => e.Id));
                    //if (lkValue.Count > 0)
                    //{
                    //    return this.Json(new Object[] { "", "Child record exists.Cannot delete.", JsonRequestBehavior.AllowGet });
                    //}

                    var chk = db.TrainingSchedule.Include(e => e.Expenses).Where(e => e.Expenses.Id == data).ToList();
                    if (chk.Count() != 0)
                    {
                        Msg.Add("Unable to delete, record is used in TrainingSchedule");
                        return Json(new Utility.JsonReturnClass { success = false, responseText = Msg }, JsonRequestBehavior.AllowGet);
                    }
                    using (TransactionScope ts = new TransactionScope())
                    {
                        db.Entry(TrainingExpenses).State = System.Data.Entity.EntityState.Deleted;
                        db.SaveChanges();
                        ts.Complete();
                    }
        ...
        public ActionResult P2BGrid(P2BGrid_Parameters gp)
        {
            try
            {
                ...
                var LKVal = db.TrainingExpenses.ToList();

                if (gp.IsAutho == true)
                {
                    LKVal = db.TrainingExpenses.AsNoTracking().Where(e => e.DBTrack.IsModified == true).ToList();
                }
                else
                {
                    LKVal = db.TrainingExpenses.AsNoTracking().ToList();
                }


                IEnumerable<TrainingExpenses> IE;
                if (!string.IsNullOrEmpty(gp.searchString))
                {
                    IE = LKVal;
                    if (gp.searchOper.Equals("eq"))
                    {
                        jsonData = IE.Select(a => new { a.Id, a.FoodFees, a.MiscFees, a.StayFees, a.TrainingFees, a.TravelFees }).Where((e => (e.Id.ToString() == gp.searchString) || (e.FoodFees.ToString() == gp.searchString.ToLower()) || (e.MiscFees.ToString() == gp.searchString.ToLower()) || (e.StayFees.ToString() == gp.searchString.ToLower()) || (e.TrainingFees.ToString() == gp.searchString.ToLower()) || (e.TravelFees.ToString() == gp.searchString.ToLower())));
                    }
                    if (pageIndex > 1)
                    {
                        int h = pageIndex * pageSize;
                        jsonData = IE.Skip(pageIndex).Take(pageSize).Select(a => new Object[] { a.Id, a.FoodFees, a.MiscFees, a.StayFees, a.TrainingFees, a.TravelFees }).ToList();
                    }
                ...
                {
                    IE = LKVal;
                    Func<TrainingExpenses, dynamic> orderfuc;
                    if (gp.sidx == "Id")
                    {
                    ...
                    {
                        orderfuc = (c => gp.sidx == "FoodFees" ? c.FoodFees.ToString() :
                                         gp.sidx == "MiscFees " ? c.MiscFees.ToString() :
                                         gp.sidx == "StayFees " ? c.StayFees.ToString() :
                                         gp.sidx == "TrainingFees " ? c.TrainingFees.ToString() :
                                         gp.sidx == "TravelFees " ? c.TravelFees.ToString() :
                                         "");
                    }
                    ...
                    if (gp.sord == "asc")
                    {
                        ...
                        jsonData = IE.Select(a => new Object[] { a.Id, a.FoodFees, a.MiscFees, a.StayFees, a.TrainingFees, a.TravelFees }).ToList();
                    }
