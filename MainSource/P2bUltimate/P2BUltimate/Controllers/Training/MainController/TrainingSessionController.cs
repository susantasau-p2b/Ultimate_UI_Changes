    ...
    public class TrainingSessionController : Controller
    {
        ...
        public ActionResult P2BGrid(P2BGrid_Parameters gp)
        {
            try
            {
                ...
                if (gp.IsAutho == true)
                {
                    TrainingSession = db.TrainingSession
                ...
                else
                {
                    TrainingSession = db.TrainingSession
                        .Include(e => e.SessionType)
                        .Include(e => e.TrainingProgramCalendar)
                        .Include(e => e.TrainingProgramCalendar.ProgramList).AsNoTracking().ToList();
                }
                ...
                if (!string.IsNullOrEmpty(gp.searchField))
                {
                    ...
                    {
                        jsonData = IE.Where(e => (e.SessionDate.Value.ToShortDateString().ToString().Contains(gp.searchString))
                            || (e.StartTime.ToUpper().ToString().Contains(gp.searchString.ToUpper()))
                             || (e.EndTime.ToUpper().ToString().Contains(gp.searchString.ToUpper()))
                            || (e.SessionType != null ?e.SessionType.LookupVal.ToString().Contains(gp.searchString) : false)
                            ||  (e.TrainingProgramCalendar!= null ?e.TrainingProgramCalendar.ProgramList.FullDetails.ToString().Contains(gp.searchString): false)
                            || (e.Id.ToString().Contains(gp.searchString))
                          ).Select(a => new { a.SessionDate , a.StartTime,a.EndTime ,a.SessionType,a.TrainingProgramCalendar ,a.Id }).ToList();

                        //jsonData = IE.Select(a => new { a.SessionDate, a.StartTime, a.Id }).Where((e => (e.Id.ToString() == gp.searchString))).ToList();
                    ...
                    if (pageIndex > 1)
                    {
                        ...
                        jsonData = IE.Skip(pageIndex).Take(pageSize).Select(a => new Object[] { a.SessionDate.Value.ToShortDateString(), a.StartTime, a.EndTime, a.SessionType != null ? Convert.ToString(a.SessionType.LookupVal) : "", a.TrainingProgramCalendar.ProgramList.FullDetails, a.Id }).ToList();
                    }
                ...
                else
                {
                    ...
                    {
                        orderfuc = (c => gp.sidx == "SessionDate" ? Convert.ToString(c.SessionDate.Value.ToShortDateString()) :
                                         gp.sidx == "StartTime" ? Convert.ToString(c.StartTime) :
                                          gp.sidx == "Endtime" ? Convert.ToString(c.EndTime) :
                                         gp.sidx == "SessionType" ? c.SessionType.LookupVal :
                                         gp.sidx == "TrainingProgramCalendar" ? c.TrainingProgramCalendar.ProgramList.FullDetails : "");
                    }
                    if (gp.sord == "asc")
                    {
                        IE = IE.OrderBy(orderfuc);
                        jsonData = IE.Select(a => new Object[] {  a.SessionDate.Value.ToShortDateString(), Convert.ToString(a.StartTime), Convert.ToString(a.EndTime), a.SessionType != null ? Convert.ToString(a.SessionType.LookupVal) : "", a.TrainingProgramCalendar != null ? a.TrainingProgramCalendar.ProgramList.FullDetails : "", a.Id }).ToList();
                    }
                    ...
                    {
                        int h = pageIndex * pageSize;
                        jsonData = IE.Skip(pageIndex).Take(pageSize).Select(a => new Object[] { a.SessionDate.Value.ToShortDateString(), a.StartTime, Convert.ToString(a.EndTime), a.SessionType != null ? Convert.ToString(a.SessionType.LookupVal) : "", a.TrainingProgramCalendar != null ? a.TrainingProgramCalendar.ProgramList.FullDetails : "", a.Id }).ToList();
                    }
        ...
        public ActionResult Create(TrainingSession c, FormCollection form) //Create submit
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                ...
                try
                {
                    ...
                    string TrainingProgramCalendar_List = form["TrainingProgramCalendar_List"] == "0" ? "" : form["TrainingProgramCalendar_List"];
                    ...
                    if (TrainingProgramCalendar_List != null)
                    {
                        if (FactExte != "")
                        {
                            int FactId = Convert.ToInt32(TrainingProgramCalendar_List);
                            var vals = db.TrainingProgramCalendar.Where(e => e.Id == FactId).SingleOrDefault();
                            c.TrainingProgramCalendar = vals;
                        }
                    }
                    ...
                    if (TrainingProgramCalendar_List != null)
                    {
                        if (FactExte != "")
                        {
                            int FactId = Convert.ToInt32(TrainingProgramCalendar_List);
                            var vals = db.TrainingProgramCalendar.Where(e => e.Id == FactId).SingleOrDefault();
                            if (c.SessionDate < vals.StartDate || c.SessionDate > vals.EndDate)
                            {
                                Msg.Add(" Session Date should be between TrainingProgram Calendar");
                    ...
                    if (ModelState.IsValid)
                    {
                        using (TransactionScope ts = new TransactionScope())
                        {
                            ...
                            //TrainingProgramCalendar TrPgmCal = db.TrainingProgramCalendar.Include(e => e.ProgramList).Where(e => c.SessionDate >= e.StartDate &&   c.SessionDate <= e.EndDate ).FirstOrDefault();
                            TrainingSession trnsession = new TrainingSession()
                            {
                                EndTime = c.EndTime,
                                Faculty = c.Faculty,
                                TrainingProgramCalendar = c.TrainingProgramCalendar,
            ...
            using (DataBaseContext db = new DataBaseContext())
            {
                ...
                var add_data = db.TrainingSession
                    ...
                    .Select(e => new
                    {
                        ...
                        TraProgCal_Details = e.TrainingProgramCalendar.FullDetails == null ? "" : e.TrainingProgramCalendar.FullDetails,
                        TraProgCal = e.TrainingProgramCalendar.Id == null ? "" : e.TrainingProgramCalendar.Id.ToString(),
        ...
        public async Task<ActionResult> EditSave(TrainingSession c, int data, FormCollection form) // Edit submit
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                ...
                try
                {
                    ...
                    if (TrainingProgramCalendar_List != null)
                    {
                        if (TrainingProgramCalendar_List != "")
                        {
                            int ContId = Convert.ToInt32(TrainingProgramCalendar_List);
                            var val = db.TrainingProgramCalendar
                                                .Where(e => e.Id == ContId).SingleOrDefault();
                            c.TrainingProgramCalendar = val;
                        }
                    }
                            ...
                            try
                            {
                                ...
                                using (TransactionScope ts = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                                {
                                    ...
                                    if (TrainingProgramCalendar_List != null)
                                    {
                                        if (TrainingProgramCalendar_List != "")
                                        {
                                            var val = db.TrainingProgramCalendar.Find(int.Parse(TrainingProgramCalendar_List));
                                            c.TrainingProgramCalendar = val;

                                            var add = db.TrainingSession.Include(e => e.TrainingProgramCalendar).Where(e => e.Id == data).SingleOrDefault();
                                            IList<TrainingSession> addressdetails = null;
                                            if (add.TrainingProgramCalendar != null)
                                            {
                                                addressdetails = db.TrainingSession.Where(x => x.TrainingProgramCalendar.Id == add.TrainingProgramCalendar.Id && x.Id == data).ToList();
                                            }
                                            else
                                            {
                                                addressdetails = db.TrainingSession.Where(x => x.Id == data).ToList();
                                            ...
                                            if (addressdetails != null)
                                            {
                                                foreach (var s in addressdetails)
                                                {
                                                    s.TrainingProgramCalendar = c.TrainingProgramCalendar;
                                                    db.TrainingSession.Attach(s);
                                                    db.Entry(s).State = System.Data.Entity.EntityState.Modified;
                                                    db.SaveChanges();
                                                    TempData["RowVersion"] = s.RowVersion;
                                    ...
                                    {
                                        var addressdetails = db.TrainingSession.Include(e => e.TrainingProgramCalendar).Where(x => x.Id == data).ToList();
                                        foreach (var s in addressdetails)
                                        {
                                            s.TrainingProgramCalendar = null;
                                            db.TrainingSession.Attach(s);
                                            db.Entry(s).State = System.Data.Entity.EntityState.Modified;
                                            db.SaveChanges();
                                            TempData["RowVersion"] = s.RowVersion;
                                    ...
                                    if (DBTrackFile.ByteArrayCompare((byte[])TempData["CurrRowVersion"], (byte[])TempData["RowVersion"]) == true)
                                    {
                                        TrainingSession corp = new TrainingSession()
                                        {
                                            ...
                                            TrainingProgramCalendar_Id = c.TrainingProgramCalendar.Id,
                                            SessionDate = c.SessionDate,
                                            SessionType_Id = c.SessionType.Id,
                                            StartTime = c.StartTime,
                                            TotalHours = c.TotalHours,
                                            TrainerName = c.TrainerName,
                                            Id = data,
        ...
        [HttpPost]
        public ActionResult GetTrainingProgramCalendar(List<int> SkipIds)
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                //List<string> Ids = SkipIds.ToString();
                var fall = db.TrainingProgramCalendar.Include(e => e.ProgramList).ToList();
