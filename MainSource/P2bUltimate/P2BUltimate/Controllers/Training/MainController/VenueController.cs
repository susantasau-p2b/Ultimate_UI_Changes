...
namespace P2BUltimate.Controllers.Training.MainController
{
    public class VenueController : Controller
    {
        ...
        public ActionResult RecruitmentIndex()
        {
            return View("~/Views/Recruitement/MainView/Venue/Index.cshtml");
        }
        ...
        public ActionResult RecruitVenueCreate(Venue c, FormCollection form) //Create submit
        {
            using (DataBaseContext db = new DataBaseContext())
            {
                ...
                var companypayroll = db.CompanyRecruitment.Where(e => e.Company.Id == id).SingleOrDefault();
                    ...
                    using (TransactionScope ts = new TransactionScope())
                    {
                        ...
                        try
                        {
                            ...
                            if (companypayroll != null)
                            {
                                ...
                                companypayroll.RecruitVenue = pfmasterlist;
        ...
        //                                //blog1.VenuType = val;
        //                                var val = db.Lookup.Include(e => e.LookupValues).Where(e => e.Code == "1054").FirstOrDefault().LookupValues.Where(e => e.Id == int.Parse(Corp)).FirstOrDefault();
        //                                L.VenuType = val;
                ...
                try
                {
                    ...
                    if (Addrs != null)
                    {
                        if (Addrs != "")
                        {
                            ...
                            var add = db.Venue.Include(e => e.Address).Where(e => e.Id == data).SingleOrDefault();
                            IList<Venue> addressdetails = null;
                            if (add.Address != null)
                            {
                                addressdetails = db.Venue.Where(x => x.Address.Id == add.Address.Id && x.Id == data).ToList();
                            }
                            else
                            {
                                addressdetails = db.Venue.Where(x => x.Id == data).ToList();
                            }
                    ...
                    {
                        var addressdetails = db.Venue.Include(e => e.Address).Where(x => x.Id == data).ToList();
                        foreach (var s in addressdetails)
                        {
                            s.Address = null;
                            db.Venue.Attach(s);
                    ...
                    if (VenueType != null)
                    {
                        if (VenueType != "")
                        {
                            var val = db.Lookup.Include(e => e.LookupValues).Where(e => e.Code == "1054").FirstOrDefault().LookupValues.Where(e => e.Id == int.Parse(VenueType)).FirstOrDefault();
                            c.VenuType = val;
                        ...
                        if (ModelState.IsValid)
                        {
                            try
                            {
                                ...
                                using (TransactionScope ts = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                                {
                                    ...
                                    //using (var context = new DataBaseContext())
                                    //{
                                    blog = db.Venue.Where(e => e.Id == data)
                                                            .Include(e => e.Address).Include(e => e.VenuType)
                                    ...
                                    var s = db.Venue.Include(e => e.VenuType).Include(x => x.Address).Where(e => e.Id == data).FirstOrDefault();

                                    s.ContactDetails = c.ContactDetails;
                                    s.Address = c.Address;
                                    s.Narration = c.Narration;
                                    s.VenuType = c.VenuType;
                                    s.Name = c.Name;
                                    ...
                                    if (DBTrackFile.ByteArrayCompare((byte[])TempData["CurrRowVersion"], (byte[])TempData["RowVersion"]) == true)
                                    {
                                        ...
                                        ts.Complete();
                                        Msg.Add("  Record Updated  ");
                                        // return Json(new Utility.JsonReturnClass { Id = s.Id, Val = s.VenuType.LookupVal, success = true, responseText = Msg }, JsonRequestBehavior.AllowGet);
                                        return Json(new Utility.JsonReturnClass { success = true, responseText = Msg }, JsonRequestBehavior.AllowGet);
                    ...
                    else
                    {
                        using (TransactionScope ts = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                        {
                            ...
                            using (var context = new DataBaseContext())
                            {
                                var obj = DBTrackFile.ModifiedDataHistory("Core/P2b.Global", "M", blog, corp, "Venue", c.DBTrack);

                                Old_Corp = context.Venue.Where(e => e.Id == data)
                            ...
                            ts.Complete();
                            Msg.Add("  Record Updated  ");
                            //return Json(new Utility.JsonReturnClass { Id = blog.Id, Val = c.VenuType.LookupVal, success = true, responseText = Msg }, JsonRequestBehavior.AllowGet);
                            return Json(new Utility.JsonReturnClass { success = true, responseText = Msg }, JsonRequestBehavior.AllowGet);
                ...
                if (!string.IsNullOrEmpty(gp.searchField))
                {
                    ...
                    if (gp.searchOper.Equals("eq"))
                    {
                        jsonData = IE.Where(e => (e.Name.ToUpper().ToString().Contains(gp.searchString.ToUpper()))
                               || (e.ContactPerson.ToUpper().ToString().Contains(gp.searchString.ToUpper()))
                               || (e.VenuType.LookupVal.ToUpper().ToString().Contains(gp.searchString.ToUpper()))
                               || (e.Address != null ? e.Address.City.FullDetails.ToString().Contains(gp.searchString.ToUpper()) : false)
                               || (e.Id.ToString().Contains(gp.searchString))
                               ).Select(a => new Object[] { a.Name, a.ContactPerson, a.VenuType.LookupVal, a.Address != null ? a.Address.City.FullDetails : "", a.Id }).ToList();
                    ...
                    if (pageIndex > 1)
                    {
                        ...
                        jsonData = IE.Skip(pageIndex).Take(pageSize).Select(a => new Object[] { a.Name, a.ContactPerson, a.VenuType.LookupVal, a.Address == null ? "" : a.Address.City == null ? "" : a.Address.City.FullDetails, a.Id }).ToList();
                    }
                ...
                else
                {
                    ...
                    else
                    {
                        orderfuc = (c => gp.sidx == "Name" ? c.Name :
                                            ...
                                            gp.sidx == "Address" ? c.Address == null ? "" : c.Address.City == null ? "" : c.Address.City.FullDetails :
                                            "");
                    }
                    if (gp.sord == "asc")
                    {
                        IE = IE.OrderBy(orderfuc);
                        jsonData = IE.Select(a => new Object[] { a.Name, a.ContactPerson, a.VenuType.LookupVal, a.Address == null ? "" : a.Address.City == null ? "" : a.Address.City.FullDetails, a.Id }).ToList();
                    }
        ...
        public ActionResult P2BGridRecruitVenue(P2BGrid_Parameters gp)
        {
            try
            {
                DataBaseContext db = new DataBaseContext();
                ...
                IEnumerable<Venue> Venue = null;
                CompanyRecruitment companydata = null;
                var compid = Convert.ToInt32(Session["CompId"].ToString());
                if (gp.IsAutho == true)
                {
                    companydata = db.CompanyRecruitment
                        .Include(e => e.RecruitVenue)
                        .Include(e => e.RecruitVenue.Select(t => t.Address))
                        .Include(e => e.RecruitVenue.Select(t => t.Address.City))
                        .Include(e => e.RecruitVenue.Select(t => t.VenuType))
                ...
                else
                {
                    companydata = db.CompanyRecruitment
                        ...
                        .Include(e => e.RecruitVenue.Select(t => t.Address))
                        .Include(e => e.RecruitVenue.Select(t => t.Address.City))
                        .Include(e => e.RecruitVenue.Select(t => t.VenuType)).Where(e => e.Company.Id == compid).SingleOrDefault();
                }
                IEnumerable<Venue> IE;
                if (!string.IsNullOrEmpty(gp.searchField))
                {
                    IE = companydata.RecruitVenue.ToList();
                    if (gp.searchOper.Equals("eq"))
                    {
                        jsonData = IE.Where(e => (e.Name.ToUpper().ToString().Contains(gp.searchString.ToUpper()))
                               || (e.ContactPerson.ToUpper().ToString().Contains(gp.searchString.ToUpper()))
                ...
                else
                {
                    ...
                    totalRecords = companydata.RecruitVenue.Count();
                }
                if (totalRecords > 0)
                {
                    totalPages = (int)Math.Ceiling((float)totalRecords / (float)gp.rows);
