...
namespace P2BUltimate.Process
{
    public static class SalaryHeadGenProcess
    {
        ...
        public static SalHeadFormula _returnSalHeadFormula(int? OSalHeadFormula)
        {
            using (DataBaseContext db = new DataBaseContext())
            {
            ...
            using (DataBaseContext db = new DataBaseContext())
            {
                ...
                if (OSalHeadFormualQuery.ServiceDependRule != null && mSalWages != 0)
                {
                    ...
                    using (var streamReader = new StreamReader(localPathchkNet))
                    {
                        ...
                        {
                            string[] OParam = line.Split('_');

                            var JoinConfermation = OParam[0];//line.Split('_')[0];
                            ServiceDependRule = JoinConfermation;
                            var comp = OParam[1];//line.Split('_')[1];
                            Compcodechk = comp;
                            if (OParam.Count() == 3)
                                SalaryMonthDays = OParam[2];
                    ...
                    if (OJoiningService != "")
                    {
                        ...
                        if (SalaryMonthDays != null && SalaryMonthDays != "") //For RajshreeShahu bank
                        {
                            var mPayMonth = "";
                            mPayMonth = mCPIMonth;
                            DateTime start = Convert.ToDateTime(OJoiningService);
                            DateTime end = Convert.ToDateTime(SalaryMonthDays + "/" + mPayMonth).Date;// DateTime.Now.Date;
                            int compMonth = (end.Month + end.Year * 12) - (start.Month + start.Year * 12);
                            double daysInEndMonth = (end - start).Days;
                            mService = Math.Round(daysInEndMonth / 365, 2);
                ...
                if (OSalHeadFormualQuery.VDADependRule != null && mSalWages != 0)
                {
                    ...
                    string pathLoanNet = requiredPathLoanNet + @"\VDADependRule" + ".ini";
                    localPathLoanNet = new Uri(pathLoanNet).LocalPath;
                    if (!System.IO.File.Exists(localPathLoanNet))
                    {
                    ...
                    string pathchkNet = requiredPathchkNet + @"\VDADependRule" + ".ini";
                    localPathchkNet = new Uri(pathchkNet).LocalPath;
                    using (var streamReader = new StreamReader(localPathchkNet))
                    {
                        string line;
                        ...
                        {
                            string[] OParam = line.Split('_');
                            var JoinConfirmation = OParam[1];
                            VDADependRule = JoinConfirmation;
                            var comp = OParam[0];
                            Compcodechk = comp;
                            if (OParam.Count() == 3)
                                SalaryMonthDays = OParam[2];
                    ...
                    if (OCPIRule.VDAOnDirectBasic == true)
                    {
                        ...
                        for (int i = 0; i < OCPIRuleDetailList.Count(); i++)
                        {
                            ...
                            else // service range vda calculation
                            {
                                ...
                                if (VDADependRule == "" || VDADependRule == null)
                                {
                                    if (vservicedate.JoiningDate != null)
                                    {
                                        OJoiningService = vservicedate.JoiningDate;
                                    }

                                }
                                else if (Compcodechk == "ADCC")
                                {
                                    if (vservicedate.LastPromotionDate == null && vservicedate.JoiningDate != null)
                                    {
                                        OJoiningService = vservicedate.JoiningDate;
                                    }
                                    else
                                    {
                                        OJoiningService = vservicedate.LastPromotionDate;
                                    }
                                }
                                else
                                {
                                    if (VDADependRule.ToUpper() == "JOININGDATE")
                                    {
                                        if (vservicedate.JoiningDate != null)
                                        {
                                            OJoiningService = vservicedate.JoiningDate;
                                    ...
                                    else if (VDADependRule.ToUpper() == "CONFIRMATIONDATE")
                                    {
                                        if (vservicedate.ConfirmationDate != null)
                                        {
                                            OJoiningService = vservicedate.ConfirmationDate;
                                        }
                                    }
                                }
                                if (SalaryMonthDays != null && SalaryMonthDays != "") //For RajshreeShahu bank
                        ...
                        .FirstOrDefault();
                            ...
                            else
                            {
                                ...
                                else // service range vda calculation
                                {
                                    ...
                                    if (SalaryMonthDays != "") //For RajshreeShahu bank
                                    {
                                        DateTime start = OJoiningService.Value;
                                        DateTime end = Convert.ToDateTime(SalaryMonthDays + "/" + mPayMonth).Date;// DateTime.Now.Date;
                                        int compMonth = (end.Month + end.Year * 12) - (start.Month + start.Year * 12);
